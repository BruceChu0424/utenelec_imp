...
| 销售收款草稿 | 引用同客户、同币种的未结 AR；应收币种只读 | 每行填写到账原币和到账汇率 | 服务端自动算到账本币、账面冲减、费用和汇兑差额 |
| 销售收款审核 | 由非制单人复核 | 不接受客户端本币结果 | 同事务核销 AR、更新账户余额和资金流水 |
| 总账 | 具备 `finance_post:execute` 的人员按期间生成/重建 | 读取已冻结的开账/到账事实 | 生成销售立账、成本、收款、费用和汇兑 AUTO 凭证并校验平衡 |
...
**最后更新**：2026-08-09 · **状态**：钱流/报表使用真实后端；销售订单原币、`SHIPPED` 正式 AR、销售收款到账汇率和按期间总账边界已同步 ADR-030；V196–V202 已包含在公司目标库 V238，V229 已改为财务审核组；资产完整生命周期、采购/委外 IQC、真实职责分离/实物 UAT 和发布签字仍为 **NO-GO**
