# 老库数据迁移 · 总索引
> **当前共享源码候选（2026-08-29）**：目录最高 V425，共 387 个迁移文件、387 个唯一版本且无重号。
> V425 审计日志全新开始：清空 `audit_log` 与冷归档 `audit_log_archive` 并 `RESTART IDENTITY`
> 让自增 ID 从 1 重新累计（新库/离线 bootstrap 目标库为空操作，业务数据不受影响），
> 详见 [迁移说明 62](62-V424审计日志降噪与中文化.md)。
...
> **离线 bootstrap 冻结状态**：`server/legacy_migration/migrate.sh`、安全合同和说明已同步到 V425/387 与 `bootstrap-v9-v425`。这只表示源码校验常量一致；尚未生成并核验受保护 lineage 的 387 行 checksum manifest，也未在目标可恢复副本执行 bootstrap/对账/恢复，因此目标库和发布仍为 **NO-GO**。
