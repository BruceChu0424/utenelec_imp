...
- 进入路径：
  ...
  - [我的页](我的页.md) → "工资条"快捷入口
  - 主导航"我的工资条"
- 在导航中的位置：**员工主导航**项（hr 角色看到的是全员视角，本页 Phase 1 仅渲染本人）。
...
1. 顶部 `UtenSegmentedFilter` 4 段：全部 / 已发布 / 已查看 / 已下载，切换实时筛选 `payrollFilterProvider`。
...
7. 数据来源 `payrollListProvider`（Phase 1 Mock 仓库），后续接入后端 `GET /payroll/slips?status=`。
...
- `PayrollBatch`：Phase 2 由 hr 生成批次后才会出现员工可见的 PayrollSlip

## 八、权限要求

- **全员可见**（`employee / hr / finance / manager / admin` 均可）。
- 数据范围：Phase 1 默认仅返回**当前登录用户**的工资条；hr/manager 角色后续可看全员（在 [工资条生成页](工资条生成页.md) 等 Phase 2 页面）。
- 关键操作权限点：列表只读 + 标记本人查看，无写操作权限点。
...
- 网络断开时：Phase 1 为内存 Mock 无影响；后端接入后需：
  - 列表数据缓存上次成功结果，顶部显示"离线缓存"提示。
  - `markPayrollViewed` 写入需排队，恢复网络后同步到服务端。
