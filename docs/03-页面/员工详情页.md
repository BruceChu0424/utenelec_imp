...
## 十、2026-07-31 权限与任职完整性

- Hero 姓名前展示部门负责人、领导层或班组管理标识，负责人优先。
- 仅 `superAdmin + authorization:manage` 显示个人“权限设置”；前端先通过 employeeId 精确解析账号，再深链 `/admin/permissions?employeeId=...`。未开通账号时给出明确提示。
- 编辑页中的部门改为只读；调整部门必须使用“调岗”以写入 `employment_history`。
- 在册员工不能直接归档，需先办理离职；删除入口只对已离职员工显示，后端再次校验。
- 未来入职、未来生效、早于入职日或早于最新任职事件的调岗/离职会被拒绝。
- 负责人调离、离职或归档时，原部门的负责人引用自动清空。

**最后更新**：2026-07-31 · **状态**：个人权限入口、领导标识和任职生命周期守卫已实现。
