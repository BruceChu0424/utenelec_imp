...
员工账号密码登录的独立全屏页，不在业务 `ShellRoute` 内。**2026-09-25 起未登录直接进入本页**：
入口选择页 `/entry`（内部人员/访客二选一）与访客登录前端已下线——本平台定位公司内部；
`/entry` 旧深链由路由重定向到本页，访客后端能力保留，将来另做独立入口。这里已经接真实
...
- 访客曾使用独立 `/visitor/login` 和访客 session（2026-09-25 访客门户前端下线，`/visitor*` 由 redirect 拦到本页；后端能力保留）。
...
5. 启动时，`sessionProvider` 从权威原子记录恢复并调用 `/api/auth/me`；在途 refresh 已换代时
   ...
   当成凭据失效。
6. 同一浏览器可开多个标签页分别登录**不同账号**，互不顶替、互不影响（ADR-061）；新开标签页
   不自动带入已有会话，需在该标签页显式登录。旧版共享记录在首次启动时被当前标签页一次性收编
   （迁入本标签页并删除共享副本）。
7. “记住此设备”和“忘记密码”控件已移除。每次显式登录都必须输入密码；当前也没有用户自助找回
