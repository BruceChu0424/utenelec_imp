...
车间路线、物料来源、实际耗用与完成目标分别有权威事实：先确认路线，按真实普通仓/直送投入显式开工，同任务分次补料与报工。
工作台、页面和命令共用权限及状态能力；操作者有效性、动作授权、对象范围和计划暂停状态分别检查，负责人/编制人身份不替代动作权限。

V614的提前完结/撤回按精确日报事件变更目标和未承诺材料；V615按来源批次记录投入、退回与释放；V616用真实实物归属控制车间改派。
未实发领料草稿的收料归属只凭同事务新ASSIGNMENT证据同步，旧事件不可刷新后冒充新授权。普通仓与技术线边位置保持不同用途及原UUID。

同需求的多条报工展示行共享唯一用料输入；复制、删除和勾选不产生第二份材料额度，也不丢失已填事实。既有草稿未读取成功时不能编辑覆盖，
材料读取失败保留已保存用料/退料意愿，直送候选失败保留原指定去向。后台到料通知按事件证据重查，反向撤回旧卡，200条游标分页处理。
业务细节与验证以[场景矩阵](../07-业务链路/车间任务路线与供料场景矩阵.md)和[并行对抗验证](../99-项目治理/2026-09-19-车间全链路并行对抗验证.md)为准。
...
| 13 | 会话一致性与弱网恢复 | 原子 token record（按标签页隔离，ADR-061）+ latest-intent/CAS + logout fence；安全读有限重试与严格 health | SecureStorage/SessionNotifier/AuthInterceptor + ConnectionRecovery | 🟡 源码候选；目标浏览器、网关、故障注入和生产运行仍为 NO-GO |
...
| 15 | 生产分析、路线与持续供料 | 各层责任确认后下达；车间明确齐套/持续/独立分批路线，普通仓与真实上下层直送混合供料；预计供给不抵物理缺口 | V234–V250及V609–V616；MaterialAnalysis/Preplan/Execution/IQC同源唤醒 | 源码与隔离验证范围见[并行对抗验收](../99-项目治理/2026-09-19-车间全链路并行对抗验证.md)，不代表已部署 |
...
>
> ```
> 有效权限 = 全员基础包(permissions.baseline，人人有份；2026-09-23 ADR-109 起管理页可编辑，角色表已删除)
...
>   `authorization:manage` 任一权限可进入；support-only 只渲染账号列表/补开账号/状态/重置密码，不请求
>   权限目录、有效权限、个人覆盖、数据范围或部门授权；真正授权管理仍只允许超管；
> - 业务页 `/page-permissions/:surfaceKey` 的人员列表包含未开户员工。未开户只显示明确空态；
>   当前操作者同时持 `account:support` 才可询问并执行开户，负责人身份或
>   `authorization:manage` 不替代账号支持权限；必须确认保存并关闭一次性凭据后才刷新账号状态和
>   加载权限，凭据关闭后不可再次读取；
...
>   唯一对象例外是 `GET /api/profile/me`：服务端从当前认证主体固定解析本人，不接受目标员工 ID；
>   本人个人 PII 可明文返回，但银行/薪酬仍按原显式权限，且不会扩大到他人档案。

### 1.1 设计决策

| 问题 | 决策 |
|---|---|
| 权限粒度 | **权限点为主**(2026-07-24 起角色已下线，2026-09-23 ADR-109 删除角色表；staff JWT 已移除 roles/perms claim，响应 profile 只下发服务端合成的 permissions，不再有 roles 字段；「能怎么授」只看 `permissions.grant_policy`) |
...
| 本人资料对象范围 | `GET /api/profile/me` 不接受 employeeId，只从 `AuthUser.employeeId` 取本人；未绑定返回明确 NOT_FOUND；个人 PII 可见，银行/薪酬仍按独立权限 |
...
abstract final class Perm {
  ...
  static const visitorHostConfirm = 'visitor:host_confirm'; // 接待访客 = 对外接待白名单(V679)
...
多级审批属于尚未采用的通用目标模型。员工报销按 [ADR-094](../99-决策记录-ADR/ADR-094-报销链路完整化.md)
明确保持一次审核与独立付款，以适应当前岗位操作；不得以本节历史目标额外增加员工审批步骤。
只有公司审批制度明确需要多级时才另行设计节点、授权与迁移。
...
**当前固定流程**（V608/ADR-094 完整化）：员工创建并提交（可附发票影像 + 结构化发票登记，
支持图片识别预填与报销域内号码查重）→ 有 `expense:approve` 权限的非申请人通过或驳回（可批量，
单事务全成全败）→ 有 `expense:pay` 权限的非申请人打款（生成 YF 财务费用单）。仍不是
主管 + 财务的多级审批。每次流转写 `expense_claim_events`(操作人姓名快照)，
审批轨迹由事件表渲染。操作与人工查验、税务及档案边界见[员工报销 SOP](../07-业务链路/员工报销全链路-SOP.md)。

```mermaid
stateDiagram-v2
    [*] --> draft
    draft --> submitted: 员工提交
    submitted --> approved: 单阶段审批通过（可批量）
    submitted --> rejected: 驳回
    rejected --> submitted: 修订后重新提交（V608）
    approved --> paid: 财务登记实际付款
    submitted --> draft: 员工撤回
    draft --> draft: 编辑（PUT，明细整组替换）
    paid --> [*]
```

`REVIEWING` 状态在 V133 中预留，服务允许把它与 `SUBMITTED` 一样审批或撤回，但当前没有
进入/推进多级节点的接口。`REJECTED` 不再是终点：申请人可编辑（`PUT /{id}`，限 REJECTED
本人)后重新提交，当前驳回字段复位但事件历史保留，并重新通知审批人。V608 发票登记
“代码+号码”唯一约束只覆盖报销域的已登记号码，不能称为全部财务入账去重或官方查验。
数电票20位号码依据税务总局2024年第11号公告；其他凭证的适用类型与校验以报销专题为准。
付款使用行锁并在同一事务内扣减系统账户、生成一般费用单、对账记录和总账凭证；同参重试
幂等、异参冲突，结果回显到详情。这里登记实际付款，未接银企直连，不证明银行已到账。
详情可打印 A4 公司内部费用报销展示单，原始凭证、会计凭证与档案移交另按财务制度管理。
...
- 报销详情通过 `ExpenseClaimTimeline` 渲染持久化 `expense_claim_events`，包含操作人、时间
  和意见；存量时间字段仅用于明确的历史兼容，不能据此伪造不存在的审核意见或操作者。
...
- 权限点：`expense:apply/approve/pay/settings`、`payroll:generate/review/publish/view:self/view:all/export`。
- 报销禁止申请人自审、自付，审核人不可支付自己批准的单据；工资目前没有制单/复核/发布人员分离校验。
- 两个领域保留数据库行锁和状态前置条件；报销写入口另传 `expectedVersion` 防止旧页面覆盖，不使用 `If-Match`。
...
- 历史缺口：`ExpenseClaim/PayrollBatch/PayrollSlip` 的版本原先不在HTTP请求间回传。2026-09-19报销已按ADR-094在DTO与写入口使用 `expectedVersion`，包含逐单批量版本；工资条是否补齐仍按工资域实际接口核对。版本保护应覆盖可编辑实体，不盲加给只追加事件、系统或查照表。
...
- [x] 状态变更使用行锁和状态前置条件
- [x] 报销按ADR-094保持单级审核与独立付款，事件、意见和附件已形成源码候选；实际岗位验收单独记录
- [ ] 业务确认工资四眼原则并在后端强制
...
> **审计现行口径(2026-09-23, ADR-105, V670-V673; 与下文历史描述冲突时以此为准)**
> 1. **请求层**: 成功的 GET/HEAD 默认不记, 只有控制器方法声明了 `@AuditedRead` 的敏感读取留请求行;
>    每个写请求由 `UserOperationAuditInterceptor` 从处理它的控制器方法生成一条语义业务事件
>    (动作码「资源.方法」, 目标取路径变量 id), 取代通用 http 行, 失败同样记录; 业务服务已写显式事件的请求不重复;
>    页面自动发起的会话维护写入(心跳、自动已读、界面偏好)在方法上声明 `@AuditAutomaticWrite`。
>    手工路径降噪清单已删除。安全事件写了就不再写通用行; 签名正确只是过期的令牌产生的 401 不记; 拒绝事件与匿名失败请求按固定一分钟窗口节流:
>    已登录会话同一路径每分钟一条, 匿名来源按「IP + 动作 + 状态码」每分钟一条(换路径换不出新键),
>    每个来源每分钟另有条数上限(会话 20、匿名 IP 5), 全部匿名来源合计每分钟最多 120 条;
>    详情查看按(操作人, 对象, 会话)30 分钟去重。
> 2. **数据变化层**: 行级审计改为 FULL / COLUMN_SCOPED / NONE 三清单(唯一登记处
>    `AuditTriggerCoverageMigrationContractTest`, 每条带理由, 新表建表迁移调用 `fn_audit_track_table` 登记),
>    「全表必审、审计触发器不许带 WHEN/列清单」与全表补挂 sweep 废止。INSERT/DELETE 存整行,
>    UPDATE 只存变化键(外加单号/编码等定位键), 忽略 updated_at/version/last_login_at 等易变列, 无变化不写;
>    `app.legacy_import='on'` 的遗留导入会话不写行审计。
> 3. **分类只算一次**: `AuditClassifier` 在写入时给出风险等级与事件类型并存储, 数据库行事件按清单直接赋值;
>    查询与显示只读存储列。
> 4. **存储**: `audit_log`/`audit_log_archive` 按北京时间月分区、只追加(运行账号无改删权限, 守卫触发器拒绝改删);
>    留存由 `fn_audit_retention_run()` 整月归档/整月删除并写 `audit_retention_completed` 事件; 函数自带「在线 + 归档
>    合计不少于 6 个月」的法定下限(运行账号改设置压不破), 分区 DDL 最多等锁 5 秒。
> 5. **操作人**: 账号只存保留后 4 位的脱敏值, 审计页列表/详情/导出与人员选择器也只显示后 4 位;
>    模拟身份期间记在真实操作人名下, `on_behalf_of` 标注被模拟的账号。
> 6. **业务不读审计**: 单据审核/红冲/驳回时间落在单据列上, 架构测试禁止业务代码 `FROM audit_log`。
...
#### 6.3.1 约束/守卫触发器的起跳条件(2026-09-23 ADR-106，V674-V676)

- **只在相关列真变了时起跳**：热表(预留、库存单据头/明细、计划明细、执行段、物料分析三表、估值节点/事件、权益事件)上
  凡是对 UPDATE 起跳的约束/守卫触发器都带 `WHEN (OLD.相关列 IS DISTINCT FROM NEW.相关列 ...)`；相关列 = 校验函数及其
  调用闭包对本表的全部读取。函数入口按行类型提前返回的条件写进 WHEN(`(条件) IS NOT TRUE`)。新写触发器照此办，
  `HotTableTriggerHygieneContractTest` 会拦住热表上不带 WHEN 的延迟约束触发器。
- **不新增 UPDATE OF**：列清单看不到 BEFORE 触发器改写的列，JPA 整行更新又会把所有列写进 SET；WHEN 在最终行上求值。
- **INSERT 与 UPDATE 共用的拆成两条**：原名留 INSERT(/DELETE)，`原名_upd` 管 UPDATE(DELETE 条件不同再起 `原名_del`)。
  重建触发器会把 `ENABLE ALWAYS` 复位，原来是 ALWAYS 的要逐条恢复(契约测试把 V674 重建的 38 条 ALWAYS 写死逐条断言，
  以后再拆出的变体须与原名启用模式一致)。
  需要 `SET CONSTRAINTS <名> IMMEDIATE` 立即校验更新路径时同时点名 `_upd`。测试要逼出延迟校验，改一个相关列再改回来，
  不要只改 `updated_at`。
- **单号不可变**：取号函数只挂 INSERT；UPDATE 由 `trg_business_document_<表>_upd`(`UPDATE OF 单号列 + WHEN`)接管。
- **货品基本单位**：「已被数量引用则不能改」在改单位时按 `fn_goods_quantity_reference_sources()` 现查(`fn_goods_quantity_unit_in_use`)，
  写数量引用前用 `GoodsQuantityBasisLocks.lockForQuantityUse` 对货品行取 `FOR KEY SHARE`。
- **收付款类别**：引用方共享咨询锁(`PaymentStyleHierarchyLock.lock`)，层级/状态变更方排他(`lockForHierarchyChange`)。
- **索引**：新迁移不得加出被前缀覆盖或与同列唯一约束重复的索引；估值图/需求/预留/计划明细/总账凭证的引用列与货品数量
  来源列要有领头索引(`SchemaIndexHygieneContractTest`，白名单逐条写原因；来源列的部分索引只认「该列 IS NOT NULL」条件)。
  新加的货品数量来源，货品列必须有指向货品的不可延迟外键，带行条件的来源必须是只追加表(同一契约测试钉住)。
- **仍未根治**：相关列变化时延迟校验仍是逐行、逐事件执行，同一个键的聚合会重复计算；按键去重、同表校验合并属于下一波
  写路径重构(ADR-106 §2.1)。
...
> 2026-08-25 ADR-050：`OwnerScope` 已拆成 `visibleOwners` 与 `writableOwners`。手工
> `user_data_scopes` 只增加查看；本人和正式人员交接来源才进入可写集合。客户 owner 为空改为
> “待分配”，单客户共享永远只读；财务审核/红冲等池化动作必须先有普通读范围，再以专用 action
> authority 写入。V393–V399 同时提供双端任职代际交接图、审计、对象/账号 CAS 和离职并发守卫。
...
员工本人资料同样属于对象级授权，而不是 `employee:view` 的特殊前端绕过：

- `GET /api/profile/me` 只接受当前已认证员工，不提供 employeeId 参数；访客拒绝，未绑定员工档案
  返回 `NOT_FOUND`，Flutter 分别呈现加载、失败、未绑定和真实未填写状态；
- 本人可查看自己的证件、主/备用手机号、人口属性、户籍/住址和紧急联系人明文，但该能力不生成
  `employee:pii:view`，不能用于读取任何同事；
- 银行字段仍要求 `employee:pii:view`，薪酬仍要求 `employee:compensation:view`；“我的”页本身
  有意不展示两类字段。普通 `GET /api/org/employees/{id}` 的他人脱敏合同保持不变。
...
- 审批必须同时满足具体动作权限和实时审核资格：财务部门树内在职、账号启用人员，或被 active 个人加授该 `approve/reject` 动作的跨部门人员。inactive 墓碑、已停用权限、超级管理员身份、已知 UUID 或本地角色都不能绕过。合格审核人仅在精确 PENDING case 上临时读取详情；业务详情不返回决定动作，财务任务中心以 `{caseId,expectedVersion}` batch 原子处理，case 结束后普通对象范围立即恢复。
- `inbound_expectations` 是未来到货任务，不是库存、AP、实物或质量结论。收货超过财务批准余量时，`ARRIVAL_EXCEPTION_PENDING` 只提交异常，原收货仍为草稿，库存/AP/累计均不变。
- 超量异常共享队列用 `view`；全批/自定义批准要求 `approve`，不批要求 `reject`，并叠加实时资格。批准量只绑定本异常/收货行，仓库仍须再次审核；未批准量只生成原下单人的退回任务。
- V196–V202 已包含在开发原库 `uten_imp` V244 和公司目标库 V238；截至 2026-08-23，共享工作树源码目录最高 V392/354，354 个版本号无重复。V288/V289 已提供现货借用结构、终态守卫与全 `public` 审计覆盖，在线 create/revoke、持久化、双趟生效计算和 Flutter UI 已形成源码候选。V253/234 与空库 V252/233 是 2026-08-11 阶段快照，一次性生产物料克隆仍是 V250/231 历史证据。V257–V392 不改变生产默认车间唯一真源 V192。目标库迁移、真实岗位权限、仓库/退回实物、完整 HTTP IQC/QMS 和发布签字仍未完成，生产 **NO-GO**。
...
- V248 将非线性包装需求冻结为 `EXACT_SNAPSHOT`；V249 要求执行段显式为 DEMANDED 或有证据的 ZERO_MATERIAL，无需求、无 DRAW、不得 WAITING。空需求集合本身不能推断零物料。V423（ADR-057）后零料原因收敛为 `DIRECT_MAKE`（无 BOM，证据=物料分析事实）与 `NO_PRODUCTION_HARD_GATE`（BOM 存在但无生产硬门槛）两类；`PLAN_BOM_OVERRIDE` 随计划级 BOM 例外机制退役，仅历史段保留。
- 采购/委外每次部分 IQC PASS 只在品质事务内追加合格基本量与仓库待入库任务；仅有申请、订单、预计到货、待检、PASS 未确认或拒收只能影响预计量/品质进度。仓库按实际量和实际库位确认后，才按稳定锁序写 `DIR_IN`，以累计已入库量减既有有效分配的差额推进正式来源、peg/预约/DRAW/正式执行段齐套；同单其它待检行不阻塞已确认入库切片，品质终态与仓库入库完成分别对账。红冲只反向实际入库库存/供给并取消未入库 PASS 任务，且不发正向增加通知。
- 计划进度分层读取：`Σfqty` 是审核完工申报扣除 FQC 失败 adjustment 后的当前有效贡献；FQC 待检/PASS/FAIL 与 FINISHED_IN 待点收另行投影；`Σiqty` 只累计仓库在 PASS 放行上限内逐行实收并入账的数量；正式物料状态来自 `CONFIRMED` 包和正式执行段。各层不能互相推导，计划结清和完成率只按仓库实收 `iqty`，分析 ready、报工完成、FQC PASS 或待点收草稿都不等于成品已入库。
- 计划详情的现有 `trace` 只是部分投影：覆盖销售订单来源、DRAW/FINISHED_IN、采购申请、委外申请；采购/委外订单、收货、IQC、报工、批次和发运尚未统一到该详情，不得把它称为完整全链路追踪。
...
- 开发原库仍为 V244/225，公司目标库仍为 V238 既有证据；V247–V250 已在一次性隔离克隆 V250/231 迁移并覆盖阶段链/HTTP 证据。目标库迁移、完整收货/IQC/QMS/MAKE/车间报工/分批发货实物链、历史对账、恢复和岗位签字未完成，生产 **NO-GO**。
...
- [ ] 权限：普通销售/采购/委外/生产/库存/财务单据已接对象级策略，但聚合入口尚未全覆盖；当前 `GET /api/production/chain-health` 只校验 `production_material_analysis:view` 且直接执行全局 SQL，未套 owner/委派 scope，修复前为治理候选 **NO-GO**
- [x] 视角：旧全局选择器提案未采用，页面不得依赖不存在的 Provider/组件
- [ ] 数据范围：多数原生 SQL 读模型已走 `nativeReadScope`，但不能由此推导所有聚合查询都合规；须补 chain-health 的生产/销售对象范围、已知 UUID/跨 owner 负向 HTTP 和导出防泄漏后才能勾选完成
...
- [x] 审批：V196/V201 实例使用 `view/approve/reject` 分权 + 实时审核资格、追加式事件和任务 `allowedActions`；业务详情只读，超管不自动代审
- [x] 审批：审核人仅在精确 PENDING case 上临时读取隐藏详情；batch 决定、订单副作用和决定 receipt 同事务，结束后不扩大列表/普通详情范围
...
### 7.1 人员数据交接与离职全局机制（V393–V399）

| 机制 | 权威规则 |
|---|---|
| Preview | `REPEATABLE_READ`；服务端返回 transfer/history/release/blocking 四类项次及 scope 实际接手人 ID/姓名，`total` 为四类和且不计 synthetic target-required |
| Manual handover | scopes 显式 1–8；全局 coordinator + request lock；保留历史 actor，只转当前责任/建立有效历史访问；零影响拒绝 |
| Multi-target offboard | 同任职代际内已有 sales→B、finance→C 等有效链继续沿用，默认 D 仅接剩余/失效范围；按 target 分组使用派生 UUID 子批次，外层 requestId 同时比较 source/default-target 代际后保证整笔幂等 |
| Graph | `requested_scopes` 记录命令；scope 子表只记录真实责任或历史访问的 effective scopes。边保存 source/target 双端任职代际；先选最新唯一边再校验 target，失效不回退；claims/supervisor/权限清理不造边 |
| Client scope | 只把有效接收人展开到本批实际转出的 A 客户；manual current source 保留逐客户 viewer，offboarding source 不保留；每客户一次 access version + event |
| Offboarding cleanup | 经理/生产/资产等 blocker 先回专业页；成功后释放认领、驳回资料申请、停用个人 override/委派、清账号 recipient 范围与 viewer、使 pending/scanning 附件上传会话失效、关闭 remote、强制改密、最后 disabled + revoke refresh |
| Retry | 悲观锁/锁超时/死锁翻译为结构化 409；客户端刷新 preview 后重试，不自动离线重放 |
| Rehire | 开头按 employee→user 锁序复验 resigned/绑定；新任职只恢复员工/账号 active。raw owner scopes/formal edges由代际门禁失效，旧个人权限、委派、viewer、remote 和原责任不自动恢复 |
| Account CAS | V399 `users.version` 保护所有 JPA 整实体写；与数据库维护、用于JWT权限快照的 `auth_version` 分离 |

已签发的附件下载预签名 URL 无法即时撤回，只能依赖短 TTL；不得把上传会话撤销误写成下载 URL 已撤销。该机制当前只有本地源码、空库 V1→V399 和 Testcontainers 候选证据；公司目标库非空升级、真实 HR/经理/接手人权限矩阵、目标规模并发与恢复演练仍为发布 NO-GO。
...
| 采购/委外订货财务审批与到货异常 | 🟡 已迁移/有源码，生产 NO-GO | 生产分析形成申请、采购/委外任务中心分解、财务共享任务（`view/approve/reject` + 实时资格）、预计到货、超量隔离和原下单人退回已形成专用流程；旧单笔入口已 fail-closed，正式决定使用 case-bound batch。目标库、岗位/实物/IQC/QMS/恢复与发布仍未验收 |
...
- [数据迁移 README 模块清单](../数据迁移/README.md) — 当前候选证据和 NO-GO
...
- [新库上线与首装操作指引](../99-项目治理/2026-09-01-新库上线与首装操作指引.md) — 当前证据、剩余工作与 GO 门禁
