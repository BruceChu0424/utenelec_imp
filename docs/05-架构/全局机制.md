...
- 采购/委外每次部分 IQC PASS 都在处置事务内按合格基本量写 `DIR_IN` 并即时刷新命中的活动分析；仅有申请、订单、预计到货、待检或拒收只能影响预计量/缺口。每次 PASS 都按稳定锁序以累计合格量减既有有效分配的差额推进正式来源，把该差额转换为订单累计、peg/预约/DRAW/正式执行段齐套；同单其它待检行不阻塞，全部终态只最终对账。红冲精确反向已 PASS 库存/供给后向下刷新，且不发正向增加通知。
...
| 采购/委外订货财务审批与到货异常 | 🟡 已迁移/有源码，生产 NO-GO | 生产分析形成申请、采购/委外任务中心分解、财务审核组（`finance_order_approval:review`）、预计到货、超量隔离和原下单人退回已形成专用流程；公司目标库保留 V238 既有证据，开发原库 V244/225；共享工作树当前最高 V392/354，V288/V289 借用结构、终态守卫、审计与在线链均为未部署源码候选；V253/234、空库 V252/233 与一次性生产物料克隆 V250/231 均为历史阶段证据，且岗位/实物/IQC/QMS/发布未验收 |
