...
> 也不得用于公司目标库或生产。
>
> **2026-09-01 live V450 本机开发库业务零基线重置（再次执行，无备份）**：执行前 219 张 CLEAR 表中
> 68 张非空、合计 414 行，库存余额/流水/预留为 11/12/12；账户金额、客户/供应商期初、货品期初/
> 安全库存/成本预算和结算类别期初已为 0。执行前停止 Spring Boot/Maven 链，数据库其它连接、业务
> Outbox 阻断和非人事附件 owner 均为 0；目录/FK/Flyway 门禁演练成功并回滚。按用户明确要求本次未创建
> 备份，提交后没有本次恢复点。单事务提交后 219 张 CLEAR 表、库存、账户及全部零基线字段、六张物化
> 视图均为 0；V450/412、禁用非内部触发器和其它连接不变，账户/FQC/收款异常与完整性视图均无异常。
> 主档、人事、权限、审计、附件治理、Flyway、编号高水位和终身占用继续保留；ERP应用保持停止。
>
> **2026-09-01 live V450 本机开发库业务零基线重置（用户明确要求无备份）**：执行库为 V450/412，
> PostGIS 扩展表排除后共 314 张应用父表，`CLEAR 219` / `PRESERVE 95`。V440 六张、V442 八张、
> V443 财审事件一张、V446 IQC 入库两张和 V447 委外权益交接五张均完整存在；V448–V450 不新增父表。
> 脚本及门禁同步到 V450，契约测试 5/5。执行前确认数据库无其它客户端，业务 Outbox 阻断和非人事附件
> owner 均为 0；219 张 CLEAR 表中 37 张非空、合计 217 行，库存余额/流水/预留为 8/8/7；账户金额、
> 客户/供应商期初、货品期初/安全库存/成本预算和结算类别期初均已为 0。按用户明确要求本次未创建备份，
> 提交后没有本次恢复点。单事务提交后219张CLEAR表、库存、账户及全部零基线字段、六张物化视图均
> 为0；V450/412、禁用非内部触发器和其它连接不变，账户/FQC/收款异常与完整性视图均无异常。
> 主档、人事、权限、审计、附件治理、Flyway、编号高水位和终身占用继续保留；ERP应用保持停止。
>
