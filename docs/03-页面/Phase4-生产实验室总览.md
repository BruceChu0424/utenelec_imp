...
> 🟡 **业务模块源码已接入候选**（不在原 10 页 UX 内，独立 hub 页）：生产管理 `/production`、仓库管理 `/warehouse`。MAKE 父需求回供、采购/委外 IQC 和新委外发料审核 409 门禁仍阻断生产发布，详见 [页面总览 §业务单据与报表](页面总览.md#业务单据与报表2026-07-新增全真实后端)。
