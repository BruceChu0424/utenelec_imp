...
│ ▾ 待办事项                                    │  ← 可折叠，位于功能模块区之前
│ ▾ 我的部门（2026-08-02 新增，全员可见）         │  ← 本分支组织树 + 花名册（8 字段）
...
3. **待办任务**：同一聚合接口返回通知待办、生产待排产、仓库/采购/委外履约、财务订货/超量审批、访客审批和资料变更审核等可操作卡片；通知打开明细，其他卡片跳转真实处理页。财务卡只显示服务端精确分配给当前 `assignee_user_id` 的任务，通知和超级管理员标记不能代替对象校验。仓库、采购、委外按分区独立查询，某一履约分区发生非安全异常时以明确的“正在自动恢复”卡替代该分区，不丢弃其他首页内容，也不把加载失败伪装成零待办。
...
7. **「我的部门」卡片**（2026-08-02 新增，位于「今日概览」与「功能模块区」之间，全员可见）：
   - 由 `MyDepartmentService.myBranchRoot` 计算员工所在大部门（管理中心/决策层分支）根节点，展示该分支架构树（`UtenDepartmentTreeView`）+ 花名册。大屏左右分屏（树 + 花名册），小屏弹窗选部门。
   - 花名册只暴露安全 8 字段：工号 / 姓名 / 岗位 / 办公电话 / 邮箱 / 是否负责人 / 是否本人；不含证件、手机、薪酬等敏感字段。
   - 任意登录员工可读本分支（`GET /api/my-department/tree`、`GET /api/my-department/roster?departmentId=`，服务端按本人分支鉴权）。
   - 部门负责人额外见「本部门员工权限」面板：调 `GET /api/department-staff-permissions/managed`（非 403 时渲染）展示成员 × 权限点 Switch 矩阵，可对成员开/关覆盖；详见 [部门管理页.md](部门管理页.md)「部门负责人权限转授面板」一节。
