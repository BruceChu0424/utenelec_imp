...
- 已按页接入：审计中心与 销售/采购/仓库出入库/钱流/委外 5 个单据列表页（返回时**静默**重拉当前页——`_load(page, silent: true)`：已有数据则不翻 `_loading`、不重建、不闪 loading，数据到达后静默换，失败保留旧数据；保留筛选与页码）；采购/委外/仓库/钱流 4 个 hub 的任务数卡片（按 hub 粒度 `invalidate` 对应 `autoDispose` 计数 provider——不放进全局刷新，避免把没打开的 hub 的 provider 激活、白白启动轮询）。
