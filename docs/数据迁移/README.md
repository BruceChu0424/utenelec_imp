...
| 说明 | 迁移 | 本次变化 |
| --- | --- | --- |
| [235](235-V692仓库负责人与仓库通知按仓分发.md) | V692 | 新表 `warehouse_keepers`(仓库 × 员工, 主仓负责人管全部子仓)与 `fn_warehouse_scope_ids` / `fn_warehouse_keeper_user_ids` / `fn_user_warehouse_scope_ids`；仓库类通知按仓分发、任务中心「我的仓库」(ADR-115)；未登记负责人时行为不变 |
