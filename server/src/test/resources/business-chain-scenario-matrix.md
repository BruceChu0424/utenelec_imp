...
| SC-32 | 历史迁移、对账、死锁恢复 | 公司目标库既有 V238 只读证据向当前源码 V289 升级，构造中断和反锁序 | 历史先快照/隔离再补链；逐版本 checksum 不变；升级前后数量、金额、权限、编号冲突、客户结算问题和审计证据对账；死锁整事务回滚并幂等重试 | `V238ToCurrentSyntheticMigrationPostgresTest` 自动覆盖非空结构升级；`CurrentHeadNonEmptyCloneRehearsalTest` 仅在可恢复克隆、集群身份、备份摘要、批准引用和两类证据摘要全部显式绑定时运行。公司数据克隆、恢复、压力和回滚演练仍未执行 | `[PARTIAL][RED][MANUAL]` |
...
5. V150–V289 源码存在、空库/合成非空迁移成功或定向测试通过，均不能替代公司数据克隆升级、历史数据快照/对账、真实岗位 UAT、实物演练和负责人签字；ADR-033/V288/V289 还须取得目标库与验收证据，当前仍是生产 `NO-GO`。
