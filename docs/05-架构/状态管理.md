...
- 财务计数按当前账号精确负责人过滤；原下单人退回计数按 owner 且按采购/委外 `orderType` 过滤；无权限或不是实例负责人必须看不到对象。
...
**最后更新**：2026-08-02（补齐原子 token record、CAS/latest-intent、logout fence、加密 refresh 撤销队列与恢复竞态；保留 §4.6 列表页刷新规范）
