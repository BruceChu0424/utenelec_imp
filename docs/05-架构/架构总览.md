...
谱系。普通 generate 零预留，只有批准事务写入执行和库存事实。

计划进度必须同时保留三条互不替代的读模型：`reportedQty=Σfqty` 是已审核报工量，`inboundQty=Σiqty` 是已审核 `FINISHED_IN` 入库量，正式物料状态只从 `CONFIRMED` 包及正式执行段/需求投影；分析 ready、未来供给或报工完成都不能伪装为正式入库/齐套。计划详情当前 `trace` 只投影销售订单来源、DRAW/FINISHED_IN、采购申请和委外申请，尚未把采购/委外订单、收货、IQC、报工、批次和发运统一为同一条时间线，必须标为“部分溯源”。
