...
> **当前正式目录：V687/614 (2026-09-23)**。V680–V682 服务端会话、敏感操作再认证与系统设置单一登记(ADR-110)：新增 auth_sessions / auth_step_up_states 两张 CLEAR 表，system_settings 删元数据列，account:support 授权策略改 INDIVIDUAL_ONLY、权限目录加 high_risk 列；V670–V673、V677–V679、V683–V687 分属审计三清单(ADR-105)、权限目录单一事实源(ADR-109)、主档删除引用保护(ADR-111)与金额精确派生(ADR-112)；V653–V654、V663 跳号。V645 让追加自制并进还没开工的生产计划(ADR-104): 新增 fn_material_analysis_plan_growable 等四个判定函数与一条「计划明细 qty(+封顶) = 关联行归需求 + 公共备货」的提交时对账触发器, 锚点补丁计划明细身份守卫 / 计划关联行数量同步触发器 / 生产供给来源守卫, 只在计划仍未开工时放开只增不减的改量; 不加表不加列。V644 把 V409 的「创建命令账本」升级为「日报命令账本」(审核接口要求幂等键)。
>
...
| [233](233-V676索引卫生冗余删除与热点领头索引.md) | V676 | 索引卫生(ADR-106)：删 96 个被前缀覆盖或与同列唯一约束重复的冗余索引(白名单保留两条并写明原因)，补 43 个估值图/需求/预留/计划明细/总账凭证引用方领头索引与 31 个货品数量来源列完整领头索引；`SchemaIndexHygieneContractTest` 守住三条规则 |
