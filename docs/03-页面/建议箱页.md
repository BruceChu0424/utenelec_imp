...
- `Suggestion`：列表项主实体（category / title / content / status / submitterName / isAnonymous / displayName / likes / likedByMe / replies）
...
- `SuggestionReply`：回复数与详情页回复展示

## 八、权限要求

- **全员可见**（所有角色都可提建议、看广场、点赞）。
- 数据范围：
  - 广场：所有非草稿状态建议（匿名建议的真名仅 submitter 自己和 hr 可见）。
  - 我的建议：仅当前用户提交的（无论匿名与否，自己看到真名）。
- 关键操作权限点：
  - 提交建议（self）
  - 点赞/取消点赞（self，每人每建议最多 1 个有效赞）
  - 回复/状态变更：仅 hr / manager（Phase 2+ 在详情页或后台操作）

## 九、边界情况

- 无数据时：`UtenEmpty` + 居中"提交建议"按钮，区分广场和我的两种文案。
- 加载失败时：`UtenEmpty.error` + 重试（`ref.invalidate(suggestionListProvider)`）。
- 性能档 = lite 下：禁用卡片阴影与进场动画，`UtenResponsiveGrid` 退化为固定单列、卡片间距压缩，类别 chip 用纯色边框替代。
- 网络断开时：Phase 1 Mock 无影响；后端接入后：
  - 点赞需支持"乐观更新"（先本地 +1，后台异步同步，失败回滚）。
  - 提交建议支持离线草稿，恢复网络后自动重试。
  - 匿名建议的真名脱敏必须由后端统一处理（前端拿到时已脱敏），避免泄露。
