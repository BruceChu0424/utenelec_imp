-- V225: refresh and validate public-business-table audit trigger coverage.
--
-- Repeats the full fail-closed coverage sweep after V224 added celebration-interaction
-- business tables (notice_acknowledgments@V224, notice_blessings@V224).
-- V169/V184/V185/V190/V193/V195/V197/V202/V218/V223 remain immutable once applied.
-- This migration attaches fn_audit() to any eligible public business table still lacking
-- exactly one valid trg_audit* trigger, and fails closed if any table has a malformed/extra
-- trigger. Exactly one prefixed trigger is accepted, only when enabled, AFTER ROW, covers
-- INSERT/UPDATE/DELETE, and calls an approved redacting audit function (fn_audit /
-- fn_audit_redacted). Triggers created here record only future operations from V225
-- application onward; this migration never invents or backfills historical audit rows.
