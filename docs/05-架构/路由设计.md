...
- V196–V201 引入的路由门槛：订货审批/超量审批列表用 `finance_order_approval:view`，仓库预计到货/异常用 `warehouse_inbound:view`，原下单人退回用 `supplier_return_task:handle`。审批权由 `finance_order_approval:review` + 财务部门资格决定（详见 ADR-027），不再单点配置负责人。
