...
**最后更新**：2026-07-25 · **Phase 0 组件进度**：0 / 12 · 新增 §2.8 Brand 分类（UtenWordmarkLogo / UtenBrandMascot）；§2.3 加 `UtenLocationField`（添加位置选择，分类/部门共用，[文档](UtenLocationField.md)）；§2.1 `UtenButton` 加 `tonal` 类型（浅青绿底+深绿字，品牌色调次要按钮）。基础资料四页共用的模块组件（`CategoryDetailCard` / `showMasterEditDialog` / `showMasterDetailSheet` / `CategoryEditDialog`）见 [基础资料页.md](../03-页面/基础资料页.md)
