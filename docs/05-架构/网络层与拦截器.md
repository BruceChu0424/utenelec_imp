...
flowchart TD
    ...
    Auth --> StepUp[StepUpInterceptor\n403 REAUTH_REQUIRED → 统一密码框\n带一次性凭证重发一次(ADR-110)]
    StepUp --> Retry[SafeRequestRetryInterceptor\n仅安全读请求]
    Retry --> Gate[服务端站点门禁\nlocal: 来源 CIDR\ncloud: remote_access]
    Gate --> API[后端 API\nSpring Boot 3]
...
final apiClientProvider = Provider<ApiClient>((ref) {
  // ADR-108: 不再 watch recoveryEpoch——Dio 与名称服务在会话内稳定, 断网恢复后由
  // 「返回即刷新」(当前可见页) 与徽章汇总各自补拉, 不整体重建约 90 个依赖它的 provider。
  final recovery = ref.read(connectionRecoveryProvider.notifier);
  final baseUrl = ref.watch(apiBaseUrlProvider);
  final dio = Dio(buildApiBaseOptions(baseUrl)); // 15s / 30s / 45s
  dio.interceptors.add(const WriteDeadlineInterceptor()); // 见 5.2，必须最先挂
  dio.interceptors.add(DeviceAuditInterceptor(...));
  dio.interceptors.add(AuthInterceptor(storage: ..., baseUrl: baseUrl));
  dio.interceptors.add(StepUpInterceptor(dio)); // 见 5.3
  dio.interceptors.add(SafeRequestRetryInterceptor(dio, recovery: recovery));
  // 成功的业务写推进本端写修订号并记下路径(返回即刷新 / 主档字典作废, ADR-108)。
  dio.interceptors.add(DataWriteRevisionInterceptor((o) => recordDataWrite(ref, o)));
...
### 5.2 写请求截止时间：Web 上 connectTimeout 不是「建连超时」

`WriteDeadlineInterceptor`(`lib/core/network/interceptors/write_deadline_interceptor.dart`)把
请求的 `connectTimeout` 抬到与该请求自己的 `receiveTimeout` 同级，默认仅在 Web 生效。
**2026-09-23 起(ADR-108)读请求同样放宽**：无 body 的 GET 同样没有上传进度, 慢查询(如执行工作台
历史上 85 次 >10s)会在服务端还在算时被 15 秒计时器 abort, 旧设计靠重试「自愈」, 实际是把慢查询再跑两遍、
最后误判断网。

为什么必须这么做：

- 浏览器的 `XMLHttpRequest` 没有独立的建连超时。`dio_web_adapter` 在 `xhr.send()` 之前就起一个
  `Timer(connectTimeout)`，到点若 `readyState` 还没到 `HEADERS_RECEIVED` 就 `xhr.abort()` 并抛
  `DioExceptionType.connectionTimeout`。
- 这个 Timer 只被「上传进度」或「下载进度」事件取消，而上传进度监听**只在请求带 body 时才注册**
  (dio 仅在 `data != null` 时建上传流)。因此 `api.post('/xxx/approve')` 这类**不带 body 的写请求**
  把 `connectTimeout` 变成了「服务端必须在这么久内开口」的硬上限，`receiveTimeout` 根本轮不到生效。
- 后果是最坏的一种：服务端事务已提交，浏览器却把连接掐了，用户看到「网络连接超时」，页面停在旧状态，
  再点一次撞上「仅草稿单据可审核」。2026-09-21 生产日报审核实测复现：服务端 15.094 秒返回 200 并落库，
  客户端 15.000 秒 abort。同期带 body 的 `decide-batch`(30.9 秒)和 `issue-plans`(20.0 秒)都正常返回。
- 它同时修好了 `postLongRunning` 的老问题：那个方法只覆写 `receiveTimeout`，在 Web 上仍会被 15 秒掐断。

非 Web 平台上 `connectTimeout` 是真正的 socket 建连超时，保持 15 秒才能对不可达服务端快速失败，
所以默认按 `kIsWeb` 生效。(旧口径「读请求不放宽、由安全重试自愈」已于 2026-09-23 作废, 见上。)

**配套的服务端责任**：写端点自己要能安全重放。生产日报审核已按 V644 加了幂等键，
同一操作者同一把键重发直接返回原结果(见 [生产执行分段与报工页](../03-页面/生产执行分段与报工页.md))。
其它尚未加幂等键的写端点，客户端只能靠下面这条页面责任兜底。

**配套的页面责任**：POST/PUT/PATCH/DELETE 永不自动重放，所以「超时」对写请求等于「结果未知」。
页面在写请求失败后**必须重读一次单据权威状态**再决定提示什么，不得继续拿旧对象画动作按钮——
那正是诱导用户重复提交的来源。参考实现见
`lib/features/production/pages/production_daily_report_detail_page.dart` 的 `_settleFailedAction`。

回归用例：`test/core/network/write_deadline_interceptor_test.dart`、
`test/features/production/pages/production_daily_report_detail_approve_test.dart`。

### 5.3 敏感操作再认证：StepUpInterceptor(ADR-110)

服务端对设/撤超管、授权与数据范围、重置他人密码、给存量员工补开账号、进入切换人、保存系统设置、
清理测试附件/清空业务数据、提交需审核的个人资料要求「5 分钟内、本人本会话、一次性」的再认证凭证，
缺了回 `403 REAUTH_REQUIRED`(服务端在参数校验与权限判定都通过后才核销，无权或请求体不合法不会先弹框)。

- `StepUpInterceptor`(`lib/core/network/interceptors/step_up_interceptor.dart`) 只认这一个错误码：
  经 `StepUpCoordinator` 向应用根部登记的 `StepUpPromptHost` 要一张凭证(弹 `ReauthDialog`，
  `POST /auth/step-up` 换取)，拿到后给原请求加 `X-Uten-Step-Up` 头原样重发**一次**
  (`extra` 标记防止死循环)。服务端在进入业务前核销/拒绝凭证，被拒的请求什么都没做，所以重发安全。
- 用户取消或没有登记弹窗时原 403 照常抛给页面；别的 403(没有权限、只读模拟)一律不碰。
- 并发要凭证时 `StepUpCoordinator` 排队，一次只弹一个框，各拿各的凭证。
- 输错(`422 REAUTH_FAILED`)留在框里显示服务端原因可重试；连续输错(`429 REAUTH_LOCKED`)框里提示暂停时长、
  输入框与确认按钮不可用(服务端同时让当前登录失效)。
- 页面不得再自己弹密码框，也不要在调用处特判 `REAUTH_REQUIRED`。
- 敏感写请求期间可以照常挂整页忙碌遮罩：`showReauthDialog` 弹框期间用 `UtenBusyOverlay.yieldWhile`
  让所有忙碌遮罩让位(遮罩是 root Overlay 裸 entry，导航器推路由会把它抬到最顶层盖住密码框，
  请求等密码、遮罩等请求会整页卡死)，关框后遮罩恢复到重发请求结束。

回归用例：`test/core/network/step_up_interceptor_test.dart`、`test/shared/widgets/reauth_dialog_test.dart`、
`test/shared/widgets/reauth_over_busy_overlay_test.dart`(宿主照搬 AbsorbPointer + 忙碌遮罩，按坐标点密码框；
另有「不让位时点不到」的对照用例复现原卡死)。

### 5.4 自动请求声明：AutomaticRequestInterceptor(ADR-110)

服务端会话按「最后一次人为请求」计空闲。哪些请求是人为的只有客户端知道：

- `UserActivityTracker`(应用根部，不渲染)挂在指针路由与硬件键盘的全局入口上，把每次点击/移动/悬停/
  滚轮/按键的时刻记进 `UserActivity`(含根导航器上的弹窗里的操作)。
- `AutomaticRequestInterceptor`(员工与访客两个网络客户端都装)在每次发请求时判定：距上次输入超过
  30 秒(`UserActivity.idleAfter`)就加 `X-Uten-Automatic: 1`，否则去掉。重试、刷新令牌后的重放、
  再认证后的重发都按当时重新判定。
- 不按端点名单判定：角标轮询、服务器状态、审核心跳、页面定时刷新以及以后新增的任何轮询都不需要登记。
- `IdleTimeoutController` 的本机计时同样取 `UserActivity` 的最近输入时刻，弹窗里一直在输入不会被误判空闲。

回归用例：`test/core/network/automatic_request_interceptor_test.dart`。
...
- `SafeRequestRetryInterceptor` 仅对 GET/HEAD/OPTIONS 做至多两次自动重试，延迟固定为 400ms、1200ms。
  触发范围是**连接错误、502/504、非结构化 503**，以及后端结构化 `503 + SERVICE_UNAVAILABLE`。
  **发送/接收超时与 408 不重试、也不判定断网**(ADR-108, 慢 ≠ 断: 请求已发出、服务端可能正在处理,
  重试只会放大负载; 超时只让这一次请求失败, 页面提示重试)。**建连超时分平台**: Web 上它只是浏览器请求
  计时器(见 5.2), 同样不重试、不判断网; 原生端(Windows 桌面包等)它是真实的 TCP 建连超时——局域网服务器
  宕机、网线断开时就是它, 请求根本没发出去, 按「连不上」处理(读请求重试, 耗尽后进入断网恢复)。
  POST/PUT/PATCH/DELETE 不走该重试器。
...
- 恢复只递增一次 `recoveryEpoch`。**2026-09-23 起(ADR-108) `apiClientProvider` 不再 watch 它**,
  Dio 与各名称服务/字典缓存在会话内保持不变; 恢复后由 `SessionRehydrateGate` 推进
  `pageRefreshRequestProvider`, 只让**当前可见页**重拉一次, 徽章汇总也补拉一次; 表单与导航状态不受影响。
...
### 6.1.1 令牌临期提前刷新(2026-09-23, ADR-108)

- 请求发出前若访问令牌剩余寿命 ≤60 秒(`AuthInterceptor.isNearExpiry`: 按 JWT `exp - iat` 得出寿命,
  减去本机首次见到该令牌后流逝的时间, 不受客户端时钟偏差影响), 先走与 401 同一把刷新锁(跨标签页单飞)
  刷新, 已被兄弟请求刷新过则直接取最新令牌。解析不了的令牌按不临近处理, 交给 401 兜底。
- 此前只在收到 401 后刷新, 每 15 分钟一轮, 该窗口内发出的请求全部先吃一次 401 再重放。

### 6.1.2 本端写修订号(2026-09-23, ADR-108)

- `DataWriteRevisionInterceptor`(`lib/core/network/data_write_revision.dart`): 每个成功的业务写
  (非 GET/HEAD/OPTIONS; 排除心跳、自动已读、偏好保存、`/auth/*`、只算不存的 `/preview`、`/resolve-batch`)
  推进 `dataWriteRevisionProvider` 并把路径写入 `lastDataWriteProvider`。
- 用途: 「返回即刷新」据此判断期间数据变没变(调用方漏了 `bumpListRefresh` 也不会显示旧数据);
  主档字典仓库据写路径作废对应字典(`masterDictKeysForWrite`)。网络层不认识任何业务模块。
...
## 2026-09-07 主档名称字典失败恢复

`MasterDictionaryService.ensureDictionaryLoaded` 按字典独立合并并发GET并只缓存成功结果。
仓库、币种、颜色、单位、供应商、部门和销售客户中任意一项503，不丢弃其它已成功数据；失败项
在下一次ensureLoaded时重新请求，成功项继续使用当前实例缓存。辅助名称读取仍降级为占位，
不向原调用者抛新异常，也不永久缓存失败完成的Future。显式reload通过请求身份检查防止更早
响应覆盖新缓存。账号/权限快照变化创建新的服务实例，旧实例的迟到结果不能写入新实例。

验证包括真实ApiClient/Dio的503后恢复、并行ensure去重、成功字典保留、销售客户恢复和跨账号
实例隔离；不改变任何业务写请求或网络重放策略。
