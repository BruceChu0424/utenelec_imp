package com.uten.imp.features.master.referencemethod;

import com.uten.imp.common.domain.SoftDeletableEntity;
import jakarta.persistence.Column;
import jakarta.persistence.Entity;
import jakarta.persistence.Table;
import lombok.Getter;
import lombok.NoArgsConstructor;
import lombok.Setter;

@Getter
@Setter
@NoArgsConstructor
@Entity
@Table(name = "finance_payment_methods")
public class FinancePaymentMethod extends SoftDeletableEntity {
    @Column(name = "legacy_id", unique = true)
    private Integer legacyId;
    @Column(nullable = false, unique = true)
    private String code;
    @Column(nullable = false)
    private String name;
    @Column(name = "is_receipt", nullable = false)
    private boolean receipt = true;
    @Column(name = "is_payment", nullable = false)
    private boolean payment = true;
    @Column(name = "legacy_name_confirmed", nullable = false)
    private boolean legacyNameConfirmed;
    @Column(nullable = false)
    private String status = "使用";
    @Column(name = "sort_order", nullable = false)
    private Integer sortOrder = 0;
    private String remark;
}
