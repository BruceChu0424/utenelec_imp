...
- **范围 B（待办）**：采购/生产/仓库改为按负责人隔离（业务已确认方向），抽平台级 `DocumentAccessPolicy` 复用 `OwnerVisibility`。
...
**最后更新**：2026-08-07（§4.5 对齐 V229/ADR-027：采购/委外财务审批改为财务部门持 `finance_order_approval:review` 的审核组授权，删除 `workflow_assignment:manage` 与单点负责人配置；高危个人-only 清单同步移除该码）。原 2026-08-02 版补齐会话/连接安全与 V196/V201 超量到货 fail-closed 边界、新增 §4.6/§4.7；公司目标库仍只确认到 V190，真实部署、岗位/实物 UAT 和生产验收未完成。
