...
class _PurchaseDocDetailPageState extends ConsumerState<PurchaseDocDetailPage> {
  ...
  Future<void> _approveFinanceApproval() async {
    ...
    final confirmed = await showUtenReviewerConfirmDialog(
      ...
      message: '通过后订货单立即生效，并为仓库生成预计到货任务。',
    );
            ...
            content: SizedBox(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text('${_detail?.billNo ?? '该订货单'}将退回给制单人，修改后可重新提交财务审核。'),
                  const SizedBox(height: UtenSpacing.s12),
                  TextField(
                    autofocus: true,
                    minLines: 3,
  ...
  Widget _actions(ThemeData theme) {
    ...
    if (widget.docType == PurchaseDocType.request) {
      ...
      if (pendingApproval?.canReject == true) {
        addAction(
          UtenButton(
            key: const Key('purchase-order-finance-reject'),
            type: UtenButtonType.danger,
      ...
      if (pendingApproval?.canApprove == true) {
        addAction(
          UtenButton(
            key: const Key('purchase-order-finance-approve'),
