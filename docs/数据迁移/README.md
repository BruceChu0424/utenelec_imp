...
> **本机开发库零基线重置（现行口径）**：`server/ops/reset_business_data.sql` 按 CLEAR/PRESERVE 双目录在单事务内清零业务数据并刷新六张物化视图（仅限可丢弃本地/测试库；范围、三闸确认与门禁见脚本头部，禁止用于公司目标库或生产）。最近一次：2026-09-02 17:23（UTC-06:00）于 V459/421 按明确指示**不创建备份**执行；清前 14 张业务表共 85 行，清后 CLEAR 222 张全部 0 行、PRESERVE 96 张逐表行数与清前完全一致，五张库存事实表与六张月度物化视图均 0 行，27 个账户金额/完整性异常均为 0，客户/供应商/货品/结算类别期初及货品安全库存/成本预算异常均为 0，1 个归属业务表的 owned/identity 序列回到起点且 `is_called=false`，Flyway 仍为 V459/421、失败迁移 0、禁用用户触发器 0、其它客户端连接 0；固定提交 `53c3dfd3` 的 V459 脚本隔离 JUnit 契约测试 5/5（仅用缓存依赖执行 javac/java，未运行 Maven；未修改并行源码，未应用 V460）。主档、人事、权限、审计、Flyway 与治理/编号高水位保留，ERP 应用保持停止；因本轮未建备份，不存在本轮恢复点；按要求未上传、未提交、未推送。2026-08-17 至 2026-09-01 间的历次清库流水（V399→V450 共 20 余次）已随 2026-09-01 文档清理移除，需要证据时查 git 历史。
