...
> 本档是 **人事功能域（Phase 2）** 的总纲，串联 8 个管理页面，并登记全员“我的”资料自助链。
> 员工 / 部门是全平台的数据依赖根，本组页面也作为后续业务功能域的组件与权限样板。
>
> 全局机制（权限 / 视角选择器 / 审批流）见 [全局机制](../05-架构/全局机制.md)。
> 页面模板见 [页面总览 §六](页面总览.md)。
>
> 📍 **现状（2026-08-27）**：全员“我的”资料改为 `GET /api/profile/me` 对象级读取，服务端只从
> 当前 `AuthUser.employeeId` 解析本人，不接受目标员工 ID；本人个人 PII 可见，但银行/薪酬仍只按
> 显式权限返回且“我的”页有意不展示。页面补齐身份、组织、联系住址、紧急联系人、合同和任职记录，
> 并标注直接生效、需 HR 审核和人事维护。业务页权限工作台与员工详情遇到未开户员工时显示明确空态；
> 只有操作者实际持 `account:support` 才询问并执行开户，组织负责人身份本身不授账号支持。
>
> 📍 **现状（2026-08-05）**：人事域完善落地（[ADR-021](../99-决策记录-ADR/ADR-021-人事域完善-转正车辆多联系方式与任务软认领.md)）：
> 转正日期老数据按入职日期回填（V210）、新员工正式入职必填转正日期、HR 任务中心重构为
> **HR 工作台**（子页面 + 快捷转正 + 任务软认领「XXX 处理中」）、员工详情页 v2（操作外显 + Tab 分组）、
> 车辆/备用手机号（V211，车牌可搜人）、更换手机号同步登录账号、花名册/架构图打印按新权限点
> `employee:export` 门控、部门架构图 v2（负责人归入班组）。以上均已接入源码并通过后端编译；
> 目标库 V210/V211 与运行实例重启、E2E 待验收。
>
...
| 9 | 我的 / 个人资料 | `/profile` | [我的页.md](我的页.md) | 本人对象级档案 + 修改入口；不使用全员员工详情 API |
| 9a | 我的部门 | `/profile/me/department` | [我的页.md §四·甲](我的页.md) | 本人分支树 + 安全花名册；不授部门/员工管理权限 |
| 9b | 我的车辆与号码 | （v7 吸收为 `/profile` 联系与车辆 Tab，路由下线） | [我的页.md §三·甲](我的页.md) | 本人车辆与备用手机号直改 |
| 9c | 我的文件 | （v7 吸收为 `/profile` 我的文件 Tab，路由下线） | [我的页.md §十](我的页.md) | 本人附件只读；上传/删除由 HR 维护 |

2026-08-14 搜索口径：`/department` 左框在具备 `employee:view` 时统一搜索部门名称/编号与在册员工姓名/工号；无该权限只搜索部门。`/profile/me/department` 只在本人授权分支内搜索部门与安全花名册，不调用全员员工接口。两者均展开完整祖先、保护快速输入与手动点树竞态，详见 [UtenHierarchySearch](../02-组件库/UtenHierarchySearch.md)。
...
| 能力 | 权限点 |
|---|---|
| 员工档案 | `employee:view/create/edit`（当前无员工删除入口） |
| 员工生命周期 | `employee:transfer`、`employee:confirm`、`employee:offboard`、`employee:rehire` |
| 合同续签/补录 | `employee:contract_renew` |
...
| 本人资料读取/修改 | `profile:edit:self` + 服务端固定当前员工对象 |
| 日常账号开通/锁定/启停 | `account:support`；负责人身份不自动获得 |
...
- **我的资料**：只取当前认证账号绑定的员工 UUID；不接受前端传入目标 ID，不因本人范围扩大到同事、
  银行或薪酬数据。
...
- [x] 部门负责人合法性、清空与调岗/离职联动已闭环；普通编辑不能绕过调岗改部门；当前无员工归档入口
- [x] 管理中心可承载直属人员/岗位/负责人；所有普通负责人根覆盖未删除子树，总经办 `GM` 直属
  在册负责人覆盖公司。管理中心结构与总经办移动/删除均锁定
...
- 负责人必须是本组织直属在册员工并可显式清空；管理中心也遵守该规则。负责人调岗或离职时自动解除。
- 全局 `/admin/permissions` 是共享壳：持 `account:support` 的人事可进入账号支持区；只有
  超级管理员且具备 `authorization:manage` 才能进入中央部门配置、个人 grant/revoke 和数据范围区。
- 已登记业务页右上角另有“权限设置”：超级管理员可维护本页完整目录；有非委派来源 ceiling 的
  `departments.manager_id` 负责人可为空部门筛选搜索全部授权子树，或选择节点后按子树交集筛选，
  再为人员转授本页可下放权限，不要求进入全局权限页。
- 业务页人员列表包含未开户员工并显示明确空态；负责人若**另外**持有全局 `account:support`，可对
  当前选中人发起开户。负责人关系或 `authorization:manage` 本身不产生开户权；凭据确认保存并关闭后
  才刷新账号状态和加载权限，取消/失败/并发开户不发送权限写请求。
- V328 与全量本地验收通过后，通用/生产源码默认开启业务页入口；紧急停用可设置 `UTEN_MANAGER_PERMISSION_DELEGATION_ENABLED=false` 并重启。该默认值不表示公司目标库已迁移或已部署。

---

**最后更新**：2026-08-22（全局超管入口与业务页负责人入口分离；部门默认空、普通负责人根覆盖子树、总经办负责人覆盖公司；页面委派默认开启、可紧急回退，尚未部署）/ 2026-08-03（入职工号服务端分配、岗位确认选择/自定义、管理中心用人与中心负责人子树范围接入源码；目标库迁移、实例重启和生产 E2E 未完成）。
