...
[Phase D] 真实入库 `repo.publish(...)`（mock 仓储插到列表最前）
...
[Phase D3] 顶部绿色 banner「已发布」
       ↓
[Phase D4] `dispatchNoticeArrival(context, notice)` 模拟接收端到达提醒：
       ↓   urgent → 屏幕正中红色弹窗（禁止遮罩关闭）
       ↓   important → 屏幕正中橙色弹窗
       ↓   normal → 顶部消息弹条（微信式）
       ↓
[Phase D5] `context.go('/notice')` 跳列表页（弹窗/弹条挂在全局层，跨路由不丢）
       ↓
[Phase E] 按钮解锁
```

> 接收端逻辑收敛在 `lib/features/notice/providers/notice_arrival.dart`。
> 接真后端后，Phase D4 由推送/WebSocket 在**每个接收端**触发，发布端不再演示。
...
flowchart TD
    ...
    B --> CF[确认对话框]
    CF -->|取消| E2[按钮解锁]
    CF -->|确认| API[repo.publish 真实入库]
    API --> E3[顶部绿色 banner + 到达提醒演示<br/>dispatchNoticeArrival + 跳列表页]
```
...
**最后更新**：2026-07-28 · **状态**：Phase 2 落地（按钮已迁到 UtenActionButton；发布真实入库 mock 仓储 + 类型→重要度映射 + `dispatchNoticeArrival` 接收端到达提醒演示）
