-- V184: refresh and validate public-business-table audit trigger coverage.
--
-- V169 is already applied and remains immutable. This migration only attaches
-- fn_audit() to eligible tables that do not already have a trg_audit* trigger.
-- Exactly one prefixed trigger is accepted, and only when it is enabled, AFTER ROW, covers
-- INSERT/UPDATE/DELETE, and calls the approved redacting audit function.
-- Any trigger created here records only future INSERT/UPDATE/DELETE operations
-- from V184 application onward; this migration never invents/backfills history.
