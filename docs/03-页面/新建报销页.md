...
- compact（手机，< 600dp）：
  ...
  └────────────────────────┘
  ```
  说明：AppBar + 单列滚动表单；合计挂在明细内容下方，存草稿/提交为右下角悬浮组（2026-09-14 起无明细置灰并提示）。
...
页面先完成标题与费用明细，可选补充说明默认折叠。明细金额框通过图标说明按实际票据人民币金额填写、同笔费用不得重复；用途说明提示审批核对用途，不承担计算规则。原有必填、金额校验、保存草稿与提交路径保持。
