 ...
 *
 * <p>历史审核（status 0→1）曾直接库存入库；之后的新退货审核改为质量冻结，
 ...
 * ar_posted 为立帐标志。未处置的冻结可随原单受控反向；已发生处置时禁止整单普通红冲。
 *
...
public class SalesReturn extends SoftDeletableEntity {
    ...
    /** Optional source shipment UUID truth; sourceDocNo is only a snapshot. */
