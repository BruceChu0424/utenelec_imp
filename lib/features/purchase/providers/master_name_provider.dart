...
class MasterNameService {
  ...
  Future<void> loadGoodsNames(Iterable<String> ids) async {
    final need = ids.where((id) => id.isNotEmpty && !_goods.containsKey(id)).toSet();
    if (need.isEmpty) return;
    try {
      final list = await api.getList(ApiEndpoints.goodsLookup, query: {'ids': need.join(',')});
      for (final e in list) {
  ...
  Future<List<GoodsOption>> searchGoods(String keyword, {int size = 20}) async {
    ...
    final json = await api.get(ApiEndpoints.goods,
        query: {'keyword': keyword.trim(), 'page': 1, 'size': size});
    final items = json['items'];
    if (items is! List) return const [];
    final out = items.map((e) => GoodsOption.fromJson(e as Map<String, dynamic>)).toList();
    // 顺手缓存命中的货品名
    for (final g in out) {
