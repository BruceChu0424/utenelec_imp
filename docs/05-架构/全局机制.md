...
- V196–V202 已包含在开发原库 `uten_imp` V244 和公司目标库 V238；截至 2026-08-14，共享工作树源码目录最高 V289/270，270 个版本号无重复。V288/V289 已提供现货借用结构、终态守卫与全 `public` 审计覆盖，在线 create/revoke、持久化、双趟生效计算和 Flutter UI 已形成源码候选。V253/234 与空库 V252/233 是 2026-08-11 阶段快照，一次性生产物料克隆仍是 V250/231 历史证据。V257–V289 不改变生产默认车间唯一真源 V192。目标库迁移、真实岗位权限、仓库/退回实物、完整 HTTP IQC/QMS 和发布签字仍未完成，生产 **NO-GO**。
...
| 采购/委外订货财务审批与到货异常 | 🟡 已迁移/有源码，生产 NO-GO | 生产分析形成申请、采购/委外任务中心分解、财务审核组（`finance_order_approval:review`）、预计到货、超量隔离和原下单人退回已形成专用流程；公司目标库保留 V238 既有证据，开发原库 V244/225；共享工作树当前最高 V289/270，V288/V289 借用结构、终态守卫、审计与在线链均为未部署源码候选；V253/234、空库 V252/233 与一次性生产物料克隆 V250/231 均为历史阶段证据，且岗位/实物/IQC/QMS/发布未验收 |
