...
统计 KPI 当前由页面业务卡或 [`DocKpiBar`](../../lib/shared/widgets/doc_kpi_bar.dart) 组合实现。项目中没有 `UtenStatCard`、`UtenAnimatedNumber`、`UtenDataTable` 或 `UtenChartPlaceholder`；时间线场景使用已实现的 `UtenProgressTimeline`（2026-08-19 新增，见 2.4），不得再假设存在 `UtenTimeline`。
...
| [`uten_field_message.dart`](../../lib/components/inputs/uten_field_message.dart) | `UtenFieldMessage`、`UtenOverflowMessage`、nullable 适配器与统一 `TextFormField.errorBuilder` | 字段辅助说明/错误及可点击通知长文案；仅真实溢出显示问号，hover/tap/focus 查看全文，错误 live-region；见 [UtenFieldMessage](UtenFieldMessage.md) |
...
不存在独立的 `UtenSelect`、`UtenDropdown`、`UtenDatePicker` 或 `UtenSwitch`。对应场景分别使用 `UtenDropdownField`、`UtenDateField`；开关使用 Material `Switch` 并遵循主题。表单提示统一使用 `UtenFieldMessage`：`lib/` 禁止裸 `helperText/errorText`，直接 `TextFormField` 必须接 `utenTextFieldErrorBuilder`；例外必须按独立文档写明原因并由源码契约登记。
...
| [`uten_editable_grid.dart`](../../lib/components/layout/uten_editable_grid.dart) | `UtenEditableGrid` 及控制器/列定义 | 可编辑明细表；列宽可拖拽调整(手动拖过即锁定)，列定义提供 `textOf` 后随内容自动加宽(只增不减、封顶 480)；另支持「只选不编」任务办理模式（`selectable` 勾选列与增删行操作条解耦 + `canSelectRow` 行级门控 + `selectedOf`/`onRowSelect` 外部受控选中 + `rowColor` 行语义底色，品质检查结果详情合并明细表为首例）；2026-09-03 起可编辑模式行级右键/长按操作菜单（复用 [UtenContextMenu](UtenContextMenu.md)：复制选中/粘贴/批量粘贴/在上方插入空行/删除选中，右键未勾选行先自动单选、已勾选保留多选，粘贴统一追加表尾；页面须提供 `cloneRow` 才显复制粘贴组，销售/采购/委外/钱流/库存/生产计划/生产日报 7 个编辑页已接） |
