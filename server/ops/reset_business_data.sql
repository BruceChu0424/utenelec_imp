...
BEGIN
    ...
    IF (applied_max_version, applied_migration_count) NOT IN (
        ...
        (649, 600),
        -- V650 热表约束/守卫触发器按相关列起跳 (ADR-106): 只改触发器, 不加表; 本迁移 600→601。
        (650, 601),
        -- V651 货品单位改为按需检查数量引用 (ADR-106): 删 104 个语句级触发器与 goods.quantity_unit_locked 列, 不加表; 本迁移 601→602。
        (651, 602),
        -- V652 索引卫生 (ADR-106): 删冗余索引、补热点领头索引, 不加表; 本迁移 602→603。
        (652, 603),
        -- V655 权限目录单一事实源 (ADR-109): 删角色体系四张 PRESERVE 表 (PRESERVE 102→98), V653-V654 跳号; 本迁移 603→604。
        (655, 604),
        -- V656 单据空归属不再全员可读、系统草稿显式声明归属池 (ADR-109): 只加列与回填, 不加表; 本迁移 604→605。
        (656, 605),
        -- V657 可对外接待的员工白名单=接待访客权限 (ADR-109): 只改权限码与授权策略, 不加表; 本迁移 605→606。
        (657, 606),
        -- V658 服务端会话与再认证 (ADR-110): 新增 auth_sessions/auth_step_up_states 两张 CLEAR 表 (CLEAR 298→300); 本迁移 606→607。
        (658, 607),
        -- V659 系统设置单一登记 (ADR-110): system_settings 删元数据列、补登记项行, 不加表; 本迁移 607→608。
        (659, 608),
        -- V660 账号支持只能个人授权 + 临时密码限时 + 高危权限标记 (ADR-110): 只改授权策略/触发器/加列, 不加表; 本迁移 608→609。
        (660, 609),
        -- V661 主档删除引用保护 (ADR-111): 只加守卫触发器, 不加表; 本迁移 609→610。
        (661, 610),
        -- V662 分类删除与往分类里加东西的并发闸 (ADR-111): 只加守卫, 不加表; 本迁移 610→611。
        (662, 611),
        -- V664 金额只由服务端精确派生 (ADR-112): 金额列改无精度 NUMERIC + 精确性检查, 不加表; V663 跳号; 本迁移 611→612。
        (664, 612),
        -- V665 总账附表行绑定 (ADR-112): 新增 finance_report_line_bindings 一张 PRESERVE 表 (PRESERVE 98→99); 本迁移 612→613。
        (665, 613),
        -- V666 到货超量异常金额快照改为精确列 (ADR-112): 只改列类型, 不加表; 本迁移 613→614。
        (666, 614)
    ) THEN
        RAISE EXCEPTION
