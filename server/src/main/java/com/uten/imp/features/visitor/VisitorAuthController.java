...
public class VisitorAuthController {
    ...
    public VisitorAuthDto.SendCodeResponse sendCode(@Valid @RequestBody VisitorAuthDto.SendCodeRequest req,
                                                      HttpServletRequest http) {
        return authService.sendCode(req.phone(), http.getRemoteAddr());
    }

    @PostMapping("/login")
    public VisitorAuthDto.VisitorTokenResponse login(@Valid @RequestBody VisitorAuthDto.VisitorLoginRequest req,
                                                      HttpServletRequest http) {
        return authService.login(req.phone(), req.code(), http.getHeader("User-Agent"), http.getRemoteAddr());
    }

    @PostMapping("/refresh")
    public VisitorAuthDto.VisitorTokenResponse refresh(@Valid @RequestBody VisitorAuthDto.VisitorRefreshRequest req,
                                                         HttpServletRequest http) {
        return authService.refresh(req.refreshToken(), http.getHeader("User-Agent"));
    }

    @PostMapping("/logout")
    public void logout(@Valid @RequestBody VisitorAuthDto.VisitorRefreshRequest req) {
