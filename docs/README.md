...
底层自制领料与 BOM 学习见 [ADR-119](99-决策记录-ADR/ADR-119-底层自制实际领料与BOM累计学习.md)：车间申请、仓库实际材料表格、真实出退料后累计单耗，以及制造底层 10% / 上层 0% 的超产默认与最新记忆。仅本地候选，部署状态另记。

物料分析“按物料汇总”的实际合单、父子用料和多产品份额见 [ADR-120](99-决策记录-ADR/ADR-120-物料汇总办理与共享制造来源.md)，现场三个产品的核对见 [只读诊断](99-项目治理/2026-09-25-物料汇总只读诊断.md)。新汇总批次保留原来源；已有单据不自动搬单。源码与隔离测试状态独立于业务库安装。
