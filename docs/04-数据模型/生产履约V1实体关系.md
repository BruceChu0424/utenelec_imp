...
- 订货“保存并提交财务”先保存草稿，再创建精确负责人审批实例；只有实例 `assignee_user_id` 且仍有审核权限的人可处理，超级管理员身份不能绕过对象校验。财务通过才将订单 `status=1` 并创建 `inbound_expectations`。
...
以上均为源码候选。公司目标库只读证据仍只到 V190，V196–V202 尚未部署、岗位 UAT、实物演练或完成 IQC，生产仍为 **NO-GO**。
...
> **迁移范围**：生产履约 V150–V164、V176 子计划归属、V191–V195 生产预排/MAKE 候选，以及 V196–V202 计划需求分解、订货财务审批、预计到货、超量控制和全表审计刷新候选；目标环境仍只确认到 V190。
...
| `workflow_responsibility_assignments` | 采购/委外两类财务行为的当前主负责人配置 | 只影响未来提交/异常；不能偷换在途实例负责人 |
...
3. 同一审核事务写已审计划、计划包、执行分段、需求、READY 整套占用/DRAW、BUY 采购申请、SUBCONTRACT 委外申请、精确销售分摊、Outbox，并将申请以 `status=1` 下达、草案终结为 `APPLIED`；这里不创建采购/委外订货草稿。
4. 采购/委外任务中心按 `open_qty` 选择申请明细，允许部分分解；一张订货草稿必须保持一个供应商/委外商且一个仓库，并逐行保存申请来源。
5. 订货保存后提交精确财务负责人；财务通过才把订单 `status=1` 并形成预计到货，驳回则释放待审来源占用供修改重提。
6. 仓库审核超量收货时先隔离且不写库存/AP；财务决定批准追加量后，仓库仍须再次审核，未批准量只交原下单人退回。
7. MAKE 只按本包当前直接 BOM 层聚合派生子计划：A→B→D 时 A 只建 B，进入 B 再建 D；禁止父包递归生成全部层级。
8. 任一约束失败时计划审核和全部下游一起回滚；已审无包可后续补建，已审有包只读回放。
9. 首次下达成功的无歧义车间和执行段后续成功 assign 可更新 V192 建议；空/多车间、失败事务和幂等 replay 不学习。
10. DRAW 审核只确认领料需求；仓库按行分轮 `issue`、确认实物发出后才减少物理库存并消费占用。
11. 报工审核后更新合格累计；成品入库审核后才增加成品库存和销售可用进度。
12. 退料由仓库验收后才回原料可用库存；结案前再次校验物料与成品双门禁。
...
- V196/V201 的审批与到货异常事件同样 append-only；在途实例负责人、订单来源和已绑定批准额度不得由通用 CRUD 偷换。
