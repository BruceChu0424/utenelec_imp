...
| `/rd/tasks` | RdTaskCenterPage | 工程研发部任务中心；要求 `rd_task:view`；入口在工作台「工程研发部」分区「任务中心」卡，角标由 `WorkbenchCardBadge(rdTask)` 统一渲染；与 `/production/schedule/forward-rd` 联动 | ✅ |
...
- 开发原库 `uten_imp` 2026-08-10 已确认到 V244/225；截至 2026-08-14，共享工作树源码目录最高 V289/270，270 个版本号无重复。V288/V289 已形成借用结构、终态守卫与审计覆盖，后端 create/revoke 路由、持久化、生效计算和 Flutter UI 已接通为源码候选。V253/234 只是 2026-08-11 阶段快照。一次性生产物料克隆仍是 V250/231 历史证据，公司目标库仍以 V238 既有只读证据为准；源码路由存在不等于真实账号、岗位、实物和生产验收完成。
