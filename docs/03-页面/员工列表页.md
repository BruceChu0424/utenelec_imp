...
|---|---|
| `UtenAppBar` | 标题和返回 |
| `UtenSearchBar` | 工号/姓名搜索，组件内 300ms 防抖 |
| `FilterChip` | `active/probation/onLeave/resigned` 多状态筛选 |
| `UtenPersonCard` | 员工摘要卡，点击进入详情 |
...
- 后端按工号升序稳定分页；页面以 `totalPages` 判断是否显示“加载更多”。
...
- 首屏错误会显示可重试错误态。
- 下一页失败目前只恢复按钮，不展示明确错误原因，是已知 UX 限制；不得假装已经加载完整。
...
**最后更新**：2026-07-30 · **状态**：真实 API + 服务端分页；全局视角选择器和桌面 master-detail 未实现。

## 八、2026-07-31 领导排序与并发保护

- 后端在分页前按“部门负责人、领导层、班组管理、普通员工、工号”稳定排序，负责人不会落到后续分页。
- `UtenPersonCard` 在姓名前展示“负责人 / 领导 / 班组管理”文字徽章，不只依赖颜色或图标表达。
- 搜索、状态筛选、重新加载和加载更多共享请求代次；旧筛选响应会被丢弃，不会覆盖或混入新结果。
- 加载更多失败会恢复按钮并显示错误消息；从员工详情返回后重新加载当前列表。

**最后更新**：2026-07-31 · **状态**：真实 API + 服务端领导优先分页 + 请求竞态保护。
