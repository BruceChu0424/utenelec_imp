-- V252: refresh audit trigger coverage after V251 added goods import tracking tables.
--
-- goods_import_batches records import/undo state and goods_import_creations preserves the
-- exact entities created by a batch. Both are business provenance needed to explain and
-- safely execute an undo, so their future changes must be captured by row-image auditing.
-- This is the same fail-closed full sweep as V242. It does not backfill historical audit rows.
