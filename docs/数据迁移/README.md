...
| 说明 | 迁移 | 本次变化 |
| --- | --- | --- |
| [227](227-V641我方供料委外件放开公共超量.md) | V641 | 只锚点补丁两个函数：`fn_guard_preplan_public_surplus_shape` 删掉「SUBCONTRACT 公共超量必须不是单一叶子子件件」那一段(V589 留下的最后半条)、`fn_preplan_direct_overorder_capacity` 删掉「委外件有 BOM 恒返 0」那一条(订货审核登记公共在途的容量上限，不放开就撞 23514)。不加表、不加列、不动触发器、不改数据；服务端同批撤掉 `notifySupplyInternal` 里同口径的可读拒绝，客户端不再把这类行的超量输入拦在本地 |
