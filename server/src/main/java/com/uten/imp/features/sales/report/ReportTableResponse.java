...
/**
 * 销售报表通用响应（8 张报表共用：订货/出货/退货/其它出货 各明细/汇总）。
 *
 * <p>设计（与采购 purchase.report.ReportTableResponse 同型）：服务端 JOIN 出"显示就绪"行
 * （客户/仓库/货品/颜色/类别/人员名/总监均已解析），前端 MasterDataTableView 按 {@link #columns}
 ...
 *   <li>{@link #facets} 每列的筛选档（value/label/count）；只对离散列（客户/仓库/状态/是否审核/结帐方式…）提供。</li>
 * </ul>
 */
