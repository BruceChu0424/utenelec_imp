-- Preserve all submitted commercial display facts without changing the canonical
-- submission_snapshot or snapshot_hash used by in-flight finance decisions.
-- Existing cases intentionally remain NULL: their missing facts are unknown.
ALTER TABLE procurement_order_approval_cases
    ADD COLUMN display_snapshot JSONB,
    ADD CONSTRAINT procurement_approval_display_snapshot_object
        CHECK (display_snapshot IS NULL OR jsonb_typeof(display_snapshot) = 'object');

COMMENT ON COLUMN procurement_order_approval_cases.display_snapshot IS
    'Immutable submitted display facts, including names, remarks, weight, loss allowance and all source allocations; NULL means legacy facts were not retained';

CREATE FUNCTION fn_procurement_approval_display_snapshot()
        ...
        SELECT ($1 - 'items') || jsonb_build_object(
            ...
            'items', COALESCE((
                SELECT jsonb_agg(submitted.value || jsonb_build_object(
                    ...
                    'weight', to_jsonb(item) ->> 'weight',
                    'giftQty', to_jsonb(item) ->> 'gift_qty',
                    'allowedLossPct', to_jsonb(item) ->> 'allowed_loss_pct',
...
CREATE TRIGGER trg_procurement_approval_display_snapshot_insert
    BEFORE INSERT ON procurement_order_approval_cases
    FOR EACH ROW EXECUTE FUNCTION fn_procurement_approval_display_snapshot();
CREATE TRIGGER trg_procurement_approval_display_snapshot_immutable
    BEFORE UPDATE OF display_snapshot ON procurement_order_approval_cases
    FOR EACH ROW EXECUTE FUNCTION fn_procurement_approval_display_snapshot();

ALTER TABLE procurement_order_approval_cases ENABLE ALWAYS TRIGGER trg_procurement_approval_display_snapshot_insert;
ALTER TABLE procurement_order_approval_cases ENABLE ALWAYS TRIGGER trg_procurement_approval_display_snapshot_immutable;
