# 老库数据迁移 · 总索引
> **当前共享源码候选(2026-09-02)**：迁移目录头为 V456，共 418 个迁移文件、418 个唯一版本且无重号。V456 页面内授权入口接线：`quality.task-center` 扩成 4 码专属面（IQC/FQC 两域 view + IQC 处置 + FQC 审批）并退役 `quality.inspection` 旧面（能力严格超集，无回退）；`quality.production-fqc` 补 FQC 审批码并更名「生产成品质检与FQC补产」，`/quality/production-inspections` 自此有专属委派面；`warehouse.quality-results` 补 IQC 入库确认码；surface 目录净 -1 至 110 个；生产调度/进度为纯前端接线无需迁移；零默认 grant，不新增业务表，重置白名单增至 V456/418、CLEAR 清单不变。空库 V1→V456 与重置 PostgreSQL 测试已过；目标库迁移、岗位 UAT、恢复与签名发布均未完成，生产 **NO-GO**，见 [迁移说明 76](76-V456页面内授权入口接线.md)。V455 权限目录收口见 [迁移说明 75](75-V455权限目录收口.md)。
>
