...
> **2026-09-01 状态**：仓库 hub 已按本路线图完成重组——任务中心三张方向卡
> （出库/入库/生产领料，见[出库任务中心页](出库任务中心页.md)等）+ 出入库单据区
> 收敛为调拨/盘点/委外特殊单据；库存查询合并进即时库存+库存详情。本文其余
> 内容为当时的路线图记录。
...
>
> 财务批准的预计到货不是库存，IQC PASS 也不是已入库。实际收货/回厂后，PASS 只形成品质合格量和
> 仓库待入库任务；仓库确认本次实际入库量与实际库位后，才写库存流水并按实际入库切片刷新物料齐套。
> 它不能直接增加生产计划的「已报工」或成品「已入库」。仓库单据应从申请/订货/收货/质检/
> 仓库确认/库存流水一路回查到原逐路径 allocation。完整后置口径见
> [采购 / 委外 IQC 合格待入库任务页](采购委外IQC合格待入库任务页.md)。当前真实多岗位与实物流转
> 仍未闭环，这是一条验收合同，不是“已全链上线”的声明。
>
...
- **订货明细「申请来源」列取代引入总结横幅（2026-09-01）**：采购订货单编辑页删除表头下方「已从采购申请 X 引入 N 行」总结横幅，改为明细表「申请来源」只读列逐行展示来源申请单号——任务中心带单新建与「从上游引入」均自动回填（引入面板确认结果随行携带来源单据 id/单号），点击单号跳转只读申请详情（无来源 id 时退化为纯文本）。该列纯前端展示不随保存提交：订单行 `sourceDocNo` 谱系仍由服务端按申请行继承计划号。手动添加的空白行该列显示「—」。
