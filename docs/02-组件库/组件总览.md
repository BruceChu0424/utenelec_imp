...
| 组件 | 用途 | Phase |
|---|---|---|
| `UtenButton` | 主按钮（primary/secondary/**tonal**/ghost/danger）+ loading + 三尺寸；`tonal`=浅青绿底+深绿字（品牌色调次要按钮，与分类树选中态视觉一致） | 0 |
...
| `UtenExportButton` | 加密 Excel 导出按钮：点击弹密码框 → POST 后端生成 → 跨端下载（Web blob / 桌面移动写文件）；**文字按钮"下载表格"**（年长用户友好）+ 防连点 + loading + 审计。详见 [UtenExportButton.md](UtenExportButton.md) | 3 |
...
| **`showUtenGoodsPicker`** | **统一货品选择器（单据明细/报表选货品）**：左分类树+右货品表（搜索+分页），右滑入面板/底部抽屉，**排除原材料/辅料/未分类**；返回完整 `GoodsListItem` 供回填颜色/单位。位于 `features/basic_data/widgets/`（跨模块共享）。[文档](UtenGoodsPicker.md) ✅ | 3 |
...
| `UtenToast` | 静态底部轻提示（success/warning/error/info）。**存量兼容，新功能一律用 UtenNotify** | 0 |
| `UtenLoading` | 加载指示器（菊花/点） | 0 |
| `UtenConfirmDialog` | 确认对话框 | 1 |
| `UtenDialog` | 确认对话框（UtenButton 动作 + 主题适配，静态 show 方法） | 2 |
| **`AppNotification`** | **顶部全局通知服务（自建）**——替代所有 SnackBar；统一 success/error/warning/info 配色；600ms 同 message 合并去重；支持自定义图标 + 点击跳详情 | 2+ |
| **`UtenCenterAlert`** | **屏幕正中弹窗（重要通知通道）**——三档紧急度 `UtenAlertLevel.normal/important/urgent`；urgent 红色警示 + 默认禁止遮罩关闭。[文档](UtenNotify.md) ✅ | 3 |
| **`UtenNotify`** | **统一通知门面（唯一调用入口）**：`banner` 顶部弹条（微信式，日常）+ `alert` 居中弹窗（重要/紧急）。[文档](UtenNotify.md) ✅ | 3 |
...
| **`MasterDataTableView`** | **Excel 风格数据表格（全局唯一数据展示组件，强制）**：横排 autofilter 列头筛选 + 列头排序（日期/金额/数量）+ 分页 + 横滚同步。位于 `features/basic_data/widgets/`（跨模块共享），6 报表族 + 6 主档 + 单据列表共用。[文档](MasterDataTableView.md) ✅ | 2+ |
...
### 2.9 State Infrastructure（状态基建）— `shared/providers/`

| 组件 | 用途 | Phase | 文档 |
|---|---|---|---|
| `UtenPagePrefsNotifier<T>` | 页面级用户偏好基类：筛选项/开关/布局「上次选择」按账号服务端持久化（`user_preferences`）；本地缓存即时渲染 → 登录后服务端同步 → 防抖 800ms 推送，子类只需声明 prefKey + 序列化 | 3 | [UtenPagePrefs.md](UtenPagePrefs.md) ✅ |
...
**最后更新**：2026-07-28 · **Phase 0 组件进度**：0 / 12 · 新增 §2.8 Brand 分类（UtenWordmarkLogo / UtenBrandMascot）；§2.3 加 `UtenLocationField`（添加位置选择，分类/部门共用，[文档](UtenLocationField.md)）与 **`showUtenGoodsPicker`**（统一货品选择器，左树+右表，排除原材料/辅料/未分类，[文档](UtenGoodsPicker.md) / [ADR-015](../99-决策记录-ADR/ADR-015-统一货品选择器与legacy到UUID桥接.md)）；§2.1 `UtenButton` 加 `tonal` 类型（浅青绿底+深绿字，品牌色调次要按钮）。基础资料四页共用的模块组件（`CategoryDetailCard` / `showMasterEditDialog` / `showMasterDetailSheet` / `CategoryEditDialog`）见 [基础资料页.md](../03-页面/基础资料页.md)
