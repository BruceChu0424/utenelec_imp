...
- compact（手机，< 600dp）：
  ...
  │ 🔒 工资信息属于个人隐私 │
  ├────────────────────────┤
  │ （右下悬浮：⬇ 下载工资条）│  ← 右下角悬浮按钮（2026-09-14）
  └────────────────────────┘
  ```
  说明：单列滚动，Hero 卡 + 多个 UtenCard 分组 + 右下角悬浮下载按钮（large）。
...
8. **底部下载按钮**：固定 SafeArea，调用 `POST /payroll/slips/{id}/download` 取得 PDF bytes，
   校验 `%PDF-` 文件签名后通过平台保存工具落盘；成功提示保存路径，并刷新列表/详情中的下载时间状态。
...
flowchart TD
    ...
    R --> DL{点下载}
    DL --> API[POST /payroll/slips/:id/download]
    API --> SIG[校验 PDF 签名]
    SIG --> SAVE[平台保存文件]
    SAVE --> T[提示保存路径并刷新状态]
```
...
- 当前没有工资条离线缓存或写队列；断网时详情、查看标记和下载明确进入错误态。工资数据敏感，
  未完成加密缓存、过期清除和身份切换隔离设计前，不得持久化离线副本。
