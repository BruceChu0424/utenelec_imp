...
> 路由：`/login` · 实现：`lib/features/auth/pages/login_page.dart` · 仓储：`DioAuthRepository`
...
4. `DioAuthRepository` 调用 `/api/auth/login`，成功后把 access/refresh token 写入
   `flutter_secure_storage`，并更新 `sessionProvider`。
5. 启动时若安全存储存在 refresh token，`sessionProvider` 调 `/api/auth/me` 恢复员工会话；
   恢复失败就清理本地凭据。access 401 的单飞刷新和最终登出由网络拦截器/会话事件总线处理。
...
**最后更新**：2026-07-30 · **状态**：真实鉴权链路已接；全量 CI、真实 API 权限/时序和多端
E2E 仍是生产验收项。
