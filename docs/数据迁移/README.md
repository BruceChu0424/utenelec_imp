...
> **当前正式目录：V644/594 (2026-09-22)**。V644 把 V409 的「创建命令账本」升级为「日报命令账本」: production_daily_report_commands 加 command_kind 一列(CREATE/APPROVE/REVERSE, 存量默认 CREATE), 原 UNIQUE(report_id) 换成 UNIQUE(report_id, command_kind); 审核接口自此要求幂等键, 同键重发原样回放已审详情而不是撞状态闸门, 状态闸门本身不放宽、只把错误码从 400 改成 409; 不加表, 清库策略与审计覆盖不变(V642、V643 留给并行会话)。V641 放开「我方供料的委外件不能公共超量备货」这条禁令：锚点补丁两个函数(`fn_guard_preplan_public_surplus_shape` 去掉「单一叶子子件不许带公共量」那一段、`fn_preplan_direct_overorder_capacity` 去掉「委外件有 BOM 就恒返 0」那一条)，不加表不加列不动触发器；口径是 2026-09-21 用户「采购能超量下、委外和车间也要能」，多下的量已由 ADR-099 修订按计划产出量如实带大子件需求，不再是无人负责的需求。实际拦住人的是客户端与服务端那两道可读校验；数据库这一处放开是**预备性的**——`PreplanPublicSupplyCaptureService.candidates` 对委外路线写死了「有活动 BOM 就排除」，这类件放开前后都走不到那道触发器、也不会撞 23514，改它只是不让同一口径的三份拷贝里留一份相反的。V640 (ADR-099 数量单一入口) 让仍未订货的采购/委外申请明细可以就地追加数量：新增「明细是否未订货」「供给行动是否可就地追加」两个判定函数，并锚点补丁 V460 的供给行动/分摊行身份守卫与 V472 的公共量身份守卫，只对「明细未订货」的供给行动放开只增不减的 requested_qty / allocated_qty / public_surplus_qty 改量，不加表不加列(V639 跳号)。V638 给委外回厂先出后进守卫的「已审出仓量 / 已消费量」合计补上 `PREPARED_OUTBOUND` 行(ADR-062 §十前置自制订货超量后同一明细 PREPARED + DIRECT 混合出仓，回厂不再被只算 DIRECT 份的出仓量拒绝；激活条件不变，V635、V637 留给并行会话)。V636 (ADR-098) 给委外订货明细加「允许损耗%」(货品主档同名默认值做记忆, 保存订货单回写), 新增回厂短交案件表与追加式事件表(到货登记累计回厂少于订货量即按行开案件, 低于允许下限须仓库确认并通知委外; 委外判定「分批到货继续等」或「接受损耗结案」= 自动损耗单 + 受控改量 + 记损耗率), 新增权限码 `subcontract_short_delivery:decide` 与供应商×货品/供应商两层损耗汇总视图; V635 跳号。 V634 只替换委外准备来源谱系断言函数：委外前置自制按 V589 超量下达(归需求量 + 公共备货产出)后，订货财务批准把订货行数量与前置自制台账 `required_qty` 比而不再与分析行 `requested_qty`(只记归需求量)比，超量整批下单不再在批准提交时被 `subcontract_prepared_outbound_lineage_guard` 拒绝(V633 跳号)。V632 让出货财务放行时确认记账汇率(预填币种主档参考汇率、财务可按放行当日汇率改、放行即冻结到本单与放行事件，仓库确认出库按冻结值折算本币立应收；本位币恒 1)，并在库里没有任何已确认收付款方式时补一套标准方式(银行转账/现金/支票/承兑汇票)，收款单「收款方式」不再是空下拉。V631 让销售出货按行记实际发出仓(明细 `warehouse_id` + 出库事件 `line_warehouses`)，仓库确认出库时按行选仓、预填建议仓，一张出货单可从多个叶仓发出，同时财务放行前先校验币种汇率。V630 退役客户「月结/现金/定金」货款类别标签(删 clients 与放行事件表两列、约束、索引、V443 异常视图)，出货财审改看本单结账方式，旧系统首导结构对账 24 项收为 23 项。V629 让无需物料的任务同样可选三条路线、给路线记忆加操作者档索引，并把物料缺口按「自制子件」而非「直送」归桶(ADR-096)。V628 让车间路线在开工前随时可换并把逐种物料事实做成列表/详情的唯一来源(ADR-095)。V609–V613 重构车间执行：冻结准确耗用曲线、普通仓与内部流转位置分离、先确认路线、同任务持续补料，以及直送基础单位和混合供给去重。新任务不再按 V606 自动选择路线；已发生的历史库存/报工/成本事实保留。V607/V608 为共享工作区的客户选填与报销迁移；V617前向补齐报销人工核验、其他凭证与财务设置。V618/V619记录当前车间余料的实际正常收仓、来源保管与正反向。目录头仅表示源码集合，不代表目标服务已升级。各版本变更、存量影响与验证边界见下表。
>
...
| 说明 | 迁移 | 本次变化 |
| --- | --- | --- |
| [228](228-V644审核生产日报幂等键.md) | V644 | `production_daily_report_commands` 加 `command_kind` 一列(CREATE/APPROVE/REVERSE，存量默认 CREATE)，原 `UNIQUE(report_id)` 换成 `UNIQUE(report_id, command_kind)`。不加表；审核接口自此要求幂等键，同键重发原样回放已审详情而不是撞状态闸门，状态闸门本身不放宽、只把错误码从 400 改成 409。V642、V643 留给并行会话 |
