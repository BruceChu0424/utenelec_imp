...
**最后更新**：2026-08-02（补 V196–V201 采购/委外任务中心、精确财务审批、预计到货、超量异常和原下单人退回路由；新增 `/rd/tasks` 工程研发部任务中心与 `POST /production/schedule/forward-rd` 转发链路；路由守卫不替代对象负责人/`allowedActions` 校验，目标库仍只确认到 V190）
