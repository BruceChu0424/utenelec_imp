...
### 3.2.4 服务端会话（`auth_sessions`，V680，ADR-110）
...
- 服务端会话表：已实施(§3.2.4，V680)；会话列表 + 远程登出界面：未实施(表已具备)
- 异地登录告警：未实施
