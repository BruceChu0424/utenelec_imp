...
class StockDocFilter {
  const StockDocFilter({this.keyword, this.warehouseId, this.status, this.departmentId, this.issueStatus});
...
class StockDocRepository {
  ...
  Future<StockDocDetail> issue(String id, List<Map<String, dynamic>> lines) async =>
      StockDocDetail.fromJson(
          await api.post(ApiEndpoints.stockDocIssue(id), body: {'lines': lines}));

  /// DRAW 反出库：对称回退已出库量
  Future<StockDocDetail> reverseIssue(String id, List<Map<String, dynamic>> lines) async =>
      StockDocDetail.fromJson(
          await api.post(ApiEndpoints.stockDocIssueReverse(id), body: {'lines': lines}));
}
