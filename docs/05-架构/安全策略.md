...
- 通知只是提醒，不是授权或任务事实；任务发现、详情和决定接口都由 `WorkflowReviewerEligibility` 实时判定当前账号是否合格审核人，并用非审核人账号(不在财务部且未点名)做 404/403 负向测试。提交/到货超量检出时通知全体合格审核人。

V196–V202 已包含在公司目标库当前 V238 以内，源码继续演进到 V244；但迁移存在不等于业务验收。
...
- 🟡 **已有安全基线**：Argon2id、JWT/refresh 轮换、会话空闲超时、pgcrypto、截至 V244 的迁移与
  分层业务表审计覆盖、
  ...
  越权矩阵仍需验收。
  2026-08-09 后端全套在 `UTEN_RUN_DB_TESTS=true` 下为 269 类、1065/1065，Flutter 530/530 且
  analyze 0 issue；FullChain 46/46 在 PostgreSQL 16.14 真实执行 225 个迁移到 V244。会话原子记录、
...
**最后更新**：2026-08-09（V241 local/cloud 三层门禁、远程授权双向会话失效、Release 固定端点、
