...
- **数据变化层**：V169 为当时的公开业务表补齐 `AFTER` 触发器，提供脱敏 before/after；V183 最终为其
  ...
  同时保留 V172 设备关联证据。凭证、令牌、短信码、Flyway、序号
  与迁移运行元数据表不复制行内容。
...
**最后更新**：2026-08-01（补充审计默认可见性、对象/来源/Request ID/操作范围筛选、高水位边界分页、
V184 触发器强校验、V185 软删除/脱敏加固、迁移静态护栏与不可补造历史边界；保留目标 PostgreSQL 和生产验收限制）
