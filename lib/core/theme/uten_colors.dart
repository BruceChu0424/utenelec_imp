...
abstract final class UtenColors {
  ...
  /// 黄色「进行中」数量徽章的**实底色**(amber-600)。
  ///
  /// 2026-09-21 定色过程(用户两轮反馈):
  ///   ① 起初用 [warning](amber-500) + 深墨字 -> 用户要「数字加粗变白、黄色再深点」;
  ///   ② 换成 amber-700 白字(对比度 4.5:1) -> 用户要「这个黄色不是黄色了, 稍微淡点」
  ///      —— amber-700 已经偏棕, 读起来不像黄色。
  /// 最终落在 amber-600: 明显比 700 淡、色相仍在黄档, 白字对比度 3.2:1。
  /// **不要再往深调**(会变棕, 违背用户口径), 也不要往浅调(白字读不清)。
  /// 明暗两档共用同一个实底 —— 深色模式那枚 [warningOnDark] 是给**文字/图标**用的
  /// 亮琥珀, 拿来做底再压白字会彻底糊掉。见 docs/00-项目准则/14-徽章与计数口径.md。
