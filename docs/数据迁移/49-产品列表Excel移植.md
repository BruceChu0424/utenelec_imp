...
| 产品分类 | `86开关插座->原材料->塑胶类->塑胶件` 路径，398 条叶路径、最深 6 级 | 建新分类树 + 重指 `goods.category_id` |
...
- **新分类树**：Excel 路径逐段按（父, 名称）幂等建节点；新节点编码 `XL01/XL02…`（每父级内按序，
  前端树按 code 排序即导入顺序）；顶层两个：`86开关插座`、`商品运费`（8 行运费类产品）。
