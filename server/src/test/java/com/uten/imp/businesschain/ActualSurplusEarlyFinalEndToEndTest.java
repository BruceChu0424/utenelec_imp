...
class ActualSurplusEarlyFinalEndToEndTest {
    ...
    void earlyFinalKeepsPublicTenSeparateAndRemakesOnlyTheOtherOrdersHundred() {
        ...
        var world = fixture.seedWorld("actual-early-final");
        ...
        Object assignment = ReflectionTestUtils.invokeMethod(fixture,"productionAssignment","actual-early-final");
        ...
        line.setQty(new BigDecimal("110")); line.setIsFinal(true);
        ...
        var usage = new DailyReportMaterialUsageLine(); usage.setDemandId(demand); usage.setQtyBase(new BigDecimal("110"));
        ...
        qty("110","SELECT fqty FROM production_plan_items WHERE id=?",planItem);
        qty("100","SELECT planned_qty FROM production_execution_segments WHERE id=?",segment);
        qty("110","SELECT fn_production_execution_cost_target(?)",segment);
        ...
        qty("10","SELECT qty FROM production_daily_report_items WHERE report_id=? AND is_actual_surplus AND NOT is_deleted",report);
        ...
        qty("110","SELECT confirmed_consumed_qty FROM v_production_material_clearance WHERE demand_id=?",demand);
