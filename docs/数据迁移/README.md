...
> **生产物料分析重构增量（2026-08-08）**：V234 新增计划前需求、全树物料快照、供应动作、命令幂等和计划分批链接；V237 在其及后续业务表之后重新执行 fail-closed 审计覆盖刷新。详见 [56-生产计划前需求与物料分析重构](56-生产计划前需求与物料分析重构.md)。本机 PostgreSQL 空库迁移与定向守恒测试不等于目标库迁移、历史对账或真实岗位 UAT，生产写链路仍为 **NO-GO**。
...
>
> **附件对象存储 + 双服务器云端授权增量（2026-08-09）**：V240 通用附件表 `attachments` + `attachment:manage/view` 权限（二进制走阿里云 OSS / 本地盘，预签名 URL 直传，默认 `provider=local` 本地测试、填 `UTEN_OSS_*` 切 `oss`）；V241 `users.remote_access` + 触发器（云端外网门禁，即时失效旧 token）。云端只读副本 + 双数据源读写路由（`@Profile("cloud")` 隔离，**非 cloud profile 零影响**；断网降级只读/写 503）+ 部署 runbook 见 [deploy/cloud/README-cloud.md](../../deploy/cloud/README-cloud.md)。云端真实部署需阿里云 ECS + VPN/专线，填 `server-cloud.env` 即跑、缺 key fail-fast；附件与路由逻辑已单测/E2E 覆盖。

<!-- PRODUCTION-PLANNING-V195-CURRENT -->
> **生产计划迁移增量（2026-08-02）**：V191 预排草案、V192 车间建议、V193 审计覆盖刷新、V194 MAKE 供给生命周期/三态人工放行/生产组织层级守卫，以及 V195 在 V194 后再次刷新审计覆盖，均为候选迁移，详见 [52-生产预排审核下达与车间建议](52-生产预排审核下达与车间建议.md)。新数据处置为 `READY`、`AUTO_WAIT`、`DEFERRED`；`release-defer` 只允许 DEFERRED 单向人工放行并立即重做齐套。车间、班组、负责人和日期由应用层与 V194 数据库守卫共同校验。V191–V195 全部禁止历史业务回填：不重算旧 BOM/计划、不补造计划包/子计划/供给分摊、不从旧单猜车间或延期状态；V193/V195 只保护迁移后的未来写入，不补历史审计。目标环境当前最高 V190，候选未部署且未完成真实岗位 UAT；在备份、前后计数/金额/数量不变证明和发布签字完成前仍为 **NO-GO**。
...
| **销售管理** | 🟡 单据导入已实现；V187–V189 新流程结构已有源码，目标库迁移/对象授权待最终验收 | `S_Order`(10653)/`S_Out`(12124)/`S_OtherOut`(1558)/`S_Withdraw`(221)（在用）+`S_Quote`(0) | `sales_orders/shipments/other_shipments/returns(+_items)` + V187 发运/仓库字段 + V188 仓库事件 + V189 退货质量冻结 | `--sales` 导单；员工迁入后 `--sales-owner` 回填（完整流程用 `--bootstrap-all`） | [18-总路线图](18-业务四模块-总路线图.md) · [20-新库与迁移](20-销售管理-新库与迁移.md) · [39-owner 迁移/授权](39-销售单据归属授权.md)；历史不补造确认、拣货事件或质检结论 |
...
目标库执行前后至少对账：四类销售主表/明细行数和数量金额汇总不变；V187 新列 null/枚举分布符合历史映射；迁移后旧草稿仍能走兼容审核，新单不能走旧审核；无权限用户看不到动作且直接调用 403。当前候选已在隔离 Testcontainers PostgreSQL 16.14 空库由 Flyway 迁移并 validate 171 个迁移至 V190，真实 PG 54/54；但这不是公司目标 PostgreSQL 的升级或历史全量对账证据，目标库门禁仍未关闭。
...
### V196–V202 计划申请分解、订货财务审批与超量到货候选
