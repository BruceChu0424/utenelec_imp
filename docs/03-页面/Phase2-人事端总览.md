...
> 员工域旧 Mock 已删除，入职一次性临时密码=证件号后 6 位（登录账号默认=手机号，工号提交时自动生成 UT 前缀）。负责人设置与调岗/离职联动、领导优先
...
| `UtenDepartmentPicker` / 入职岗位选择弹层 | 组织、岗位选择 | 入职两者均为“弹层内草稿 + 底部确定回填”；岗位默认留空、支持搜索和自定义新名称 |
...
- [x] 管理中心可承载直属人员/岗位/负责人，中心负责人可在对象级守卫下管理子孙部门；中心结构位置仍锁定
...
- 负责人必须是本组织直属在册员工并可显式清空；管理中心也遵守该规则。负责人调岗、离职或归档时自动解除。
- 部门和个人权限入口沿用权限管理页，只对超级管理员且具备 `authorization:manage` 的账号开放。

---

**最后更新**：2026-08-03 · **状态**：入职工号服务端分配、岗位确认选择/自定义、管理中心用人与中心负责人子树范围已接入源码；目标库迁移、实例重启和生产 E2E 未完成。
