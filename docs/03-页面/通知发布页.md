...
> 上层：[页面总览](页面总览.md) · 读端见 [通知列表页](通知列表页.md)
...
`UtenFloatingActionGroup`（右下悬浮发布/取消，2026-09-14 全站口径）、`UtenActionButton`、
`UtenDateField`（待办截止日期，2026-09-18 起：按天选择、当天 23:59 到期）和顶部通知；
类型、置顶、范围选择暂用主题化 Material 控件，部门复用 `UtenDepartmentPicker.multi`，
人员使用 `UtenEmployeeMultiPicker`。
...
1. **类型**：9 种（复用 `NoticeType`），由分组选择器 `NoticeTypePicker` 选择——
   ...
   - **类型 → 打卡提示（2026-09-10，ADR-063 §8）**：非庆典类型选择器下固定一行「公告/制度/系统/紧急/福利 类型要求接收人登录打卡确认（未打卡每次登录都会弹窗提醒）；任务/审批/流程类只提醒」。语义：`interaction_mode` 仍由类型静态派生（`NoticeService.interactionModeFor`，本页不可改）——acknowledge 类型的接收人每次登录都会在居中「登录提醒」弹窗看到该通知并须点【打卡确认】（`POST /api/notices/{id}/acknowledge`），直到打卡为止，无时间上限；待办通知（kind=TODO，type=task，none 模式）只在 14 天内未读时提醒，「知道了」即止。接收范围：全员动态可见，指定范围按发布时快照，弹窗口径与列表可见性一致。
...
5. **附件**：后端仍兼容附件文件名数组；通用 Attachment 已有源码候选，但通知域 owner policy、上传 UI 和生产附件门禁尚未接入，因此人工发布页暂不开放上传。
