# 老库数据迁移 · 总索引
> **当前共享源码候选(2026-09-02)**：迁移目录头为 V455，共 417 个迁移文件、417 个唯一版本且无重号。V455 权限目录收口：补 10 个守卫页 `permission_surfaces`（授权管理总控/审计中心/系统设置/本机回执核查/品质任务中心/生产FQC补产队列/仓库质检结果/生产调度/生产进度/生产链路健康，零默认 grant）；下线 15 个双端零引用孤儿码（`stock:edit`、`inventory:view`、`attachment:manage` 等，V423 模式清授权（含 `manager_permission_delegations` 外键引用）再删码，存量部门授权 20 条一并清理）；`quality.lab-test` 空壳面随 lab:test 码退役，surface 目录净 +9 至 111 个；不新增业务表，重置白名单增至 V455/417、CLEAR 清单不变。空库 V1→V455 与重置 PostgreSQL 测试已过；目标库迁移、岗位 UAT、恢复与签名发布均未完成，生产 **NO-GO**，见 [迁移说明 75](75-V455权限目录收口.md)。V454 庆典祝福聚合卡见 [迁移说明 73](73-V454庆典聚合祝福卡.md)。
>
...
> **本机开发库零基线重置（现行口径）**：`server/ops/reset_business_data.sql` 按 CLEAR/PRESERVE 双目录在单事务内清零业务数据并刷新六张物化视图（仅限可丢弃本地/测试库；范围、三闸确认与门禁见脚本头部，禁止用于公司目标库或生产）。最近一次：2026-09-02 09:41（UTC-06:00）于 V454/416 按明确指示**不创建备份**执行；清前 20 张业务表共 119 行，清后 CLEAR 220 张全部 0 行、PRESERVE 95 张逐表行数与清前完全一致，五张库存事实表与六张月度物化视图均 0 行，27 个账户金额/完整性异常均为 0，客户/供应商/货品/结算类别期初及货品安全库存/成本预算异常均为 0，1 个归属业务表的 owned/identity 序列回到起点且 `is_called=false`，Flyway 仍为 V454/416、失败迁移 0、禁用用户触发器 0、其它客户端连接 0；脚本契约测试 5/5。主档、人事、权限、审计、Flyway 与治理/编号高水位保留，ERP 应用保持停止；因本轮未建备份，不存在本轮恢复点。2026-08-17 至 2026-09-01 间的历次清库流水（V399→V450 共 20 余次）已随 2026-09-01 文档清理移除，需要证据时查 git 历史。
