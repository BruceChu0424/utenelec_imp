...
- **竖向滚动条按滚动阶段显隐（2026-09-14 起）**：表内竖向滚动条在
  [UtenCollapsingHeaderScrollView](UtenCollapsingHeaderScrollView.md) 内（联动折叠页）按
  `UtenInnerScrollActiveScope` 门控——外层收头部阶段（表格未置顶）隐藏，进入表体内滚后
  显示（thumb 与表体内容对应）；独立表格（不在该容器内）查不到 scope，维持常显。
...
- **`summaryBarInline`(2026-09-15 新增，合计条随表体滚动)**：默认 false(钉在表体外的
  全站口径)。传 true 时 `summaryBar` 改渲染进**表体竖向滚动内容的末尾**——跟在最后一行
  数据(与「加载更多」指示器)之下：行少时紧贴末行、行多时要滚到底才见，合计条属于表格
  那一块。用于详情页「合计行是表格脚注」语义(销售单据详情、销售订单财务审核详情)；
  列表/报表页维持钉底口径(合计随时可见)。
  **横向不跟着表格内容走**：合计条整体按当前横滚位置左移(`_ViewportPinnedRow`，
  `Transform.translate` 只改绘制、不改布局)，右边缘恒定落在表体**可视框**右缘——表比卡片
  宽时不必把表格拖到最右才看得到「合计数量/合计金额」(2026-09-15 用户反馈)。表宽没撑满
  可视框时保持贴表格右端(位移不取正)。契约见
  [`test/features/sales/pages/sales_doc_detail_totals_bar_test.dart`](../../test/features/sales/pages/sales_doc_detail_totals_bar_test.dart)。
