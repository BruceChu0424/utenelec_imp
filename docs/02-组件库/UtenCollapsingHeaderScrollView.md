...
**何时用**：页面是「上方有一块固定顶部（卡 / 统计 / 工具条）+ 下方 `Expanded(MasterDataTableView)`」的结构，顶部又较高、挤压了表格。已接入：货品 / 模具 / 客户 / 供应商 分类详情页；任务工作台（采购 / 委外 / 仓库，expanded 断点）；订单进度查询；采购 / 仓库 / 销售（订货单）单据列表页（2026-08-17）。
