...
| 6 | 认证写操作审计 | 所有认证 POST/PUT/PATCH/DELETE 记录请求元数据，关键表补 before/after | `UserOperationAuditInterceptor` + `AuditService` + DB triggers | 🟡 源码已落地，覆盖矩阵/故障演练待验收 |
...
List<String>? requiredAnyPermFor(String location) {
  ...
  if (location == '/admin/permissions') {
    return const [Perm.accountSupport, Perm.authorizationManage];
  }
  // 审计、系统设置及其它 admin 页面属于授权/高危管理。
...
`UserOperationAuditInterceptor` 对认证后的 `POST/PUT/PATCH/DELETE /api/**` 记录账号、方法、路径、
结果/状态码、耗时、IP 与 UA。请求正文不入库，避免复制密码、令牌和 PII；关键表通过触发器补
before/after，登录、导出、配置等通过显式业务事件补语义。通用 HTTP 记录不代表所有表都有
before/after，审计落库失败目前会告警但不回滚已完成响应，上线前仍需覆盖矩阵和故障演练。
...
- 导出有独立权限、每用户频率、单次行数、进程内并发（源码默认 2）、可选 AES 加密和审计。
...
- [ ] 审计：认证写请求元数据 + 关键表 before/after + 显式业务事件覆盖矩阵和存储故障演练
...
| 认证写操作审计 | 🟡 源码已实施 | 通用请求元数据已覆盖；关键表 before/after 矩阵、告警与恢复待验收 |
