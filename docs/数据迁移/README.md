# 老库数据迁移 · 总索引
> **当前共享源码候选(2026-09-01)**：迁移目录头为 V454，共 416 个迁移文件、416 个唯一版本且无重号。V454 庆典祝福聚合卡：新增 `notice_celebration_subjects` 主角快照表（存量单人卡回填），自动调度与 HR「一键全部送祝福」改为**每天每类型一张聚合卡**（入职周年各人年数逐人标签）；幂等/今日庆典/HR 徽标口径统一迁主角表，删除 V224 旧去重索引；零默认 grant，重置白名单增至 V454/416、CLEAR 220 张。空库 V1→V454 与调度器/重置 PostgreSQL 测试已过；目标库迁移、岗位 UAT、恢复与签名发布均未完成，生产 **NO-GO**，见 [迁移说明 73](73-V454庆典聚合祝福卡.md)。
>
...
> **本机开发库零基线重置（现行口径）**：`server/ops/reset_business_data.sql` 按 CLEAR/PRESERVE 双目录在单事务内清零业务数据并刷新六张物化视图（仅限可丢弃本地/测试库；范围、三闸确认与门禁见脚本头部，禁止用于公司目标库或生产）。最近一次：2026-09-01 18:32（UTC-06:00）于 V453/415 按明确指示**不创建备份**执行；清前 10 张业务表共 26 行，清后 CLEAR 219 张全部 0 行、PRESERVE 95 张保留，五张库存事实表与六张月度物化视图均 0 行，27 个账户金额/完整性异常均为 0，客户/供应商/货品/结算类别期初及货品安全库存/成本预算异常均为 0，1 个归属业务表的 owned/identity 序列回到起点且 `is_called=false`，Flyway 仍为 V453/415、失败迁移 0、禁用用户触发器 0、其它客户端连接 0；脚本契约测试 5/5。主档、人事、权限、审计、Flyway 与治理/编号高水位保留，ERP 应用保持停止；因本轮未建备份，不存在本轮恢复点。2026-08-17 至 2026-09-01 间的历次清库流水（V399→V450 共 20 余次）已随 2026-09-01 文档清理移除，需要证据时查 git 历史。
> 历史备份清理（2026-09-01，上一轮）：按明确授权删除 `server/backups` 中 22 个旧备份和 PostgreSQL 容器 `/tmp` 中 7 个旧 dump；复查工作区备份文件 0、容器临时备份文件 0。删除不可恢复，不影响当前数据库卷。
