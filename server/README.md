...
| 本轮通过项与尚未完成项 | [当前版本验证](../docs/99-项目治理/当前版本验证.md) |
...
先确认运行后端与正式迁移同版。V540 后，后台约启动5秒开始核对，随后每轮结束60秒继续，按25项游标批次处理已批准且允许自动齐套的等待段；锁冲突或失败留到下一轮，不要求员工反复手动点击。来源、颜色、数量、质量和实际入库未完整满足时仍正确等待，不直接修改工单状态或伪造领料。

停用存放仓禁止新选，但该位置的已有合法实物仍按原仓清出。自动恢复生成真实分仓 DRAW 草稿并通知对应仓库/车间办理人员，实际领料与开工继续由有权限人员确认。系统审计保留“系统自动核对备料”和固定原因，不能创建系统假员工或全局绕过登录。具体边界见 [V540](../docs/数据迁移/138-V540停用仓既有库存与自动齐套恢复.md)。

若开发者先启动新类而尚未应用 V540，自动补偿检测不到对应数据库守卫时跳过，原人工事件仍兼容旧列；应完成正式迁移后再验证恢复，不手工加列或改 WAITING。生产按同一签名包先迁移后激活。V537 历史校验值保持首次执行版本，后续检查由 V539 前滚；遇校验差异保留备份及日志，遵循 [V539说明](../docs/数据迁移/137-V539已执行迁移兼容与附件证明前滚.md)，不执行 repair。
