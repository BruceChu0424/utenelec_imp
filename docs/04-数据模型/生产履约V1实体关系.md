...
V196–V234 已包含在开发原库 `uten_imp` V244 及公司目标库 V238 以内；当前源码到 V250/231 个迁移，
...
> **迁移范围**：开发原库 `uten_imp` 实测 V244/installed_rank 225；源码和一次性隔离克隆到 V250/installed_rank 231；公司目标库保留 V238 既有只读证据。目标环境版本必须以现场 `flyway_schema_history` 为准。
