...
| SC-41 | 精确财务订货审批越权 | 指定负责人、同部门其他财务、超级管理员分别尝试审批 | 只有快照负责人且同时具备 `finance_order_approval:review` 可通过/驳回；责任设置变更不改派已在途实例；处理追加事件且幂等 | V196 与审批投影/权限契约测试；真实账号正反向 UAT 未完成 | `[GREEN][MANUAL]` |
