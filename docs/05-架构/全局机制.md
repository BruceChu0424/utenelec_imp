...
- 审批必须同时满足动作权限和实例 `assignee_user_id`。负责人配置权、超级管理员身份、已知 UUID 或本地角色都不能绕过对象校验；前端只使用服务端 `allowedActions`，缺失即 fail closed。
- `inbound_expectations` 是未来到货任务，不是库存、AP、实物或质量结论。收货超过财务批准余量时，`ARRIVAL_EXCEPTION_PENDING` 只提交异常，原收货仍为草稿，库存/AP/累计均不变。
- 超量异常由创建时冻结的精确财务负责人选择全批、自定义批准或不批；批准量只绑定本异常/收货行，仓库仍须再次审核。未批准量只生成原下单人的退回任务，原下单人不能修改财务决定。
...
- [x] 审批：V196/V201 专用订货与超量实例使用精确负责人快照、追加式事件和服务端 `allowedActions`；超管不能对象绕过
...
| 采购/委外订货财务审批与到货异常 | 🟡 V196–V202 源码候选，生产 NO-GO | 计划申请只读、任务中心分解、精确财务负责人、预计到货、超量隔离和原下单人退回已形成专用流程；目标库仍只确认到 V190，岗位/实物/IQC/发布未验收 |
