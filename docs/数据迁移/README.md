# 老库数据迁移 · 总索引
> **当前共享源码候选(2026-08-31)**：迁移目录头为 V447，共 409 个迁移文件、409 个唯一版本且无重号。V446 将采购/委外 IQC PASS 与仓库实际入库拆开；V447 在其后建立委外前置自制跨分析需求、未来供给和 exact entitlement 的精确交接。二者及关联应用必须停机单版本切换；目标库迁移、岗位 UAT、恢复与签名发布均未完成，生产 **NO-GO**。
>
...
> 也不得用于公司目标库或生产。
>
> **2026-08-31 live V443 本机开发库业务零基线重置（用户明确要求无备份）**：执行库为 V443/405，
> PostGIS 扩展表排除后共 307 张应用父表，按 `CLEAR 212` / `PRESERVE 95` 精确分类；V436/V440 六张、
> V442 八张和 V443 财审事件表均完整存在，V446 两张 IQC 入库表尚不存在。当前脚本后置兼容
> V443/405、V446/408 或 V447/409，并增加 V440、V442、V443、V446、V447 各组完整性门禁；V447 完整目录为 314 张应用父表，`CLEAR 219 / PRESERVE 95`。执行前停止
> Spring Boot/Maven 启动链，Navicat 元数据连接随后关闭；其它连接、业务 Outbox 阻断和非人事附件 owner
> 均为 0。212 张 CLEAR 表中 70 张非空、合计 549 行，库存余额/流水/预留为 12/17/17；账户金额、
> 客户/供应商期初、货品期初/安全库存/成本预算和结算类别期初已为 0。按用户明确要求本次未创建备份，
> 提交后没有本次恢复点。单事务提交后 212 张 CLEAR 表、库存、账户及全部零基线字段、六张物化视图均
> 为 0；V443/405、禁用非内部触发器和其它连接不变，账户/FQC/收款异常与完整性视图均无异常。
> 主档、人事、权限、审计、附件治理、Flyway、编号高水位和终身占用继续保留；不重建、不升级数据库。
>
> **2026-08-30 V429 本机开发库完整零基线重置(第六次执行，当前状态；业务 identity 重置)**：第五次
> 清理后外部启动链再次运行并将本机库推进至 V429/391，新增 `production_daily_report_workers`。更新
> 目录后 192 张 CLEAR 表中 60 张非空、合计 298 行；库存余额 8 行、数量 `70,505`、台账金额 `8,000.5`，
> 流水/预留各 9 行。门禁前关闭 cmd/Maven/Spring Boot 链并通知上传任务保持只读。真正写入前备份
> `server/backups/uten_imp_pre_full_reset_v429_20260830_041845_34e20895.dump` 为 5,289,870 字节，
> SHA-256 `DF365ABA276CB7629A7F30AC9D92D0D8F5A31E59762A8BD43E3CEB0049297D1D`，`pg_restore -l`
> 5,765 项；恢复库与源库的 284 张父表/34,358 行、V429/391、全部序列、审计、基础资料、人事、账户、
> 货品/BOM、新日报参与人表、库存及六张物化视图指纹一致。脚本/合同同步 V429，将新表归 CLEAR，更新为
> `CLEAR 192 / PRESERVE 92`，聚焦测试 5/5 通过。第六次清理 request ID
> `0d6517d2-c352-4d54-a79e-dd1c2f486910` 已提交：192 张 CLEAR 表、新参与人表、库存、账户、期初、
> 安全库存/成本和六张视图均为 0；业务序列 start=1、last_value=NULL。审计 4,946 行、审计 ID 4,946
> 保持不变。全部 92 张 PRESERVE 表(34,060 行)及审计、Flyway、保留序列/编号治理、基础资料、人事、
> 权限指纹与恢复库一致。临时资产已删除、主机备份保留，项目 Java/Maven、8080 和数据库应用连接均为 0。
> 该记录是当前本机最终状态。
>
> **2026-08-29 V425 本机开发库完整零基线重置(第五次执行，历史记录；业务 identity 重置)**：第四次
> 清理后后端再次运行并将本机库推进至 V425/387(源码 V426/388 仍未应用)，形成 270 行 CLEAR 数据
> (45 张表非空)、7 行库存余额(数量 `80,005`、台账金额 `21,200.5`；流水 7、预留 8)和 3 个数据库
> 应用连接。业务方要求对应业务表 ID 重新开始；核对确认 191 张 CLEAR 表仅有
> `finance_reconciliations.posting_seq` 一个拥有的数据库序列，其余业务主键主要为 UUID。后端停写后备份
> `server/backups/uten_imp_pre_full_reset_v425_20260829_204834_9dfc4966.dump` 为 4,865,653 字节，
> SHA-256 `8C497BC2808392DDDF29AC95BAE97DA6A1B537BFD54378CCA684D9907FBC05DF`，`pg_restore -l`
> 5,745 项；恢复库与源库的 283 张父表/31,469 行、V425/387、全部 public 序列、审计、基础资料、人事、
> 用户、账户、货品/BOM、库存及六张物化视图指纹一致。脚本/合同同步 V425，并强制 191 张业务表
> `TRUNCATE ... RESTART IDENTITY`。本段不据此声明当前测试或部署完成；清空审计及复用审计 ID 因没有单独明确授权被
> 安全门禁拒绝，未写当前库；脚本已恢复为保护 `audit_log/audit_log_archive`。安全范围清理 request ID
> `22de4d16-82c2-4e73-a4ec-111666484a84` 已提交：191 张 CLEAR 表、库存、账户、期初、安全库存/成本和
> 六张视图均为 0；业务序列 start=1、last_value=NULL。审计仍为 2,148 行、审计 ID 仍为 2,148。
> 全部 92 张 PRESERVE 表(31,199 行)及审计、Flyway、保留编号序列、基础资料、人事、权限指纹与恢复库
> 完全一致。临时资产已删除、主机备份保留，项目 Java/Maven、8080 和数据库应用连接均为 0。该记录是
> 当时状态。
>
> **2026-08-29 V423 本机开发库完整零基线重置(第四次执行，历史记录)**：第三次清理后，同一
...
> **2026-08-31 live V431 本机开发库业务零基线重置（不重建、不升级）**：执行库保持 V431/393，
> 当前源码虽已到未冻结 V441/403，但本次未应用 V432–V441。重置脚本改为严格双目录：live V431 的
> 287 张父表按 `CLEAR 195` / `PRESERVE 92` 执行；V440+ 六张未来业务表只有全部真实存在时才条件加入，
> 部分存在、未知表或其它 catalog 形状失败关闭。执行前 195 张 CLEAR 表合计 549 行，库存余额/流水/预留
> 为 12/17/17；账户五项金额、客户/供应商期初、货品期初/安全库存/成本预算和结算类别期初均已为 0。
> 备份 `server/backups/uten_imp_pre_full_reset_v431_20260831114641Z.dump` 大小 6,171,943 字节，SHA-256 为
> `50F1F16B2A073B3B93EC216BD296E3CA11171597F315B9D4BA6C79FAD38D2962`，完整恢复后287张实体表和
> Flyway指纹与源库一致。首次目标执行因可选策略块插入点语法错误在任何TRUNCATE/UPDATE前失败且未提交；
> 修正后先在恢复库完整执行成功，再执行目标库。最终目标库与清后恢复库287张表指纹一致，195张CLEAR、
> 库存、账户及全部零基线字段、六张物化视图均为0，92张PRESERVE保持一致。临时恢复库与容器`/tmp`
> 副本已删除，主机备份保留，ERP应用保持停止。该操作不重建数据库、不降低保留表序列或编号高水位，
> 不触碰网站、附件实体、内测或生产。
>
...
当前源码候选目录是 V426/388，但这个数字本身不能授权目标库升级。公司目标库迁移前必须依次完成：

1. **冻结同一发布 lineage**：从受保护、签名 commit 和签名 annotated tag 生成主 JAR、migration-only JAR、
   Flyway checksum manifest 与 Release 签名；三者的 388 个 `version/script/checksum/source SHA-256` exact-set
