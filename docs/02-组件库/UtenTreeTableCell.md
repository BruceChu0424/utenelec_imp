...
2. **连续参考线**：`_TreeGuidePainter` 画祖先竖线 + 分支拐角线，`ancestorContinuations` 精确控制「下边还有兄弟」的断线，`isLastChild` 收尾。**颜色按明暗对称调（2026-09-14）**：原来两种明暗都取 `outlineVariant`，白底上几乎与表格网格线同色；改为 `onSurfaceVariant` 两档——浅色 0.70（看得清层级走向又不抢名称）、深色 0.40（深底上线条本就更跳，压下去才不刺眼）。宿主给了 `foregroundColor` 时仍按它的 0.35 走；
3. **级联序号徽标**：`P1.1.2` 式，tabular 数字、按 depth%4 轮换取色（primary/secondary/tertiary/onSurfaceVariant）。**`sequence` 传空串即不渲染徽标**（2026-09-14：物料分析主表与三个分桶详情按用户口径去掉 P1 / P1.1，层级由缩进 + 连接线表达；货品 BOM 等宿主传非空仍照旧）；
...
| 叶子节点圆点 | 无子级时 8px 圆点占位（alpha 0.45，与实心圆底拉开对比），与箭头同列对齐。**`showLeafMarker: false` 关掉圆点**（2026-09-14 物料分析系表格用户口径「最下面的层级不要前面的圆点」）；占位宽度仍保留，各层级名称对齐不变，默认 true 不影响既有宿主 |
