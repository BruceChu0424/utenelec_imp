...
| 说明 | 迁移 | 本次变化 |
| --- | --- | --- |
| [169](169-V577下达车间超量与公共备货产出.md) | V577 | 下达车间可超出剩余需求：`production_material_analysis_plan_links` 新增 `public_surplus_qty`，计划行数量对账改为「归需求量 + 公共备货产出量」之和，原 `submitted_qty > 0` 放宽为两者非负且合计为正；不抬 `requested_qty`、不自动展开下层物料需求、销售订单来源顶层行仍按订单量封顶 |
