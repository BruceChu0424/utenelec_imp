...
export default async function CaseEditPage({ params }: { params: { id?: string[] } }) {
  ...
  if (id && !c) notFound();
  const zh = c ? tr<{ title: string; location?: string; content?: string }>(c.i18n, 'zh') : { title: '', location: '', content: '' };
  const en = c ? tr<{ title: string; location?: string; content?: string }>(c.i18n, 'en') : { title: '', location: '', content: '' };
  ...
  return (
    <form action={saveCase} className="max-w-2xl">
      ...
      <div className="mt-6 flex gap-3"><button className="btn-accent">保存案例</button><a href="/admin/cases" className="btn-outline">取消</a></div>
    </form>
  );
}
