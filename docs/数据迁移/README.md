...
> **当前正式目录：V720/649 (2026-09-25)**。V720 汇总下单通道来源标签并入 V719 编号口径：同料合并汇总单此前写「物料分析汇总 <日期>」，现按 `preplan_supply_actions`/订货行来源表把能精确回溯的存量标签回写成 WL 分析编号（汇总批次锚定单一分析，文档级映射无歧义；多来源无唯一归属的行保留原标签），服务端 `AggregateMaterialOrderWriteService` 下单时直接落编号（不加表）。V719 物料分析编号：`production_material_analyses` 增加 `analysis_no`（WL+YYYYMMDD+6位日流水，V279 命名空间口径），存量按上海日期逐日回填并把日流水播种进 `business_document_sequences`；经 `preplan_supply_actions`/订货行来源表把采购/委外链上能精确回溯的「计划前物料分析 <日期>」展示字符串回写成编号（多来源无唯一归属的保留原标签）；执行工作台根视图 root_label 优先显示编号（不加表，允许 NULL——预览入口是唯一生产写入方）。V718 货品「未分类」根放开为普通分类（退役 material 保护触发器、注册表 material_category_id 脱钩置空+窄守卫、改名「未分类」；客户/模具/供应商三根保护不动，不加表）。V717 基础资料五个主档（模具/颜色/单位/结算方式/仓库）补齐导出权限点 `mould:export`/`color:export`/`unit:export`/`settlement_method:export`/`warehouse:export` 并挂 `basic.*` 权限面，按 V71 同款「导出跟随查看」回填（只插权限目录与授权，不加表）。V716 播种 production_plan:create 权限码并授予生产部（补 #29 与 V714 的可达性欠账，只插目录不加表）。V712/V713/V715 完成物料汇总实际下单、共享制造的来源份额、子料转交与精确直送；V714 是并行权限登记，见 [迁移说明](241-V712至V715物料汇总来源与直送.md)。V709–V711 完成底层自制实际领料、BOM 累计学习及分层超产默认与记忆，见 [迁移说明](240-V709至V711底层自制领料与BOM学习.md)。V708 重建采购审批 display 快照函数，weight/giftQty/allowedLossPct 对齐 trim_scale 全站数值去尾随零口径(只重建函数、不加表不改存量数据, displayVersion 1→2)。V704–V707 补齐计划初始允许超产比例、提前完结/追加取消与已应用迁移恢复后的前向兼容(ADR-118)。V703 新增车间催计划记录表 `production_planning_urges`(ADR-117)：车间任务缺的料里有计划还没下单的，车间可一键催计划、计划员收到居中待办卡；计划下够单或任务结束后由核对任务办结并撤卡。新表 CLEAR、审计 NONE(queue 组)，不改任何数量与旧数据。V694-V702 为自制实际产出分账、公共库存及成本、计划进度分离与仓库实点确认(ADR-118；本地候选，不代表现场已迁移)。V693 新增仓库负责人(仓管员)关系表 `warehouse_keepers` 与三个读函数(仓库范围展开、仓库有效负责人、账号「我的仓库」)，仓库类通知只发给单据所在仓的负责人、仓库任务中心可按「我的仓库」筛选(ADR-115)；新表 PRESERVE、审计 FULL、不回填不改旧数据。V692 把 V646/V690 新增的四个热表延迟约束触发器改为按相关列起跳(ADR-106 口径, 只重建触发器)。V691 为采购委外审批展示增加提交时快照(并行财务修复)。V690 补齐同一执行段中订单生产与公共备货的独立报工、成品入库额度，保留既有销售分摊与历史数量；公司现场半量报工问题与验证见[专项记录](../99-项目治理/2026-09-24-车间半量报工后公共备货来源缺失.md)。V689 增加报销提交对照快照。V688 委外损耗独立结清(ADR-114, 原写作 V648/ADR-106, 合并时改号)：原订货量与申请占用保留，正常损耗不触发财务改量或重新下单。V680–V682 服务端会话、敏感操作再认证与系统设置单一登记(ADR-110)：新增 auth_sessions / auth_step_up_states 两张 CLEAR 表，system_settings 删元数据列，account:support 授权策略改 INDIVIDUAL_ONLY、权限目录加 high_risk 列；V670–V673、V677–V679、V683–V687 分属审计三清单(ADR-105)、权限目录单一事实源(ADR-109)、主档删除引用保护(ADR-111)与金额精确派生(ADR-112)；V648–V669 跳号(V646/V647 是并行会话已应用到开发库的版本, 整改迁移整体顺延到 V670 起)。V647 将未提交领料的自制追加直接加到原 ZX 工单，保留冻结用料规则和旧预留，只补新增缺口并记录不可变增长事件。V646 为单一子件委外增加精确库存归属交接(ADR-113)，任务中心和发料使用本产品的子件权益，新增不可变交接记录及对称恢复守卫(两张新表在 V670 审计三清单里归 NONE/ledger、清库归 CLEAR)。V645 让追加自制并进还没开工的生产计划(ADR-104): 新增 fn_material_analysis_plan_growable 等四个判定函数与一条「计划明细 qty(+封顶) = 关联行归需求 + 公共备货」的提交时对账触发器, 锚点补丁计划明细身份守卫 / 计划关联行数量同步触发器 / 生产供给来源守卫, 只在计划仍未开工时放开只增不减的改量; 不加表不加列。V644 把 V409 的「创建命令账本」升级为「日报命令账本」(审核接口要求幂等键)。
>
...
V709 新增制造底层超产默认与货品记忆；V710 新增底层自制实际领料申请和精确来源；V711 按有效实际生产与净耗量增量学习 BOM。新规则、数据范围和兼容边界见 [ADR-119](../99-决策记录-ADR/ADR-119-底层自制实际领料与BOM累计学习.md)。六张新表中业务事实四张清空、主档学习知识两张保留，该轮结束时为 640 条迁移。
...
| 说明 | 迁移 | 本次变化 |
| --- | --- | --- |
| [245](245-V720汇总下单来源标签并入编号.md) | V720 | 汇总通道「物料分析汇总 <日期>」存量标签按 psa/来源表回写成 WL 编号；服务端下单直接落编号（不加表） |
| [244](244-V719物料分析编号.md) | V719 | 物料分析编号：`analysis_no` 列（WL+日期+日流水）+存量回填+序列播种；采购/委外谱系标签按 preplan_supply_actions/来源表精确回写为编号；roots 视图标签优先编号（不加表） |
| [243](243-V718货品未分类根放开.md) | V718 | 货品未分类根放开为普通分类：退役 material 保护触发器、注册表脱钩+窄守卫、改名「未分类」（不加表） |
| [242](242-V717基础资料五主档导出权限.md) | V717 | 模具/颜色/单位/结算方式/仓库五主档补齐 `*:export` 权限点并挂 basic.* 面，「导出跟随查看」回填（只插目录与授权，不加表） |
| [241](241-V712至V715物料汇总来源与直送.md) | V712–V715 | 同料实际合单、共享制造、来源数量与直送交接；V714 并行权限登记 |
| [240](240-V709至V711底层自制领料与BOM学习.md) | V709–V711 | 制造层级超产默认与最新记忆、仓库填写实际材料及真实出库、按生产族净耗/产量增量学习 BOM、人工维护保护与反向 |
| [239](239-V704至V707计划超产比例与历史迁移恢复.md) | V704–V707 | 计划及下达初始比例、追加反向/提前完结保护；恢复 V694/V700/V701 的真实已应用原件，并前向补齐后续变更 |
| [238](238-V703车间催计划记录.md) | V703 | 新表 `production_planning_urges`(车间任务 × 物料分析, 一个任务最多一条在催)：车间一键催计划下单子层物料、计划员待办卡与徽章，计划下够单或任务结束由核对任务办结撤卡(ADR-117)；CLEAR、审计 NONE，不改任何数量 |
| [237](237-V694至V702自制实际产出与仓库实点.md) | V694–V702 | 原计划不变，实际批次拆需求/计划公共/实际超产；公共身份贯穿库存与成本，公共超产不计原需求完成；自动点收必须有实际点数证据 |
| [236](236-V693仓库负责人与仓库通知按仓分发.md) | V693 | 新表 `warehouse_keepers`(仓库 × 员工, 主仓负责人管全部子仓)与 `fn_warehouse_scope_ids` / `fn_warehouse_keeper_user_ids` / `fn_user_warehouse_scope_ids`；仓库类通知按仓分发、任务中心「我的仓库」(ADR-115)；未登记负责人时行为不变 |
| [235](235-V692热表延迟约束触发器按相关列起跳补齐.md) | V692 | V646/V690 新增的四个延迟约束触发器 UPDATE 路径加 WHEN(相关列真变了才排队), 与 ADR-106/V674 同一拆法; 校验语义不变 |
| — | V691 | 采购/委外财务审批展示快照: 提交时冻结展示用明细(提交快照必须带 items), 审批页按快照对照; 只补列、函数与守卫, 不加表 |
| — | V690 | 同一执行段内订单生产与公共备货分开计量: 无销售分配的报工/成品入库以公共备货量与已审核报工为上限(`fn_assert_execution_segment_public_surplus_capacity`) |
| [V689](V689-报销提交版本对照.md) | V689 | 报销提交版本快照与重新提交对照 |
| [234](234-V688委外损耗独立履约结清.md) | V688 | 订货量不变，独立损耗结清；同步剩余供给、任务、关闭守卫与成本口径(ADR-114; 原写作 V648) |
| [231](231-V680至V682服务端会话再认证与设置登记.md) | V680–V682 | 服务端会话、敏感操作再认证与系统设置单一登记(ADR-110)：V680 新表 `auth_sessions`(登录会话：吊销/空闲/绝对期限的唯一权威，含一次性再认证凭证哈希)与 `auth_step_up_states`(再认证连续失败计数)，存量刷新令牌作废，两表登记清空清单 CLEAR；V681 `system_settings` 删六个元数据列(改由 `SystemSettingKey` 枚举登记)、补七个登记行、越界令牌期限收口；V682 `account:support` 改个人点名授权(删部门级授予与委派，`grant_policy` 置 `INDIVIDUAL_ONLY`，由 V677 通用授权策略守卫拦截部门级写入)、权限目录加 `high_risk` 高危标记、存量无过期时间的非超管临时密码置过期 |
| [233](233-V676索引卫生冗余删除与热点领头索引.md) | V676 | 索引卫生(ADR-106)：删 96 个被前缀覆盖或与同列唯一约束重复的冗余索引(白名单保留两条并写明原因)，补 44 个(含 V647 增长事件计划明细列)估值图/需求/预留/计划明细/总账凭证引用方领头索引与 31 个货品数量来源列完整领头索引；`SchemaIndexHygieneContractTest` 守住三条规则 |
| [232](232-V675货品单位改为按需检查数量引用.md) | V675 | 货品基本单位「已被数量引用则不能改」改为改单位时按需检查(ADR-106)：删 52 张表上 104 个语句级触发器、`fn_lock_goods_quantity_unit_from_references` 与 `goods.quantity_unit_locked`；新增 `fn_goods_quantity_unit_in_use`(沿用 V498 来源清单)，改单位守卫先 `FOR UPDATE` 等在途引用再判定；引用被硬删除或草稿明细改指别的货品后可改单位(推翻 V498 终身锁定，待业务确认) |
| [231](231-V674热表触发器按相关列起跳.md) | V674 | 热表约束/守卫触发器只在相关列真变了时起跳(ADR-106)：94 个触发器 UPDATE 路径加 WHEN(INSERT/UPDATE 共用的拆 `_upd`/`_del`)，删 3 条被覆盖校验完全包含的采购溯源触发器；单号取号只挂 INSERT、UPDATE 改列级不可变守卫；收付款类别引用方改共享咨询锁；只去掉无关列变化的排队，逐行逐事件重复校验(按键去重)留给下一波 |
...
| [226](226-V640未订货申请明细就地追加.md) | V640 | 新增 `fn_preplan_external_item_unordered` / `fn_preplan_supply_action_growable` / `fn_is_preplan_supply_line_growth` 三个判定函数，并锚点补丁 V460 的供给行动与分摊行身份守卫、V472 的公共量身份守卫、申请明细生产联动守卫共四处：只对「明细仍未订货」的供给行动放开只增不减的改量 |
| [225](225-V638委外回厂守卫计入前置自制出仓.md) | V638 | 只锚点补丁 `fn_assert_subcontract_target_outbound_receipt`：「已审出仓量 / 已消费量」合计的 JOIN 流向集合补上 `PREPARED_OUTBOUND`(前置自制订货超量后同一明细 PREPARED + DIRECT 混合出仓，回厂 33 不再因 `v_issued` 只算到 DIRECT 份 3 而被 `subcontract_target_outbound_first_guard` 拒绝)；激活条件(明细含 DIRECT/MAKE_THEN/COMPONENT 行)与纯 PREPARED 明细口径不变；同批把草稿态(`status = 0`)采购/委外订货行里客户端浮点乘积(3×0.10 = 0.30000000000000004)一次归一为数量×单价(×表头汇率)并重算表头合计；不加表、不加列、不动触发器 |
| [224](224-V634委外前置自制超量订货批准.md) | V634 | 只 `CREATE OR REPLACE` `fn_assert_subcontract_preparation_source_before_v535`(V535 包装函数仍调用)：`PREPARED_OUTBOUND` 行的数量上限改看前置自制台账 `preplan_subcontract_make_tasks.required_qty`(归需求量 + V589 公共备货产出)，不再看分析 `SUBCONTRACT_MAKE` 行 `requested_qty`；批次 `notify_qty >= planned_qty`、货品/颜色/单位一致性与 `MAKE_THEN_OUTBOUND` 分支原样保留；不加表、不加列、不改行、不动触发器 |
| [223](223-V636委外允许损耗与回厂短交案件.md) | V636 | `goods.subcontract_allowed_loss_pct`(允许损耗默认值/记忆)与 `subcontract_order_items.allowed_loss_pct`(本行冻结值)两列, 0 到 100 CHECK；新表 `subcontract_short_delivery_cases`(一行明细同一时刻最多一个开放案件的部分唯一索引, 状态/判定/结案成对 CHECK, 审计触发器)与 `subcontract_short_delivery_case_events`(追加式守卫 + 审计触发器)；权限 `subcontract_short_delivery:decide`(面 subcontract.order, 默认授予持 submit_finance 的部门)；视图 `v_subcontract_supplier_goods_loss_summary` / `v_subcontract_supplier_loss_summary`；运行期清库策略补两张 CLEAR；不回填历史、不改保留列数据；V633-V635 跳号 |
| [222](222-V632出货放行记账汇率与标准收付款方式.md) | V632 | `sales_shipment_finance_release_events.exchange_rate/exchange_rate_source`(放行冻结的记账汇率与来源：CURRENCY_MASTER/FINANCE_MANUAL，三条 CHECK 成对)；`sales_shipments.exchange_rate` 语义改为「财务放行时冻结的记账汇率」(注释)；`finance_payment_methods` 在没有任何已确认方式时播种 4 条标准方式(固定 UUID、ON CONFLICT 幂等)；不加表、不回填历史事件、不改保留列数据 |
| [221](221-V631出货明细按行记实际发出仓.md) | V631 | `sales_shipment_items.warehouse_id`(行实际发出仓，历史已出库行回填表头仓)+部分索引；`sales_shipment_warehouse_events.line_warehouses` jsonb；`fn_guard_sales_shipment_picking_evidence` 锚点补丁校验逐行发出仓；不加表、不改保留列数据；扣库存/消预留/红冲/退货来源按行仓走，财审放行前校验币种汇率 |
