...
> 通用调研框架见 原 06-老系统融合/01-老系统现状调研（已删除，现行口径见 ADR-017 与 数据迁移/）；字段级映射见各模块文档（如 [02-货品分类-老库溯源.md](02-货品分类-老库溯源.md)）。
>
...
> **计数口径校准（2026-08-01）**：上表 `B_BomItem=218,822` 保留 2026-07-24 原始调研快照；
> 实际导出并被迁移脚本解析的 `goods_bom.csv` 为 **218,820** 行，后续 BOM 守恒与 reject 对账统一使用
> `218,820 = 198,022 有效源行 + 20,798 reject`。两行差异尚未取得可证明的源端筛选解释，不静默改写历史快照。
| `M_Out` | 44,534 | 财务付款单（?） |
| `M_In` | 42,489 | 财务收款单（?） |
...
- **对策（待定，详见 02-双写与同步方案.md（原 06-老系统融合 文档已删除，现行口径见 ADR-017 与 数据迁移/））**：
...
| `B_Client` 客户 | 客户资料（`client:view` + owner 范围） | ✅ V36/V86/V89 已落库并接真实页面；迁移对账见专项文档 |
| `B_Provider` 供应商 | 供应商资料 | ✅ V38 已落库并接真实页面 |
| `B_Goods` 物料 / `StockGoods` 库存 | 货品主档 / 库存查询 | ✅ 真实后端；迁移与数量对账仍须按生产清单验收 |
| `S_`/`P_`/`O_` 单据 | 采购、销售、委外、仓库等单据 | ✅ 已分模块落库；全模块结构化对账仍是上线阻断项 |
| `W_Laborage` 工资 | 工资条 | 🟡 新库 V133 领域已建；老库字段探源、映射、导入、reject 和对账尚未实现 |
| `C_*` 质检 | 检测记录（实验室） | 🔲 占位 |
| `F_*` 生产计划/成本 | 生产计划、日报、成本快照与报表 | ✅ 已落库；旧“流水线/产量”三页仅为未实施产品草案 |
| `Sys_Operator`/`Sys_Purview` | 账号 / 权限管理 | ✅ 已实现（权限模型已重构，与老系统差异大） |
| `M_*` 财务收付款 | 钱流单据、报表与账户资料 | ✅ 已落库并接真实页面；总账迁移仍有专项阻断项 |
| `B_Bom*` 物料清单 | BOM/生产成本展开 | 🟡 已有历史成本快照与反查；源 BOM 的完整迁移边界见专项文档 |
| `F_Arrange` 排产 / `F_*Cost` 成本核算 | 生产计划与成本核算 | ✅ 已落库；仍须完成生产迁移对账 |
...
6. **字段口径**：每个模块的字段级映射见对应模块文档（如 [02-货品分类-老库溯源.md](02-货品分类-老库溯源.md)）。
