...
> 本档是 **人事功能域（Phase 2）** 的总纲，串联 8 个管理页面，并登记全员安全入口“我的部门”。
...
| 9 | 我的部门（全员安全入口） | `/profile/me/department` | [我的页.md §四·甲](我的页.md) | 本人分支树 + 安全花名册；不授部门/员工管理权限 |
...
- 全局 `/admin/permissions` 只对超级管理员且具备 `authorization:manage` 的账号开放，用于中央部门配置、个人 grant/revoke 和数据范围。
