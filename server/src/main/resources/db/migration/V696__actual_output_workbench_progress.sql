-- Actual production is independent of the original plan commitment (ADR-116).
-- A started, open task retains its reporting entry after planned quantity is met.
-- Surplus is visible as physical output, but never advances planned completion.
-- Plan iqty uses the plan's frozen quantity unit. Keep legacy receipt totals,
-- subtracting only receipts proven to be actual surplus by their report source.
CREATE OR REPLACE FUNCTION fn_plan_original_inbound_qty(p_plan_item_id UUID)
RETURNS NUMERIC LANGUAGE sql STABLE AS $$
    SELECT COALESCE((
        SELECT COALESCE(plan_item.iqty, 0) - COALESCE((
            SELECT SUM(item.qty)
            ...
            WHERE item.upstream_item_id = plan_item.id AND NOT item.is_deleted
              AND document.doc_type = 'FINISHED_IN' AND document.status = 1
              AND NOT document.is_deleted AND source.is_actual_surplus), 0)
        FROM production_plan_items plan_item WHERE plan_item.id = p_plan_item_id), 0);
$$;

CREATE OR REPLACE FUNCTION fn_production_actual_output_reportable(p_segment_id UUID)
RETURNS BOOLEAN LANGUAGE sql STABLE AS $$
    SELECT EXISTS (
        ...
        WHERE segment.id = p_segment_id AND NOT segment.is_deleted
...
SELECT segment.id AS segment_id,
       ...
       fn_production_actual_output_reportable(segment.id) AS reportable,
       (GREATEST(COALESCE(orders.source_count, 0)
          + CASE WHEN segment.planned_qty > COALESCE(orders.allocated_qty, 0) THEN 1 ELSE 0 END, 1)
          + COALESCE(recovery.source_count, 0))::integer
           ...
           WHEN segment.status <> 'IN_PROGRESS'
                  AND NOT (segment.material_requirement_mode = 'ZERO_MATERIAL'
                     OR (material.demand_count > 0
                         AND material.fulfilled_count = material.demand_count))
             THEN '仓库尚未完成全部备料出库'
           WHEN plan.status <> 1 OR plan.is_closed OR plan.is_canceled OR plan.is_stopped
                  OR package.status <> 'CONFIRMED'
             THEN '生产计划或计划包当前不可执行'
           WHEN NOT fn_execution_material_custody_valid(segment.id)
             THEN '实领或直送物料与当前车间不一致，请核对原来源'
           ELSE NULL
       ...
       COALESCE(progress.actual_surplus_reported_qty, 0)::numeric AS actual_surplus_reported_qty,
       COALESCE(finished.actual_surplus_inbound_qty, 0)::numeric AS actual_surplus_inbound_qty,
       GREATEST(COALESCE(finished.inbound_qty, 0) - COALESCE(finished.actual_surplus_inbound_qty, 0), 0)::numeric AS planned_inbound_qty
...
LEFT JOIN LATERAL (
    SELECT (SELECT COUNT(DISTINCT sales_order.id)
           ...
           (SELECT COALESCE(SUM(allocation.allocated_qty), 0)
            FROM execution_segment_sales_allocations allocation
            WHERE allocation.execution_segment_id = segment.id) AS allocated_qty,
...
LEFT JOIN LATERAL (
    SELECT SUM(item.qty) FILTER (WHERE NOT item.is_actual_surplus AND item.fqc_recovery_authorization_id IS NULL) AS gross_reported_qty,
           SUM(item.qty)
             - COALESCE(SUM((
           ...
           COALESCE(SUM(item.qty - COALESCE((SELECT SUM(adjustment.adjusted_qty)
               FROM production_fqc_contribution_adjustments adjustment
               WHERE adjustment.source_report_item_id = item.id), 0))
               FILTER (WHERE item.is_actual_surplus), 0) AS actual_surplus_reported_qty
...
LEFT JOIN LATERAL (
    SELECT COALESCE(SUM(item.qty) FILTER (WHERE document.status = 0), 0)
           ...
           COALESCE(SUM(item.qty) FILTER (WHERE document.status = 1), 0)
               AS inbound_qty,
           COALESCE(SUM(item.qty) FILTER (WHERE document.status = 1 AND source.is_actual_surplus), 0)
