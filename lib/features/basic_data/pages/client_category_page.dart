...
class _DetailPaneState extends State<_DetailPane> {
  ...
  // 该分类下的客户分页（仅叶子分类加载；分组节点提示用户选具体子分类）。
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      // 始终加载客户列表——后端 list 已改为子树汇总，分组节点（如外贸钟）会返回
      // 其自身 + 所有子分类下的全部客户，不再因有子分类而隐藏直接挂的客户。
      await _loadClients(1);
    } on ApiException catch (e) {
      if (!mounted) return;
      setState(() {
        _error = e.message;
  ...
  static const _clientFields = [
    ...
    MasterFieldDef(key: 'fullName', label: '全称'),
    MasterFieldDef(key: 'clientRank', label: '等级'),
    MasterFieldDef(key: 'region', label: '区域'),
    MasterFieldDef(key: 'placeId', label: '地区'),
    ...
    MasterFieldDef(key: 'credit', label: '信用额度', type: MasterFieldType.money),
    MasterFieldDef(key: 'initTotal', label: '期初应收', type: MasterFieldType.money),
  ...
  Future<void> _showClientDetail(String id) async {
    ...
    await _openClientDialog(d);
    if (mounted) _detailLoading = false;
  }

  Future<void> _openClientDialog(ClientDetail d) {
    return showDialog<void>(
      context: context,
      builder: (ctx) => AlertDialog(
        title: Text(d.name?.isNotEmpty == true ? d.name! : (d.code ?? '客户详情')),
        content: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              _row('税号', d.taxId),
              _row('信用额度', d.credit?.toStringAsFixed(2)),
              _row('期初应收', d.initTotal?.toStringAsFixed(2)),
        ...
        if (_canEditMaster)
          Padding(
            ...
            child: FilledButton.icon(
              ...
              label: const Text('添加客户'), // TODO(l10n): 补 arb
            ),
          ),
  ...
  Widget _buildClientSection(ThemeData theme) {
    ...
    return Column(
      ...
      children: [
        UtenSectionHeader(
          title: '客户 ($total)', // TODO(l10n): 补 arb
          icon: Icons.people_outline,
        ),
        if (detail.childCount > 0)
          Padding(
            ...
            child: Text(
              '已汇总 ${detail.childCount} 个子分类下的全部客户', // TODO(l10n): 补 arb
              style: theme.textTheme.bodySmall?.copyWith(
                color: theme.colorScheme.onSurfaceVariant,
              ),
            ),
        ...
        _buildClientBody(theme),
        if (_clientPage != null && _clientPage!.totalPages > 1)
          _buildClientPager(theme),
      ],
    );
  }

  Widget _buildClientBody(ThemeData theme) {
    if (_clientLoading && _clientPage == null) {
      return const Center(
        child: Padding(
    ...
    if (_clientError != null) {
      return UtenEmpty.error(
        message: _clientError,
        actionLabel: '重试', // TODO(l10n): 补 arb
        onAction: () => _loadClients(_clientPageNum),
      );
    }
    final items = _clientPage?.items ?? const <ClientListItem>[];
    if (items.isEmpty) {
      return const UtenEmpty(
        icon: Icons.people_outline,
        message: '该分类暂无客户', // TODO(l10n): 补 arb
      );
    }
    return Column(
      children: [
  ...
  String _clientSubtitle(ClientListItem m) {
    final parts = <String>[
      if (m.code != null && m.code!.isNotEmpty) m.code!,
      if (m.region != null && m.region!.isNotEmpty) m.region!,
  ...
  Widget _buildClientPager(ThemeData theme) {
    final page = _clientPage!;
    final canPrev = page.page > 1 && !_clientLoading;
    final canNext = page.page < page.totalPages && !_clientLoading;
    return Padding(
      ...
      child: Row(
        ...
        children: [
          TextButton.icon(
            onPressed: canPrev ? () => _loadClients(page.page - 1) : null,
          ...
          TextButton.icon(
            onPressed: canNext ? () => _loadClients(page.page + 1) : null,
