...
保留卡片 + `HrTaskTile`（移动端手感，与其他任务中心一致）。

| 列 key | 列名 | 说明 |
|---|---|---|
| `code` / `name` | 工号 / 姓名 | |
| `deptName` / `positionName` | 部门 / 岗位 | 部门列**带表头筛选** |
| `date` | 转正预计日 \| 生日 \| 周年日 \| 入职日 | 列名按类型切换 |
| `days` | 距转正(天) \| 距生日(天) \| 入职年数 \| 已入职(天) | **逾期取负数并标红**（服务端 `days` 存的是逾期天数，今日行取 0） |
| `window` | 区间 | 逾期 / 今日 / 即将，**带表头筛选**（与服务端分组同源） |
| `claim` | 认领 | 认领人 + 租约到期（`claimedByName` + `claimLeaseUntil`），**带表头筛选**（未认领/我处理中/他人处理中） |
| `blessed` | 已祝福 | 仅生日/周年类，**带表头筛选** |

- **筛选桶客户端聚合**：summary 是服务端一次性下发的全量结果、页面无分页，
  故部门/区间/认领/已祝福四列在客户端聚合与裁剪，不存在「命中行落在其他页」问题。
- **行菜单（右键/长按）**：查看档案 / 登记转正（转正类 + `employee:confirm`，被他人认领时不出）/
  送祝福（今日庆典 + 未祝福 + `notice:publish`）/ 认领 · 释放 · 接管（软认领，§五）。
- **多选批量**（`idOf=employeeId`，「已选 N 项」只由 `UtenSelectionSummaryPill` 呈现）：

  | 类型 | 批量动作 | 口径与门控 |
  |---|---|---|
  | 转正办理 | 批量登记转正(N) | 一次选日期（默认今天、不可选未来），逐人 `POST /employees/{id}/confirm`；409（已是正式员工）自动回退 `PUT /{id}` 补登 `confirmedAt`；**被他人认领的行前端跳过并计入失败**（后端 confirm 无认领守卫）；门控 `employee:confirm` + `employee:edit` |
  | 生日关怀 / 入职周年 | 批量送祝福(N) | `POST /notices/celebration/batch`，只发「选中 ∩ 今日 ∩ 本年未祝福」；服务端按 (员工,类型,当年) 再去重；门控 `notice:publish` |
  | 新近入职 | 无 | 没有可批量的状态动作，故不开多选 |
...
> 2026-09-01 升级（V454 聚合祝福卡）：「一键全部」不再**每人一张卡**——当天同类型的全部未祝福主角合并为**一张聚合祝福卡**（如 5 人生日 → 一张卡，标题「祝 张三、李四等 5 人 生日快乐！」、正文点名全员）；入职周年各人年数不同，逐人标签存 `notice_celebration_subjects`（张三 入职5周年、李四 入职10周年），卡上主角徽章逐人展示。批量接口返回 `published`(覆盖人数)/`skipped`/`notices`(卡数 0|1)；幂等去重按 (员工,类型,当年) 在主角表判定，与自动调度同口径；每行「送祝福」仍为单人定制卡（走发布页）。详见 [数据迁移说明 73](../数据迁移/73-V454庆典聚合祝福卡.md)。
...
**最后更新**：2026-09-10（子页改统一表格 + 表头筛选 + 批量登记转正/批量送祝福）· 2026-09-01 · **状态**：工作台/子页/软认领/快捷转正已接入源码并通过后端编译；
