...
| 员工报销 | `/expense/*` | **Dio + V133 + 真实 API + V138 分页索引源码** | 独立 claim/items、服务端分页与事务化付款会计交接已接；不能与 `/api/finance/expenses` 一般费用单混为一谈；附件、审批口径、容量和生产 E2E 待验收 |
...
| 销售 | `/sales/*` | 真实 | 5 单据 + 报表 |
| 委外 | `/subcontract/*` | 真实 + V196–V200 候选 | 计划下达申请只读；任务中心分解订货并提交精确财务负责人 |
| 仓库 | `/warehouse/*` | 真实 + V196/V201 候选 | 8 单据 + 14 报表；预计到货、超量异常与批准后再审任务 |
| 库存查询 | `/stock/*` | 真实 | 余额 + 流水 |
| 生产 | `/production/*` | 真实 | 计划/成本/日报 + 报表 |
| 钱流与资产子账 | `/finance/*` | 真实 + 专用候选流程/资产安全门禁 | 5 单据 + 应收应付 + 对帐 + 22 报表；V196/V201 增精确负责人订货/超量任务和负责人设置；`/finance/assets` 为 V183 专业子账，完整生命周期 NO-GO |
| 个人信息自助 / HR 审核 | `/profile/*`、`/hr/profile-changes/*` | 真实 | 字段策略（直改/需审核/HR 专属只读） |
| 系统管理 | `/admin/*` | 真实 | 账号支持/授权管理、审计中心、系统设置；逐路径权限，审计核查不等于超管（ADR-013） |

### 5.1 计划需求到仓库的跨模块候选链（V196–V202）
...
flowchart LR
    ...
    Isolated --> Decision["精确财务负责人全批/自定义/不批"]
    Decision --> WarehouseReview["批准量由仓库再次审核"]
    Decision --> OwnerReturn["未批准量只交原下单人退回"]
```

通知只是提醒，任务和动作以服务端投影、对象负责人快照及 `allowedActions` 为准；超级管理员不能绕过实例负责人。V202 对全部 `public` 业务表做审计触发器 sweep，并非只补到货异常表。公司目标库仍只确认到 V190，V196–V202 尚未部署、真实岗位/实物 UAT 或完成 IQC，生产 **NO-GO**。
...
**最后更新**：2026-08-01（登记 V183 资产与待摊专业子账、CQRS 工作台、不可变月度批次、
默认安全门禁与完整生命周期 NO-GO；保留工资/报销、访客、授权和设备验收边界）
