...
不存在独立的 `UtenSelect`、`UtenDropdown`、`UtenDatePicker` 或 `UtenSwitch`。对应场景分别使用 `UtenDropdownField`、`UtenDateField`；开关使用 Material `Switch` 并遵循主题。表单消息分两通道（2026-09-04 起）：**静态字段说明**统一走 `fieldLabel(info:)` 标签旁 ⓘ 悬停提示（`required_field_decoration.dart`，与 `UtenEditableGrid.headerInfo` 同视觉），不得常驻输入框下方；**实时状态**（校验错误/预填提醒）走 `UtenFieldMessage`。`lib/` 禁止裸 `helperText/errorText`，直接 `TextFormField` 必须接 `utenTextFieldErrorBuilder`；例外必须按独立文档写明原因并由源码契约登记。
