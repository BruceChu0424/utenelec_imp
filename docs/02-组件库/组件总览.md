...
>
> 使用约束见[组件库使用规范](../00-项目准则/09-组件库使用规范.md)。最后核对：2026-07-30。
...
| [`uten_button.dart`](../../lib/components/buttons/uten_button.dart) | `UtenButton`，primary/secondary/tonal/ghost/danger，三种尺寸，loading/disabled | 常规动作 |
...
| [`uten_export_button.dart`](../../lib/components/buttons/uten_export_button.dart) | `UtenExportButton` | 加密表格导出 |
...
| [`uten_search_bar.dart`](../../lib/components/inputs/uten_search_bar.dart) | `UtenSearchBar` | 带清空能力的搜索 |
...
新代码统一使用 [`UtenNotify`](../../lib/core/ui/uten_notify.dart)；全局排队、去重和展示由 [`app_notification.dart`](../../lib/core/ui/app_notification.dart) 负责。通用错误反馈规则见 [`action_feedback.dart`](../../lib/core/ui/action_feedback.dart)。
...
应用外壳位于 `lib/features/shell/`，没有独立 `UtenScaffold`。公共双栏 API 是 `UtenListTwoPane`，没有 `UtenMasterDetail`。
...
| 基础资料模块 | [`uten_category_tree_view.dart`](../../lib/features/basic_data/widgets/uten_category_tree_view.dart) | 分类树 |
| 基础资料模块 | [`uten_goods_picker.dart`](../../lib/features/basic_data/widgets/uten_goods_picker.dart) | `showUtenGoodsPicker` |
...
`MasterDataTableView` 和货品选择器虽然被多个业务调用，仍依赖基础资料模型，因此暂留基础资料模块；不得下沉到纯基础组件层。
