...
「我的部门」（我的页 `/profile/me/department`，2026-08-03 由工作台移植）为部门负责人（`department.manager_id`，数据驱动判定）开放有限的成员权限转授能力，**不要求 `authorization:manage` 或超管身份**。防越权升级的边界如下：
...
- **权限校验数据驱动**：`GET /api/department-staff-permissions/managed` 与 `PUT /api/department-staff-permissions/employees/{id}/overrides/{code}` 不挂 `@PreAuthorize`，逐请求复查 `department.manager_id` = 当前用户；服务端按 `MyDepartmentService.myBranchRoot` 隔离分支。
