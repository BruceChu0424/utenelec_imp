...
- refresh 的保存/清理使用完整快照 CAS。会话记录按标签页隔离（ADR-061），跨标签页通知已移除；
  profile 刷新事件携带三个代次字段，
...
- 退出先同步把 `state` 切为未登录并激活 fence（Web 上栅栏随标签页隔离）；随后在 token record 锁内读取最新记录，通过
...
- **「分类树 + 右栏详情」页**（基础资料四个分类主档页、部门管理页）还有第三层刷新问题（2026-08-17 补齐）：`_load/shellReload` 只换左树，右栏详情面板以选中 id 为身份、id 没变就不重拉，**编辑分类/部门保存后右栏停在旧名称/旧前缀**。范式：分类页重写 `CategoryPageShell.shellAfterCategorySaved` 自增 `_detailEpoch` 并以 `ValueKey('dp-$id-$epoch')` 重挂右栏（四页全部接入，回归测试 `test/features/basic_data/pages/client_category_page_refresh_test.dart`）；部门页由 `DepartmentOverviewPane.didUpdateWidget` 在树节点对象被整体替换（`!identical`）时重载（回归测试 `test/features/department/pages/department_page_edit_refresh_test.dart`）。**后续新增同类「左树+右栏」页必须接同一钩子。**
...
### 4.6.1 会话级快照与徽章汇总(2026-09-23, ADR-108)

| 数据 | Provider | 来源 | 生命周期 |
| --- | --- | --- | --- |
| 已登录会话身份 | `authenticatedScopeProvider` | `sessionProvider` 派生 | 未登录/失效为 null; 换号/代操作/清库重登变新值 |
| 全站徽章 + 通知未读 | `badgeSummaryProvider` | `GET /api/workbench/badges` | watch 上一行; 60s 轮询、隐藏暂停、单飞; 页面只读入口/容器/事实数, 不做加法 |
| 可委派页面 / 单据写能力 / 偏好整表 | `sessionSnapshotProvider` | `GET /api/auth/me` 的 `session` | watch 会话身份; 权限点变化或权限设置保存后重取; 偏好写成功后就地更新(同一加载批次, 偏好 notifier 不回灌); 取数失败(含服务端没带快照)按 5/15/45s 退避重取, 断网恢复时立即重取 |
| 主档小表字典 | `masterDictionaryRepositoryProvider` | 各 `/master/*/dict` | 绑定 `masterDataSessionKeyProvider`; 按端点单份 + 单飞; 本端写后按路径作废 |
| 通知未读索引 | `noticeUnreadIndexProvider` | `GET /api/notices/unread-index` | 摘要(随徽章汇总)对不上才重拉 |
| 页面可见性 | `appVisibilityProvider` | AppLifecycleState | hidden/paused 为 false, 所有后台轮询据此暂停 |

- **禁止**再为某个徽章单独写计数 provider + 定时器; 需要新的红/黄数 = 服务端徽章目录加一行入口(或来源),
  前端 `BadgeEntry` 加同名映射; 页内分段细数读 `badgeFactProvider` / `badgeFactOrNullProvider`(来源无权或
  未到为 null, 不渲染数字)。
- 写操作成功后: 列表类调 `bumpListRefresh(ref, key)`(精准刷新 + 写修订号 + 徽章汇总); 仅需刷新徽章时调
  `refreshBadges(ref)`(单飞合并, 多处同时调用只发一个请求)。

### 4.7 跨部门任务计数与动作失效（V196–V201 引入）

> 2026-09-23 起(ADR-108)本节的「各计数 provider」均已合并进徽章汇总, 下文保留为历史口径说明。

采购/委外未分解任务、财务订货审批、仓库预计到货/超量异常和原下单人退回任务都必须使用服务端计数端点，不能在客户端从全量列表、部门或角色推导：

- 财务订货审批为共享队列，对持 `finance_order_approval:view` 的入口用户可见（动作仅合格审核人可执行）；合格审核人只在精确 PENDING case 上获得该订单详情的临时读取，不能由队列状态推导列表/`*:view:all`。原下单人退回计数按 owner 且按采购/委外 `orderType` 过滤；无权限必须看不到对象。
...
这些 Provider/端点由 V196–V201 引入，已包含在开发原库 `uten_imp` V244 以内；公司目标库仍以 V238
既有只读证据为准。生产验收前仍不得把迁移
存在或测试计数写成真实岗位/实物上线证据。

### 4.8 生产物料分析的本地选择与服务端事实

- 行复选、表头全选/半选、选中高亮、BOM 产品/分支折叠和计划向导当前页都属于页面草稿状态；它们不能先写业务表，也不能把“已勾选”显示成已提交、已采购或已生成计划。只有当前可执行节点显示真实复选框，路线未确认、MAKE 下层未齐和只读节点只显示等高门槛状态。
- 路线确认和供给下达提交精确 material/action 标识、分析 CAS 和幂等键；成功后必须用服务端返回的 analysis/action/notified targets 重建页面。任务按稳定身份每 500 条分块，单请求原子；多个分块逐批提交，不能在客户端伪装成全批回滚。采购/委外申请已形成不等于商业订货已形成。
- 计划向导只处理 MAKE/root，逐张维护 workshop/负责人/日期/数量，班组可选；普通预览、拖动和翻页不创建计划，也不学习默认车间。最终 generate 必须携带 analysis version、分析/预览指纹和每张数量；冲突后丢弃旧预览并重读。只读“生产备料计划汇总单”只打印/导出，不承担 generate。
- `ready_start/ready_finish/ready_ship` 是服务端阶段齐套投影，正式可下达只认 ready_finish；客户端不得根据库存、在途或包装规则自行相除重算。IQC 合格入库/红冲后由服务端事务内刷新命中分析，客户端只在页面恢复/刷新时重读。
- 计划、通知或财务决定成功前不得乐观减少待办；发生超时或响应失败时按幂等键/对象详情查询权威结果。财务批准/驳回与响应详情同事务，若响应组装失败服务端会回滚，客户端不能假定决定已提交。
...
final apiClientProvider = Provider<ApiClient>((ref) {
  ...
  dio.interceptors.add(AuthInterceptor(storage: storage, baseUrl: baseUrl));
...
`apiBaseUrlProvider` 只在构建期可信的本地/云端之间选择：原生 auto 本地健康优先，Web Release
固定同源 `/api`。服务器模式变化会重建 Dio；该选择不是 `users.remote_access` 授权，也不允许
Release 从偏好读取任意 host。
...
- [网络层与拦截器.md](网络层与拦截器.md)
- [全局机制.md](全局机制.md)（sessionProvider 与权限合成）
- [ADR-002 选 Riverpod](../99-决策记录-ADR/ADR-002-状态管理选Riverpod.md)

---

**最后更新**：2026-08-11（补统一 BOM 门槛、多选 500 分块、MAKE/root 向导与只读汇总单的状态边界）/ 2026-08-10（补生产物料分析本地多选、服务端 action/version/ready、IQC 刷新事实边界，以及订货审批响应同事务）/ 2026-08-03（§4.6 补「返回即刷新」pageResume 交叉引用；此前：补齐原子 token record、CAS/latest-intent、logout fence、加密 refresh 撤销队列与恢复竞态；保留 §4.6 列表页刷新规范）
