...
├──────────────────────────────────────────────┤
│ 物料编码 名称    类型  仓库  余量   状态        │
│ M001    钢材A   原料 1号库 5.2吨  ✅充足        │  ← MasterDataTableView
...
## 四、涉及的 Uten 组件
当前真实页使用 `UtenAppBar`、`UtenContentContainer.wide`、`UtenListTwoPane`、
`MasterDataTableView` 和主题化筛选控件。库存由后端分页/筛选，不再维护一套不存在的
`UtenDataTable` 桌面与卡片手机双实现。
