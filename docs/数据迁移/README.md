...
> **当前正式目录：V666/611 (2026-09-23)**。V658–V660 服务端会话、敏感操作再认证与系统设置单一登记(ADR-110)：新增 auth_sessions / auth_step_up_states 两张 CLEAR 表，system_settings 删元数据列，account:support 授权策略改 INDIVIDUAL_ONLY、权限目录加 high_risk 列；V646–V649、V655–V657、V661–V666 分属审计三清单(ADR-105)、权限目录单一事实源(ADR-109)、主档删除引用保护(ADR-111)与金额精确派生(ADR-112)；V650–V654、V663 跳号。V645 让追加自制并进还没开工的生产计划(ADR-104): 新增 fn_material_analysis_plan_growable 等四个判定函数与一条「计划明细 qty(+封顶) = 关联行归需求 + 公共备货」的提交时对账触发器, 锚点补丁计划明细身份守卫 / 计划关联行数量同步触发器 / 生产供给来源守卫, 只在计划仍未开工时放开只增不减的改量; 不加表不加列。V644 把 V409 的「创建命令账本」升级为「日报命令账本」(审核接口要求幂等键)。
>
...
| 说明 | 迁移 | 本次变化 |
| --- | --- | --- |
| [231](231-V658至V660服务端会话再认证与设置登记.md) | V658–V660 | 服务端会话、敏感操作再认证与系统设置单一登记(ADR-110)：V658 新表 `auth_sessions`(登录会话：吊销/空闲/绝对期限的唯一权威，含一次性再认证凭证哈希)与 `auth_step_up_states`(再认证连续失败计数)，存量刷新令牌作废，两表登记清空清单 CLEAR；V659 `system_settings` 删六个元数据列(改由 `SystemSettingKey` 枚举登记)、补七个登记行、越界令牌期限收口；V660 `account:support` 改个人点名授权(删部门级授予与委派，`grant_policy` 置 `INDIVIDUAL_ONLY`，由 V655 通用授权策略守卫拦截部门级写入)、权限目录加 `high_risk` 高危标记、存量无过期时间的非超管临时密码置过期 |
