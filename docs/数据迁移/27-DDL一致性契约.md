...
> **当前目录边界（2026-08-14）**：共享工作树最高 V289，共 270 个迁移文件、270 个唯一版本且无重复；V279 是本文所述全局业务标识注册，V280–V287 是其后并发加入的候选，V288/V289 提供生产物料分析借用结构、终态守卫和审计覆盖，在线链已接通为源码候选。下文较早日期记录的“源码头/迁移总数”均为当时冻结快照，不能覆盖这一目录事实，也不表示任何目标库已经迁移。
>
> **2026-08-28 财务 DDL 后置覆盖**：当前共享目录已并发推进到 V419/381；财务冻结顺序止于 V417/379，文件固定为
> `V407__finance_receipt_settlement_authority.sql` →
> `V408__append_only_account_flow_and_integrity.sql` →
> `V416__finance_migration_exception_queues.sql` →
> `V417__finance_payment_command_idempotency.sql`。V407/V408 已被本机 Flyway 执行，字节不得再改；
> V416 只以前向方式新增 V0 收款/旧流水异常队列、外部证据/金额/maker-checker 与新流水来源/币种守卫，
> 不改写历史金额、余额或 GL；V417 为付款增加创建幂等身份和草稿乐观版本。PostgreSQL 16.15 空库已验证
> 379 个迁移可到 V417，但这不是公司目标非空库迁移、历史对账、UAT 或生产放行。现行金额、流水和
> 异常队列细节以 [钱流迁移文档](26-钱流管理-新库与迁移.md)及
> [ADR-053](../99-决策记录-ADR/ADR-053-多币种收款毛额净额与追加式账户流水.md) 为准。
>
...
> **2026-08-11 后置 DDL 事实**：该时点源码头为 V253/234；V253 是网站询盘，不是生产默认车间或节点委派迁移。开发原库保持 V244/225，一次性生产物料 clone 仍是 V250/231 历史证据。V247–V250 的既有生产契约不变，V251–V253 也不改写它们。后续生产结构变更只能使用新的前向版本，公司目标库仍须从可恢复 V238 副本演练，不得据 clone 修改历史迁移。
...
> **2026-08-14 编号后置覆盖（V279）**：本节下方 V76 的 `YYMM + 4 位月流水`、21/25 个 Service 及按 `(prefix, period)` 回填的表述是 2026-07 历史落地记录，不再是切换后新号权威。V279 建立 `business_identifier_namespaces`、`business_document_sequences`、全局前缀/完整标识终身保留、成员和冲突证据表；登记 37 个文档命名空间及 `VISITOR_ACCOUNT=V`。新文档号为 `PREFIXYYYYMMDDNNNNNN`，按 PostgreSQL 上海业务日和命名空间原子递增；固定前缀与分类显式 `code_prefix` 共用全局精确 token 保留，文档/系统/主档完整标识全局终身占用。销售订单 `XD`，财务收款 `XS`，仓库损耗 `QW`，生产子计划 `SZ`。旧号原样保留并登记冲突，不批量改码。详见 [ADR-036](../99-决策记录-ADR/ADR-036-全局业务标识与单据号命名空间.md)。
...
**历史更新**：2026-08-11 · 早期跨模块通用约束继续有效；公司目标库到 V238，当时源码 V253/234，开发原库 V244/225，一次性生产物料 clone V250/231。V76 `doc_number_sequences`、V77 `master_code_sequences`、V78 `production_where_used` 及 V247–V250 后置生产不变量共同适用；V251/V252 分别追加货品导入来源与审计覆盖，V253 仅为官网询盘且与生产默认车间无关。§九已收紧为已应用迁移不可变和事故化处理。版本迁入不代表生产签字；当前版本分配以上方 V289/270 目录边界为准。
