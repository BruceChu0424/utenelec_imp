...
# 2. 干跑：只出报告不写库（务必先看 summary.txt 与四份 CSV）
...
| `warehouse_unmatched.csv` | 所属仓库段的差集：Excel 有这个产品名、库里没有同名货品（2026-09-15 实测 16918 条，属正常差集，不强灌） |

## 五之二、所属仓库段（V587，2026-09-15 追加）

Excel 第 16 列「所属仓库」写的是这批货平时归哪个仓管。V587 给货品主档加了
`goods.owning_warehouse_id`，本脚本负责回填。**这一段与上面按编号的补字段完全分开**：

| 口径 | 取值 |
|---|---|
| 匹配键 | **产品名称**(去全部空白后比对)，不是产品编号 |
| 为什么不用编号 | 这批 Excel 的编号只命中 22488 分之 85；名称干净得多 |
| Excel 去重名称 | 17010(脚本内归一后 16998)，**同名映射到两个仓库的冲突 0 条** |
| 取值分布 | 五金仓库 5316 / 塑胶仓库 8262 / 成品仓库 6771 / 包材仓库 1962 / 五金车间 4 |
| 写入规则 | **只填空**；库里已有值一律不动(界面上人工改过的不被冲掉) |
| 强制覆盖 | `--overwrite-warehouse`(默认关闭) |
| 缺仓库怎么办 | 按名称补建 `auto_created=true` 存根，编码 `XW01`/`XW02`…，挂在根仓下 |
| V587 未应用 | 整段跳过并打印 `[SKIP]`，不炸 42703 |

**开发库实跑验证（2026-09-15，pg_dump 克隆出 `uten_wh_probe` 后执行，未动开发库）**：

```
所属仓库：Excel 去重名称 16998（丢弃冲突名 0）；库内匹配 80；本次写入 84；补建仓库 2
```

117 个货品里 84 个落到仓库（五金仓库 44 复用既有 C01、塑胶仓库 30 与包材仓库 10
按名补建为 XW01/XW02），33 个是没有 Excel 对应行的测试货品，保持 NULL。
**第二遍再跑写入 0 行、补建 0 个仓库**，幂等成立。

> 正式库如果本来就有「塑胶仓库 / 包材仓库 / 五金车间」同名仓库，脚本直接复用，
> 不会重复建存根。
