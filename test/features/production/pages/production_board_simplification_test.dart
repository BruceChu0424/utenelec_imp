...
//  ② 大类行「待排产 / 进行中」后跟纯数字计数（行数/批次数，不是通知徽章）；
...
void main() {
  ...
  testWidgets('pending segment label shows plain row count', (tester) async {
    SharedPreferences.setMockInitialValues({});
    final preferences = await SharedPreferences.getInstance();
    final requests = <RequestOptions>[];
    await tester.binding.setSurfaceSize(const Size(1400, 900));
    ...
    // 2026-09-06 改徽章口径：「待排产」挂标准通知徽章（UtenFilterSegment.count，
    // 数字在徽章组件里，不再拼进 label 文本）；进行中仍是纯数字文本。
  ...
  testWidgets('ongoing segment label shows plain count of batches', (
    tester,
  ) async {
    SharedPreferences.setMockInitialValues({});
    final preferences = await SharedPreferences.getInstance();
