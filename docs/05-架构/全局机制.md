...
- [x] 权限：所有列表/详情/写/批量入口使用同一对象级数据范围守卫，并覆盖已知 UUID 越权测试（销售 V91 + 采购/委外/生产/库存 V233 + 财务 SEC-MED-4，均走 `DocumentAccessPolicy`/`OwnerVisibility`，真实多账号越权测试绿）
- [x] 视角：旧全局选择器提案未采用，页面不得依赖不存在的 Provider/组件
- [x] 数据范围：各模块列表/详情/写/批量/引用/导出使用同一后端 Policy 并有负向测试（V233；进度看板等原生 SQL 走 `nativeReadScope` 同口径）
