...
├──────────────────────────────────────────────┤
│ 样品编号  项目    结果   判定   日期          │
│ S001     拉伸强度 320MPa ✅合格 07-15        │  ← UtenDataTable
│ S002     含水率   8.2%   ❌不合格 07-15      │
└──────────────────────────────────────────────┘
```

## 四、涉及的 Uten 组件
`UtenAppBar`（搜索+筛选+FAB）/ `UtenDataTable`（桌面）/ `UtenResponsiveGrid`+`UtenCard`（手机）/ `UtenStatusBadge`（合格 success/不合格 error/待测 pending）/ `UtenSegmentedFilter`（全部/合格/不合格）/ `UtenEmpty` / `UtenSkeletonList`。
