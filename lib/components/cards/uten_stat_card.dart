...
class UtenStatCard extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    if (isLoading) {
      return UtenCard(child: _buildSkeleton(theme));
    }

    // 数值样式：28px w700 + tabular figures（数字等宽，跳动时不抖）
    final valueStyle = TextStyle(
      fontSize: 28,
      fontWeight: FontWeight.w700,
      height: 1.2,
      letterSpacing: -0.2,
      color: isDark ? UtenColors.darkTextPrimary : UtenColors.textPrimary,
    ...
    return UtenCard(
      ...
      child: Column(
        ...
        children: [
          // 第一行：图标 + 标题（13px textTertiary，弱化标签、突出数值）
          Row(
            children: [
              if (icon != null)
                Container(
          ...
          Row(
            ...
            children: [
              tier.enableNumberAnimation
                  ? UtenAnimatedNumber(value: value, style: valueStyle)
                  : Text(_format(value), style: valueStyle),
              if (unit != null) ...[
