...
// (dio 仅在 data != null 时建上传流)。于是所有不带 body 的请求把 connectTimeout 变成了
// 「服务端必须在这么久内开口」的硬上限，与 receiveTimeout 完全无关。
//
// 写请求被掐的后果最坏：服务端事务已提交，浏览器却报「网络连接超时」。读请求同样中招
// (ADR-108 前端性能审计)：一次 17-27 秒的生产执行工作台查询在 15 秒被掐断，自动重试
// 把服务端负载放大 3 倍，最后还被判成断网、整站重建。慢不等于断，所以 Web 上所有方法的
// connectTimeout 都放宽到与 receiveTimeout 同级——「总共愿意等多久」只有 receiveTimeout
// 一个口径。非 Web 平台上 connectTimeout 是真正的 socket 建连超时，保持短值才能对不可达
// 的服务端快速失败，因此默认只在 Web 生效。
