# 老库数据迁移 · 总索引
> **当前正式目录：V539/498 (2026-09-08)**。V533将附件原件身份与内部存储表示分离，V534新增服务器状态只读权限，V535使有完整合格来源的本批库存跟随实际入库仓，V536支持生产输出分仓及原单撤回，V537使业务附件真实清理完成后才允许清空测试业务，V538统一主仓公共库存安全数、保留实际子仓领料和成本。V539以独立前向迁移补齐附件删除证明，V537保留已执行文件的原校验值。最终V539组合的迁移、恢复与全量验收仍进行中；专项通过不等于公司服务器已更新。
