...
public class SupplierSettlementService {
    ...
    public BatchDetail freeze(FreezeRequest request) {
        ...
        assertLegacyOpeningWindowReplayable(request.supplierId(), request.currencyId(), start, end);
        assertCashBookKnown(request.supplierId(), request.currencyId(), end);
    ...
    private void assertCashBookKnown(UUID supplierId, UUID currencyId, LocalDate end) {
        long unknownBook=((Number)em.createNativeQuery("SELECT count(*) FROM finance_payments payment"
                +" WHERE payment.supplier_id=:supplier AND payment.currency_id=:currency AND payment.bill_date<=:end"
                +" AND payment.status IN(1,-1) AND NOT payment.is_deleted AND payment.legacy_id IS NULL AND "
                +com.uten.imp.features.finance.LegacyOpeningBalanceSql.unknownCashBook(false,"payment"))
                .setParameter("supplier",supplierId).setParameter("currency",currencyId)
                .setParameter("end",end).getSingleResult()).longValue();
        if(unknownBook>0)throw conflict("历史付款核销的本币账面金额尚未核验，不能按零差额重建月结");
    }

    private void assertLegacyOpeningWindowReplayable(UUID supplierId, UUID currencyId,
                                                       LocalDate start, LocalDate end) {
        long unknown = ((Number) em.createNativeQuery(SupplierSettlementSnapshotSql.UNREPLAYABLE_OPENING)
    ...
    private List<SnapshotLine> snapshotLines(UUID supplierId, UUID currencyId,
                                             LocalDate start, LocalDate end) {
        @SuppressWarnings("unchecked")
        List<Object[]> rows = em.createNativeQuery(SupplierSettlementSnapshotSql.LINES)
                .setParameter("supplierId", supplierId).setParameter("currencyId", currencyId)
    ...
    private void assertNoUnreplayablePaymentReversals(UUID supplierId, UUID currencyId, LocalDate end) {
        long count = ((Number) em.createNativeQuery("""
                ...
                WHERE payment.status=-1 AND payment.bill_date<=:end
                  AND payment.legacy_import_run_id IS NULL
