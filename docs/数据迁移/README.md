...
> **当前正式目录：V557/515 (2026-09-12)**。V557在原品质事件上保留完整批请求摘要用于精确重放，旧事件不回填；V556共享审计风险与分类计算，规则、旧审计值和索引不变；V555保留原单不合格实退后的真实替补来源权益；V554修正合并订货来源的剩余供给区间；V553订正设置说明。原收货、库存、金额事实和管理员配置值保持不变。目录版本不代表目标服务器已经升级，各版本规则、数据影响和兼容要求见下表。
>
