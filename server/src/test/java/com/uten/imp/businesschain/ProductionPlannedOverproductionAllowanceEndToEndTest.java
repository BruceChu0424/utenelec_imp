...
class ProductionPlannedOverproductionAllowanceEndToEndTest {
    ...
    @Test void initialPlanRateFlowsThroughReadOnlyPreviewApprovalAndSameRateGrowthWithoutOverwritingAnotherRate() {
        ...
        // A legacy client omits the rate: its new 10% task must not overwrite or reuse the existing 20% task.
        ...
        var separate=commands.issueWorkshopPlans(analysis.analysisId(),defaultRate).plans().getFirst();
        ...
        assertDecimal("1",db.queryForObject("SELECT sum(public_surplus_qty) FROM production_material_analysis_plan_links WHERE analysis_id=?",BigDecimal.class,analysis.analysisId()));
    ...
    @Test void draftPlansCanSetZeroOrMoreThanOneHundredPercentAndRejectInvalidPrecisionBeforeWriting() {
        ...
        assertDecimal(".1",plans.create(manual(null)).getItems().getFirst().getAllowedOverproductionRate());
    }
    ...
    @Test void batchSplitInheritsTheApprovedEffectiveRateInsteadOfReturningToTheOriginalPlanRate() {
        ...
        assertDecimal(".1",plans.detail(plan).getItems().getFirst().getAllowedOverproductionRate());
