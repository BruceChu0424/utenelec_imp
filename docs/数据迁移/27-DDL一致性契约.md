...
> **2026-08-10 后置 DDL 事实**：当前源码头为 V250/231；开发原库保持 V244/225，一次性 clone 迁到 V250/231。V247 冻结 BOM 控制阶段/包装计量和分析分配快照，V248 冻结非线性执行段精确需求，V249 冻结 `DEMANDED/ZERO_MATERIAL` 证据形态，V250 冻结物料分析外部采购/委外来源。公司目标库仍须从可恢复 V238 副本演练，不得据 clone 修改历史迁移。
...
**最后更新**：2026-08-10 · 早期跨模块通用约束继续有效；公司目标库到 V238，当前源码 V250/231，开发原库 V244/225，一次性 clone V250/231。V76 `doc_number_sequences`、V77 `master_code_sequences`、V78 `production_where_used` 及 V247–V250 后置生产不变量共同适用；§九已收紧为已应用迁移不可变和事故化处理。版本迁入不代表生产签字。
