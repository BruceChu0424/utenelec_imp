...
redirect: (context, state) {
  ...
  // 路径 → 所需权限点列表（任一满足即可；返回 null = 不需要权限）
...
- `requiredAnyPermFor` 返回 `List<String>?`：支持「任一权限满足」的多权限路径（如客户资料 `customer:view:self/department/all` 三级任一即可见入口）。
- 多级数据范围（self/department/all）由后端按用户持有的最高范围过滤，路由守卫只判"能否看到入口"。
...
- [ ] 多 Tab 同时操作时的状态同步
- [ ] 跨页跳转的来源感知导航（已落地 `nav_helpers.dart`：`goFrom` 带 `?returnTo`、`backTo` 读 returnTo；详见 memory：go-router-origin-aware-nav）

---

**最后更新**：2026-07-27（`authStateProvider` → `sessionProvider`；权限守卫对齐 `requiredAnyPermFor` + ADR-011；新增 mustChangePassword 分支）
