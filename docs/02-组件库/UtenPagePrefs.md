...
2. **服务端同步**：会话就绪(登录/换号)后从会话快照 `sessionSnapshotProvider`(`GET /api/auth/me` 一次带回的
   偏好整表, ADR-108)取 `prefKey` 覆盖本地；服务端没存过该 key → 保留本地缓存/默认值，不打断页面。
   全部子类共用这一份快照, 整个会话只有 `/auth/me` 那一次请求(原 `GET /user/preferences` 已删除)。
   写成功后就地更新快照(`updatePreference`), 不为此重拉。
   **只采用新加载的快照**(`SessionSnapshot.generation` 变了: 登录/换号/恢复会话/授权变化重取)：本端写偏好后
   就地派生的快照不回灌任何 notifier; 同一身份重取快照时, 本地有未确认的改动(防抖中/推送中/推送失败)
   也不被覆盖; 换身份时上一个身份没推完的改动作废, 不会推到新身份名下。此前 A 键推送成功会把 B 键还在
   防抖中的改动冲回登录时的旧值、再把旧值推上服务端(回归测试 `uten_page_prefs_snapshot_race_test.dart`)。
3. **写路径**：乐观更新 `state` + 立即写缓存 + 防抖 800ms `PUT /user/preferences/{prefKey}`；
   **离线兜底**：PUT 失败静默(本地已生效，本会话内不被快照覆盖)，下次登录以服务端为准自然收敛，不做重试队列。
