...
- [x] 前端组件 `TaskClaimBadge` / `TaskClaimHandle`（claim/30s 心跳/释放，非阻塞）+ **报销审批详情页**接入（flutter analyze 0）。**销售订单/采购订货/履约工作台页面接入待补**（这些是复杂多动作核心页，为不破坏核心功能留作专项；后端守卫已提供正确性）。
...
### 4.0 已实施的 refresh 撤销队列不属于业务离线能力

当前存在一个刻意窄化的 `PendingRefreshRevocationStore/Drainer`，只解决“用户已本地退出，但网络暂时
无法通知服务器吊销旧 refresh token”：

- 退出激活 fence 后，在跨标签 token record 锁内先把精确 refresh token 持久交接到
  `flutter_secure_storage` 的加密队列，再写 logout tombstone；交接失败时 fence 与原 token record 保留
  供重试，禁止先删设备上的唯一 refresh 副本。队列最多 32 项、保留 30 天；
- 队列只调用公开、幂等的 `POST /auth/logout`，成功后按 token 精确删除；启动、恢复事件和
  5 秒/30 秒/2 分/10 分/30 分封顶退避触发 drain，网络等待不阻塞退出/新登录；
- 队列有独立跨标签页锁，网络调用在锁外；并发标签页可能重复幂等 logout，但不能覆盖后来入队的 token；
- 损坏队列直接删除且不得影响 auth record/会话恢复。

这不是下文设想的 `OfflineActionQueue`：不得承载报销、建议、订单、审批、付款、库存或任意通用
业务 payload，也不能作为“业务写已支持离线重放”的证据。当前连接恢复只自动重试 GET/HEAD/OPTIONS；
其它业务写仍须在线并由调用方处理不确定结果。
...
已补强订单/出货的新写入口、特殊仓库/财务动作与来源订单复核，但跨全部模块的统一负向矩阵尚未完成，
仍是 NO-GO 阻断，不能用前端隐藏或 UUID 难猜作为缓解。
...
- 采购/委外申请由计划链以 `status=1` 下达，业务端只有列表/详情查看；新建、编辑、审核、红冲、删除和商业字段都不得从客户端能力推导。
...
- 审批必须同时满足动作权限和合格审核人资格（财务部门 `DEPT_FIN` 及子树内在职、账号启用且持 `finance_order_approval:review`，或在权限管理被个人加授该权限），由 `WorkflowReviewerEligibility` 实时查库判定。超级管理员身份、已知 UUID 或本地角色都不能绕过实时资格判定；前端只使用服务端 `allowedActions`，缺失即 fail closed。
...
- V196–V202 仍是源码候选，公司目标库只读证据仅到 V190。真实岗位权限、并发、仓库/退回实物、完整 IQC 和发布签字未完成，生产 **NO-GO**。
### 6.8 销售履约的商业、库存与质量边界

- 订单/出货商业事实由服务端计算或从已审核来源订单重取；客户端价格、金额、币种/汇率、税率和付款条件不能成为权威。财务已审出货先反审才可编辑、删除或驳回，价格脱敏在服务端执行。
...
| 采购/委外订货财务审批与到货异常 | 🟡 V196–V202 源码候选，生产 NO-GO | 计划申请只读、任务中心分解、财务审核组（`finance_order_approval:review`）、预计到货、超量隔离和原下单人退回已形成专用流程；目标库仍只确认到 V190，岗位/实物/IQC/发布未验收 |
...
| 审计中心与应用操作留痕 | 🟡 主体已实施 | 请求、显式事件、只读核查、设备证据、加密导出及两阶段留存已接；V183–V185 加固覆盖/软删/脱敏，V190/V197/V202 在新增表后全 `public` sweep；V196–V202 尚未应用公司目标库，生产容量、边缘联查与恢复演练待验收 |
...
**最后更新**：2026-08-02（补会话一致性与弱网恢复，并对齐 V196–V202 计划申请只读、订货精确财务审批、预计到货、超量隔离和全 `public` 审计 sweep；公司目标库仍为 V190，生产 NO-GO）
