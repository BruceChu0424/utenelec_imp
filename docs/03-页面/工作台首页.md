...
> 路由：`/dashboard` · 实现源：`lib/features/dashboard/pages/dashboard_page.dart` + `lib/features/dashboard/widgets/dashboard_overview_sections.dart` + `lib/features/dashboard/widgets/workbench_module_area.dart` + `lib/features/dashboard/widgets/system_test_area.dart` + `server/.../features/dashboard/DashboardOverviewController.java`
...
> 2026-08-31 源码候选：品质管理部顶层只保留「品质任务中心」；旧工作台 `/lab/test` 占位移除。IQC/FQC 待办与真实只读「检测记录」统一在中心分组，记录页路由为 `/quality/inspection-records`；它读取既有追加式质量决定/撤销事实，不等于实验室模块已实现。
> 2026-09-03 新增（ADR-067）：页面**最底部**新增「系统测试」区（仅超管可见、默认收起），首张卡「清空数据 → 清空业务数据」把当前环境（本地开发库/内网测试服务器）重置为干净测试起点，详见文末第十一节。
...
## 十一、系统测试区（2026-09-03 · ADR-067，仅超管）
...
- **清空口径**（后端 V462 函数 `business_data_reset()`，与 `server/ops/reset_business_data.sql` psql 停机版同一份 318 表清单，`BusinessDataResetSqlContractTest` 锁定同步）：
  - 保留：基础资料（货品/客户/供应商/账户/仓库…）、人事、账号权限、审计日志与治理证据；保留表行数不变校验；
  - 清空：销售/采购/委外/生产/仓库/财务全部业务单据、明细、流水、预留、核销及派生数据（222 张表），`TRUNCATE RESTART IDENTITY`，自增 ID 从 1 重新开始；
...
- **安全门禁**（后端四重）：`uten.features.business-data-reset-enabled` 运行开关（仅 dev / internal-test profile 开启，生产与云端默认 403 拒绝）→ 数据库确认的超管（`@PreAuthorize("principal.superAdmin")`）→ 请求体口令逐字匹配 → 排水闸（清空期间其它 `/api` 请求 503，等在途请求清零再执行；替代 psql 版「停应用」静默前提）。单事务执行，任一校验失败整体回滚；不创建备份、不可恢复。
- **拒绝语义**：outbox 有待处理/失败事件、附件存在非人事 owner、出现未分类新表等可自行解除的拒绝返回 409（SQLSTATE UT900）并在通知里给出处置提示。
