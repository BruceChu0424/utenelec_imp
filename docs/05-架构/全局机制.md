...
- 采购/委外每次部分 IQC PASS 都在处置事务内按合格基本量写 `DIR_IN` 并即时刷新命中的活动分析；仅有申请、订单、预计到货、待检或拒收只能影响预计量/缺口。只有同一 receipt 全部明细终态，才按稳定锁序执行一次正式来源推进，把累计 PASS 量转换为订单累计、peg/预约/DRAW/正式执行段齐套。红冲精确反向已 PASS 库存/供给后向下刷新，且不发正向增加通知。
- 计划进度分三维读取：`Σfqty` 是报工、`Σiqty` 是已审核 FINISHED_IN 入库、正式物料状态来自 `CONFIRMED` 包和正式执行段。三者不能互相推导；计划结清按入库口径，分析 ready 或报工完成都不等于成品已入库。
