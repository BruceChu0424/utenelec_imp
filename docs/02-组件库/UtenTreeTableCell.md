...
> 已接入：货品资料-组件信息（`goods_bom_tab.dart`）/ 生产物料分析统一物料表（`material_analysis_material_table.dart`）。**凡在 MasterDataTableView/UtenEditableGrid 类表格里展示「父子层级行」，层级标识一律用本单元格，不得各页自写缩进**（2026-09-04 起）。
...
| 元素 | 行为 |
|---|---|
| 展开箭头（48×48） | 有子级（`hasChildren`）才显示；独立命中区，**与行选择/双击打开互不抢手势**；`Semantics(button, expanded)` 带中文朗读 |
| 叶子节点圆点 | 无子级时 8px 实心圆点占位，与箭头同列对齐 |
