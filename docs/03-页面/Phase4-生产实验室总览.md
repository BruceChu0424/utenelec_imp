...
> - **仓库管理**（9 类出入库单据 + 14 张报表）作为独立模块 `/warehouse` 接入候选；完整 WMS、生产交接和 IQC 仍未验收。
...
> 🟡 **业务模块源码已接入候选**（不在原 10 页 UX 内，独立 hub 页）：生产管理 `/production`、仓库管理 `/warehouse`。V194 MAKE 回供仍待目标库迁移和真实岗位/实物 UAT；采购/委外 IQC 与新委外发料审核 409 门禁继续阻断生产发布，详见 [页面总览 §业务单据与报表](页面总览.md#业务单据与报表2026-07-新增全真实后端)。
...
三页只有在现有排程、日报和生产报表不能满足业务后才重新立项；库存/仓库主体已接入候选。
**候选已接入**：生产管理 + 仓库管理 + 库存查询（含报表 + 加密导出）；不等于完整生产履约上线。
