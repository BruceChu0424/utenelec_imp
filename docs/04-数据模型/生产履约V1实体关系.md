...
全树是分析视图，不是跨层需求合并：父产品只直接消耗其直接层组件；MAKE 节点的下层材料归属系统 `MAKE_COMPONENT` child item。子件未完工并合格入库前，只能影响预计齐套日，不能提高父需求可立即生产量。采购/委外也只把 IQC 合格数量计入可用量；待检、拒收和仅有订单/预计到货均不能提高当前齐套量。
...
V196–V234 已包含在开发原库 `uten_imp` V244 及公司目标库 V238 以内；当前源码到 V252/233 个迁移（V251 货品导入，V252 审计覆盖），
一次性隔离克隆已到 V250/231。迁移存在和本轮工程验证不等于目标部署、岗位 UAT、实物演练
...
> **迁移范围**：开发原库 `uten_imp` 实测 V244/installed_rank 225；当前源码目录为 V252/233 个迁移，一次性生产物料克隆仍只到 V250/installed_rank 231；公司目标库保留 V238 既有只读证据。目标环境版本必须以现场 `flyway_schema_history` 为准。
...
5. 下游按采购/委外分解、正式订单财务审批、到货和 IQC 链履约。只有检验合格且真实入库的基本数量进入当前可用量；待检/拒收只留预计或缺口。采购/委外合格入库、红冲及成品入库在来源事务内按稳定顺序锁定并刷新命中的活动分析，刷新失败则来源事务回滚；MAKE child 同样须完成自身分析、正式计划、报工及合格入库。
