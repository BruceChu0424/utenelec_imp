 ...
 *
 * <p>角色体系下线（ADR-011）后改为按【权限点】判定，入参为当前用户的
 * 有效权限集合（JWT permissions claim，登录/刷新时由 PermissionResolver 合成，
 * 超管恒为全量因此天然放行）：
 * <ul>
 *   <li>身份证号 / 手机号 / 银行卡号 / 开户行：employee:pii:view（新增）</li>
 *   <li>薪资额（基本/绩效/社保/公积金/补贴）：employee:compensation:view</li>
 * </ul>
 * 不按角色名（hr/finance/admin）判定：角色下线后残留角色会造成
 * “权限被收回但仍能看明文”（fail-open），故一律走上方权限点。
 */
@Component
