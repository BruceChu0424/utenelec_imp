...
- V196–V202 已包含在开发原库 `uten_imp` V244 和公司目标库 V238；当前源码到 V252/233（V251 货品导入，V252 审计覆盖），一次性生产物料克隆仍是 V250/231 历史证据。真实岗位权限、仓库/退回实物、完整 HTTP IQC 和发布签字仍未完成，生产 **NO-GO**。
...
| 采购/委外订货财务审批与到货异常 | 🟡 已迁移/有源码，生产 NO-GO | 生产分析形成申请、采购/委外任务中心分解、财务审核组（`finance_order_approval:review`）、预计到货、超量隔离和原下单人退回已形成专用流程；公司目标库保留 V238 既有证据，开发原库 V244/225，当前源码 V252/233，一次性生产物料克隆 V250/231，但岗位/实物/IQC/发布未验收 |
