-- V308: refresh the fail-closed full public audit-trigger sweep after V307.
--
-- V307 creates the business-bearing preplan exact-stock peg ledger and attaches
-- an explicit audit trigger. Re-run the shared sweep so the repository contract
-- proves that every public business table has exactly one valid enabled AFTER ROW
-- INSERT/UPDATE/DELETE trigger backed by fn_audit/fn_audit_redacted.
...
BEGIN
    ...
    LOOP
        ...
        END IF;
        EXECUTE format(
            'CREATE TRIGGER trg_audit_%1 AFTER INSERT OR UPDATE OR DELETE ON %1 '
