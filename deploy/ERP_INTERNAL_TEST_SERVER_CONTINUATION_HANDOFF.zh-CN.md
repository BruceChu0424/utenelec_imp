...
全量业务代码已通过 PR #42 合并到 `main`；Flyway 当前为 287 个唯一版本、head V306，业务代码层的
Flutter、Java、迁移与 main 合并后 CI 均已通过。发布预检随后发现并修复三个随真实日期老化的 onboarding
测试夹具，并在本候选中将 909 项部署合同、模板与脚本语法校验加入日常 Quality Gate，待独立 PR 合并后
生效。909 项为 0 失败、0 错误、46 项按平台/权限门控跳过。完整证据和当前外部门禁见
[当前状态第 9 节](current-test-server-status.zh-CN.md#9-2026-08-20-源码合并与发布前刷新)。

这项进展只把第 5.1 节从“未形成受审源码”推进到“业务集成已合并、发布合同后续修复形成独立候选”；
该候选进入 `main` 并通过 post-merge CI 后，才可登记为源码/合同修复完成。
第 5.2–5.4 节仍未完成：当前没有受保护 release tag、签名制品、OSS readback、H01–H12、目标数据库/备份
或 UAT/恢复证据。禁止给旧本地 bundle 打签名或复用旧 `v2026.08.11-1`；必须先合并后续修复并重新读取
最终 `origin/main`，再建立当前签名候选。目标连接仍须等待 H01–H12 受控证据包，且第一次只允许固定
Host Key 的只读刷新。
