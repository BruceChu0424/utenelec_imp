...
> 当前包含「我的部门」卡片（全员只读 + 部门负责人权限转授面板）和工程研发部任务中心；所有模块待办角标由 `WorkbenchCardBadge` 统一渲染。
...
4. **功能模块区**（核心）：原侧边栏全部分组迁入——
   ...
   - 工程研发部分区「任务中心」卡（`/rd/tasks`，权限点 `rd_task:view`）接收生产 BOM 缺失转发等任务；共享角标读取 `/api/rd-tasks/count` 展示未完成数，详见 [工程研发部任务中心](工程研发部任务中心.md)。
