...
abstract final class Perm {
  ...
  static const visitorHostConfirm = 'visitor:host_confirm'; // 接待访客 = 对外接待白名单(V657)
...
> **审计现行口径(2026-09-23, ADR-105, V646-V649; 与下文历史描述冲突时以此为准)**
...
#### 6.3.1 约束/守卫触发器的起跳条件(2026-09-23 ADR-106，V650-V652)
...
- **INSERT 与 UPDATE 共用的拆成两条**：原名留 INSERT(/DELETE)，`原名_upd` 管 UPDATE(DELETE 条件不同再起 `原名_del`)。
  重建触发器会把 `ENABLE ALWAYS` 复位，原来是 ALWAYS 的要逐条恢复(契约测试把 V650 重建的 38 条 ALWAYS 写死逐条断言，
  以后再拆出的变体须与原名启用模式一致)。
