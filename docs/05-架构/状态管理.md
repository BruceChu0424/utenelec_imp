...
- 行复选、表头全选/半选、选中行高亮、BOM 节点展开和 A4 向导当前页属于页面草稿状态；它们不能先写业务表，也不能把“已勾选”显示成已通知/已下达。
- 通知提交选中的 material/action 标识和幂等键；成功后必须用服务端返回的 analysis/action/notification targets 重建页面。采购/委外申请已形成不等于商业订货已形成。
- 分批计划向导逐张维护 workshop/负责人/日期/数量，但普通预览和翻页不创建正式计划。最终 generate 必须携带 analysis version、分析/预览指纹和每张数量；冲突后丢弃旧预览并重读。
...
**最后更新**：2026-08-10（补生产物料分析本地多选/A4 草稿与服务端 action、version、ready、IQC 刷新事实边界，以及订货审批响应同事务）/ 2026-08-03（§4.6 补「返回即刷新」pageResume 交叉引用；此前：补齐原子 token record、CAS/latest-intent、logout fence、加密 refresh 撤销队列与恢复竞态；保留 §4.6 列表页刷新规范）
