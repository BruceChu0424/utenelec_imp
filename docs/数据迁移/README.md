# 老库数据迁移 · 总索引
> **当前正式目录：V552/510 (2026-09-12)**。V552 新增 `server_status:alert:receive`（服务器状态告警的站内推送收件权限，并授予已持 `server_status:view` 的部门）。V551 让待交货视图排除草稿单（草稿不再计入待交货汇总）。V550 把生产链仓库单据的 remark 移出身份不可变集合（出库备注可追加）。V546 把附件上传/删除权限扩授到业务单据归属部门并归类「通用·附件」，V549 把清空业务数据的附件保护集扩到货品主档（EMPLOYEE/EMPLOYEE_CONTRACT/GOODS）。V547 新增生产成品品质检查单聚合层（同仓一次送检合并一张单，数量仍在 inspection），V548 兑现产成品送检登记撤回（品质未处理前可撤，报工行回到待登记）并把「未登记」谓词收口到视图。V546 把附件上传/删除权限扩授到销售/采购/委外/仓库/计划/生产/品检等单据归属部门，并把 `attachment:*` 归类到「通用 → 附件」（新建单据保存前暂存附件、财务审核页只读、Office 在线预览的权限前提）。V545 把销售订单行 chain_status 统一为「剩余未排量优先」派生并一次性回填（部分排产行留在待排产）。V541 收回生产部（6 车间继承）16 个供应链查看/编辑码，V542 产成品送检登记加备注，V543 清 V541 残留 22 行（`production_plan:view:all` 范围码/计划包/调度/模具写码/僵尸码）、`production.*` 面下架停用码并补齐我的车间任务面三码、9 个 `*:view:all` 退出批量授权。V533将附件原件身份与内部存储表示分离，V534新增服务器状态只读权限，V535使有完整合格来源的本批库存跟随实际入库仓，V536支持生产输出分仓及原单撤回，V537使业务附件真实清理完成后才允许清空测试业务，V538统一主仓公共库存安全数、保留实际子仓领料和成本。V539以独立前向迁移补齐附件删除证明，V537保留已执行文件的原校验值。V540保留停用存放仓的既有库存清出，并为自动齐套恢复记录真实系统身份。最终V540组合的迁移、恢复与全量验收以统一记录为准；专项通过不等于公司服务器已更新。
