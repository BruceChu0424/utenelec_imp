...
- 上述路由权限只决定“能否进入这一类页面”。财务详情还必须由服务端按财务部门资格 + `finance_order_approval:review` 实时判定（`WorkflowReviewerEligibility`）；退回详情必须校验当前账号是原下单人。超级管理员不在财务部且未被点名加授不可代审，已知 ID 也必须 404/403。
...
- 公司目标库 2026-08-09 已确认到 V238、源码到 V244；但路由与迁移存在不等于真实账号、岗位、实物和生产验收完成。
