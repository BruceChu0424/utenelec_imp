...
export default async function ProductEditPage({ params }: { params: { id?: string[] } }) {
  ...
  const [product, series] = await Promise.all([
    id ? prisma.product.findUnique({ where: { id }, include: { series: true } }) : Promise.resolve(null),
    prisma.series.findMany({ orderBy: { sortOrder: 'asc' } }),
  ]);
  if (id && !product) notFound();
  const seriesLite = series.map((s) => ({ id: s.id, code: s.code, name: tr<{ name: string }>(s.i18n, 'zh').name }));
