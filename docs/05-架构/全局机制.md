...
| 问题 | 决策 |
|---|---|
| 权限粒度 | **角色级为主**（employee/hr/finance/lab/production/manager/admin）+ **关键写操作加功能点**（`payroll:generate` 等）|
| 多角色用户 | 一个账号可多角色，权限取**并集** |
...
| 权限来源（2026-07-23 起） | **三层合成**：直接角色 ∪ 直属部门角色（`department_roles`）→ 权限点并集 → ± 个人覆盖（`user_permission_overrides`：grant 加授 / revoke 回收）；超管全量放行 |
| 权限管理入口 | [权限管理页](../03-页面/权限管理页.md)（仅超管，`/admin/permissions`）：按员工（角色+覆盖）/ 按部门（默认角色）；`AuthService.permsOf` 在登录/刷新令牌时重新合成，变更下次登录生效 |
| 菜单显隐数据源 | 工作台模块区与路由守卫共用 `permission_by_path.requiredPermFor()`，单一映射不再两处维护 |
...
abstract final class Perm {
  ...
  // 决策支持（V25 起独立权限点，与 viewcontext 解耦）
  static const analyticsView = 'analytics:view';        // 经营Dashboard/多维分析/异常告警
}
...
const Map<String, Set<String>> rolePermissions = {
  ...
  'manager':   {Perm.employeeView, Perm.payrollViewAll, Perm.expenseApprove, Perm.viewContextScoped, Perm.analyticsView /* 全部只读 */},
  'admin':     {/* 全部，前端用通配 */},
};
```

> **2026-07-23 更新（V25）**：决策支持从 `viewcontext:scoped` 拆出独立权限点 `analytics:view`，
> 默认仅 manager/admin 持有（hr/finance 不再顺带可见）；其他人由超管在
> [权限管理页](../03-页面/权限管理页.md) 显式授予。`viewcontext:scoped` 仅保留「管理视角切换」语义。
...
const Map<String, String> permissionByPath = {
  ...
  // Phase 5 管理层
  '/analytics/dashboard':  Perm.analyticsView, // V25 起：决策支持独立权限点（原误标 employeeView）
  '/analytics/explore':    Perm.analyticsView,
  '/analytics/alerts':     Perm.analyticsView,
};
