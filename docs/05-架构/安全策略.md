...
sequenceDiagram
    ...
    B->>B: 短读事务取账号 → 事务外校验密码（Argon2 并发闸门）
    B->>B: 错误次数+1（超限锁定；锁定期内只跑 dummy 哈希）
    B->>B: 短写事务：清零计数、建服务端会话 auth_sessions
    B-->>F: {accessToken(短期，含 sid), refreshToken, user}
...
**锁定期内不校验真实密码 (ADR-110，security-01)。** 账号处于临时锁定时只跑一次 dummy 哈希，
回与「账号或密码错误」相同的码和文案，同时照常计数并顺延锁定——锁定期内反复试密码的人得不到
任何「这次对了」的信号。管理员手动锁定、停用只在密码正确之后才透出具体原因。

**哈希并发闸门 (security-06)。** `BoundedPasswordEncoder` 把同时进行的 Argon2 校验限制在 CPU
核数以内，排队 1 秒拿不到名额回 `503 AUTH_BUSY` 并带 `Retry-After`。登录与改密都拆成「短读事务 →
事务外哈希 → 短写事务」(`StaffLoginTransaction` / `PasswordChangeTransaction`)，哈希期间不占数据库
连接；写事务里按读到的密码哈希做一次比对，期间密码被别人改过则拒绝。

**限流 (security-13)。** 先扣 IP 桶再扣主体桶，IP 超限直接拒绝、不为随机账号新建主体条目；IP 桶
改为平滑回填、瞬间最多放行 20 次；条目按访问过期(2 分钟)，表总量上限 5 万项，满额时先清过期条目、
仍满只拒绝需要新建条目的请求，绝不整表清空重置限流状态；IPv6 按 /64 聚合。网站询价推送入口：生产
环境 token 不足 32 位拒绝启动，入口每 IP 每分钟最多 5 次失败。

### 3.2 Token 策略

| Token | 有效期 | 存储 | 用途 |
|---|---|---|---|
| accessToken | 默认 **15 分钟**（`jwt_access_ttl_minutes`，5–1440）；携带服务端会话号 `sid`，登出/空闲/账号变动即时失效，与本值无关 | 标签页级会话存储（Web：sessionStorage，各标签页独立；原生：flutter_secure_storage） | API 鉴权 |
| refreshToken | 会话的绝对期限：从**登录时刻**起算 7 天（`jwt_refresh_ttl_days`，1–30，系统设置页「登录保持时长」），轮换不延长 | 同上（随 token record 单记录保存） | 静默续期 accessToken |
...
### 3.2.4 服务端会话（`auth_sessions`，V658，ADR-110）

会话的权威状态在服务端：每次登录(员工或访客)插一行 `auth_sessions`，会话号 `sid` 写进 access token。

- **每请求一条 SQL**：`JwtAuthFilter` 在原来查账号状态的同一条 SQL 里连带查会话行与空闲阈值
  (`AuthSessionService.loadStaff/loadVisitor`)，判定只有五种：有效 / 不存在 / 已吊销 / 空闲超时 /
  超过绝对期限，后四种一律结构化 401(安全审计记原因)。空闲超时由过滤器(事务外)顺手把会话标记吊销；
  刷新事务只判定不写。
- **空闲只算人为请求**：`last_seen_at` 最多 60 秒更新一次；用户没在操作时发出的请求不更新。判定只看
  客户端声明头 `X-Uten-Automatic: 1`(`AutomaticRequestPolicy`)：前端全局采集键盘/鼠标/触摸输入
  (`UserActivityTracker`，含根导航器弹窗里的操作)，距上次输入超过 30 秒才发出的请求一律由
  `AutomaticRequestInterceptor` 带上此头——不管是角标轮询、服务器状态、审核心跳还是页面定时刷新，
  新增轮询不需要登记。页面开着不动，照样按时超时。这个头只能让会话更早过期，伪造没有好处；
  请求审计的降噪仍由服务端按端点声明，不看此头。
- **绝对期限**：从登录时刻起算 `jwt_refresh_ttl_days` 天，refresh 轮换只换令牌不延长会话。
- **吊销**：登出(旧 access 下一请求即 401)、改密、管理员重置密码、账号锁定/停用/启用/解锁、离职、
  复职、改登录手机号、远程访问授权变更、refresh 重放、再认证连续输错都吊销会话；原因写在
  `revoked_reason`(9 种枚举，含 `login_account_changed`)。
- **重放判定**：只有已被轮换出新令牌(`replaced_by` 非空)的旧 refresh 再次出现才算被盗重放(吊销全部会话 +
  安全事件)；整族被作废的令牌(改登录手机号、权限变更…)在会话仍有效时出现，是普通 401，不误报。
- 模拟身份(切换人)令牌挂在发起模拟的超管自己的会话上：超管登出或空闲超时，模拟令牌随之失效。
- 测试：`SessionIdleEnforcementPostgresTest` 用可控时钟逐项验证空闲/绝对期限/登出/自动请求不续期，
  并实测语句数(见 ADR-110 §四)。

### 3.2.5 会话空闲超时（服务端判定 + 前端及时提示）

空闲超时的**权威判定在服务端**(§3.2.4)：连续无人为请求超过 `session_idle_timeout_minutes`(默认 30 分，系统设置页「自动退出登录」可配)，下一次任何请求都是 401；本机计时被绕过(改时钟)也一样，页面上的角标轮询与定时刷新在用户没操作时都声明为自动请求，不续期。前端 `IdleTimeoutGuard`（包在已登录区根）只负责**及时回登录页**——监听用户点击/滑动/输入续期(另取全局输入采集 `UserActivity` 的最近时刻，根导航器弹窗里的输入同样算)，到点弹窗提示后登出（清 access+refresh），不必等下一次点击才发现已退出。前端计时**按标签页独立**（ADR-061）。
...
两层叠加：**一直操作**时 access 静默续期、idle 不计时（都不打扰）；**离开 30 分** 服务端会话空闲超时、前端同时回登录页；**登录满 7 天**(不论中途是否操作)会话到绝对期限需重输密码。前端阈值实时生效（进系统拉一次 + 每 5 分钟轮询 + 超管保存后 `idleThresholdVersionProvider` 信号立即重拉）；服务端每请求读当前值，保存即生效。

### 3.3 密码策略

- 至少 `password_min_length` 位(默认 8，可调 8–64；公开下发，改密页按它提示和预校验)，最长 128 位
- 至少包含字母 + 数字
- 不允许和账号相同
- 错误 `lockout_threshold` 次（默认 5）锁定 `lockout_minutes` 分钟（默认 15）
- 不可使用最近 `password_history_size` 次（默认 5）用过的密码
- 改密时的旧密码输错与敏感操作再认证共用失败计数（§3.7），连续输错到上限暂停并让当前登录失效

前端在登录/改密页做强度提示，后端强制校验。

**管理员临时密码（V297，`POST /api/admin/users/{id}/reset-password`，`account:support`）：**

- 只由系统生成（20 位高熵随机，四字符类全覆盖）；**不再接受管理员自定义**（ADR-110，security-02）。
  入职、补开账号的初始密码同样随机生成，不再由证件号推导。
- 一次性交付：明文只出现在当次响应，不持久化、不写日志/审计详情；重置即置
  `must_change_password=true`、撤销全部会话，员工登录后由 `PasswordChangeRequiredFilter`
  收口到改密端点(另放行 `GET /api/settings/public` 以取密码最短长度)，其余 API 一律 403。
- 有效期：写 `users.temp_password_expires_at = now + temp_password_ttl_hours`(默认 72 小时，1–168)；
  到期后即使密码正确也拒绝登录（401「临时密码已过期」），改密成功清除该列。开通/入职/重置同一口径；
  V660 把存量「必须改密但没有过期时间」的非超管账号置为已过期，并加触发器
  `trg_users_temp_password_expiry`：必须改密的非超管账号写空即按已过期落库(失败关闭)，任何写路径都造不出
  永久临时凭据。
  离职(含交接加固)即把旧密码置为已过期，复职保持作废，由账号支持重新发放；超管被降级时若还没改过
  初始密码同时置过期。
- 权限边界：`account:support` 只能个人点名授权(V660)；一律禁止操作超级管理员与本人；**发放登录凭据**
  (重置密码、给存量员工补开账号)时目标有效权限含任一高危权限(权限目录 `permissions.high_risk`，
  迁移集中维护：V212 高危清单、账号支持、余额调整、财税模块全部审批/执行类、报销审批与付款、工资生成/
  审核/发布/查看全员)只有超管能发放(`CredentialIssuancePolicy`)——否则账号支持人员可借「重置/开号 +
  拿到临时密码」冒充审批人、付款人。重置与补开账号都要求再认证(§3.7)。
- 锁定/解锁/停用/启用/重置都在同一事务里写语义化安全事件(`password_temporary_reset` 等)，并给目标
  本人发一条账号安全提醒(本人不能删除)；账号支持人员(非超管)重置别人密码时另提醒其他超管。
...
- 服务端会话表：已实施(§3.2.4，V658)；会话列表 + 远程登出界面：未实施(表已具备)
- 异地登录告警：未实施
...
- local 默认包含 loopback/RFC1918 便于安全开发，生产必须用 `UTEN_LOCAL_ALLOWED_CIDRS` 收窄到公司
  ...
  prod profile 不给默认网段：未配置即启动失败；配置里含整段私网(10/8、172.16/12、192.168/16)只告警不拦
  (security-17，现网部署脚本已配置，不打断现有部署)。
...
### 3.7 敏感操作再认证(ADR-110，V658)

敏感操作前重新输入登录密码，换一张**绑定当前会话、5 分钟有效、只能用一次**的凭证：

- `POST /api/auth/step-up {password}` → `{stepUpToken, expiresInSeconds}`；会话行只存凭证的 sha256。
  请求带 `X-Uten-Step-Up` 头使用，服务端在进入业务前用一条 `UPDATE … RETURNING` 原子核销，
  被拒的请求什么都没做(所以前端可以安全地带凭证重发一次)。
- 输错回 `422 REAUTH_FAILED`(不用 401，避免前端当成登录过期去刷新重放、一次输错算两次)；连续输错
  `lockout_threshold` 次暂停 `lockout_minutes` 分钟并回 `429 REAUTH_LOCKED`，同时吊销当前会话。
  改密旧密码、进入切换人与再认证共用 `auth_step_up_states` 计数。
- 接口标 `@RequiresStepUp`，由 `StepUpInterceptor` 统一拦截，缺凭证回 `403 REAUTH_REQUIRED`：
  设/撤超管、远程访问授权、个人权限覆盖、数据范围、部门权限、重置他人密码、给存量员工补开账号、
  进入切换人、保存系统设置、清理测试业务附件、清空业务数据；提交需人事审核的个人资料(手机号/姓名/
  户籍地址)在服务里核销。
- 核销时机：`StepUpInterceptor` 是挂在控制器代理上的方法拦截器(`StepUpAdvisorConfig`，顺序 700)，
  排在 `@PreAuthorize`(100–600)之后：参数校验不合法直接 422、无权直接 403 `FORBIDDEN`，都不消耗凭证、
  也不会先弹密码框。
- 锁边：`ArchitectureBoundaryTest` 要求 `/api/admin/**` 下所有写接口、以及响应里带明文临时密码的接口
  (不论前缀；按 DTO 的 `temporaryPassword` 组件自动识别)标 `@RequiresStepUp` 或 `@StepUpExempt("原因")`
  (锁定/解锁/停用/启用账号——要求填原因且只收紧；审计导出；入职开号——新员工首次拿账号、当面交付)，漏标即红。
- 删除 `/api/auth/verify-password`(独立的「对/错」接口无限次可试、结果不绑定操作)。
- 前端：页面不自己问密码。`StepUpInterceptor`(Dio) 收到 `403 REAUTH_REQUIRED` 经 `StepUpCoordinator`
  弹统一的 `ReauthDialog`，换到凭证后把原请求带头原样重发一次；并发时弹框排队；取消则原 403 照常抛出。
  弹框期间整页忙碌遮罩一律让位(`UtenBusyOverlay.yieldWhile`)，否则遮罩会被导航器抬到密码框之上、整页卡死。
...
`GET /api/auth/me` 的 `session.documentScopes`(ADR-108 起六个 scope 随会话快照一次带回, 原逐 scope 端点已删除)。快照只含 `sales/finance/purchase/subcontract/
...
或仍在加载时，Flutter 一律按只读处理。能力随会话快照在登录、恢复会话、切换代操作身份、令牌刷新带回的权限点变化以及用户点「重试」时重取(ADR-108，不再每次打开详情都作废重拉)；正式交接生效后，接收人最迟在下一次登录或点「重试」时看到可写，期间显示为只读不越权，服务端写接口仍逐次按对象校验。
...
- `production_material_analysis:*` 细分 view/create/refresh/cancel/route/notify/generate/reallocate/cross_reallocate/claim_shared_future/over_supply（`manage` 为已停用的历史复合权限；`bom_override` 已于 V423 随 BOM 例外机制下线删除）。其中 `cross_reallocate` 同时管「跨计划让料」与「跨路线在途调入」，`claim_shared_future` 管公共在途认领，`over_supply` 管超出需求的公共备货（含按起订量抬量抬出的富余）。路由 any-of 只允许进入工作台；每个按钮和 API 仍按动作权限、对象范围、analysis version/指纹及服务端动作条件独立拒绝。
...
- **2026-08-16 加密（详见 [99-项目治理/2026-08-16-附件与工资保密安全加固报告](../99-项目治理/2026-08-16-附件与工资保密安全加固报告.md)）**：
  ...
  - **上传原件保留**（2026-09-08）：客户端不缩图或转 JPEG。内部存储对适用文件无损压缩，
    下载还原原始字节；扫描、图片尺寸限制与对象权限继续生效。原件 SHA 与物理存储身份分离，
    历史对象固定后端路由，详见 [ADR-074](../99-决策记录-ADR/ADR-074-公司内部附件存储与原件保留.md)。
...
- **服务器选择** `apiBaseUrlProvider` + `ServerSwitchDialog`：原生 auto 本地优先，Release 只在两个构建
  ...
  客户端模式覆盖。
- **会话空闲超时守卫** [`lib/features/shell/widgets/idle_timeout_guard.dart`](../../lib/features/shell/widgets/idle_timeout_guard.dart)：包在已登录区根，滑动会话——监听用户活动续期，连续无操作超阈值（默认 30 分）弹窗登出；阈值实时生效（进系统拉 + 5 分钟轮询 + 超管保存立即重拉，见 §3.2.5）。权威判定在服务端会话(§3.2.4)，本守卫只负责及时回登录页。
- **统一再认证** [`lib/core/network/interceptors/step_up_interceptor.dart`](../../lib/core/network/interceptors/step_up_interceptor.dart) + [`lib/shared/widgets/reauth_dialog.dart`](../../lib/shared/widgets/reauth_dialog.dart)：服务端回 `403 REAUTH_REQUIRED` 时弹统一密码框、带一次性凭证重发一次(§3.7)；页面不再各写密码框。
- **敏感检索值不进 URL**：客户/供应商按手机、电话、备用电话、银行账号筛选，以及搜索框关键字(会匹配手机号)，改走 `POST /master/{clients,suppliers}/search`，值只在请求体，GET 列表不再接受关键字；导出放进导出请求体 `sensitiveFilter`(security-19)。
...
- 成功离职停用个人 override、本人获得/发出的经理委派、remote access，旧密码立即作废(必须改密且临时密码已过期)并吊销全部会话，清来源账号范围/单客户共享；复职不恢复旧个人授权，旧密码保持作废，由账号支持重新发放临时密码(ADR-110)。
...
| ③ **再认证** | 唯一写路径 `PUT /api/admin/system-settings`(批量)标 `@RequiresStepUp`：要一张 5 分钟、绑会话、一次性的再认证凭证(§3.7)——**即使 access token 被盗，改策略还需密码** |
| ④ 审计 | `update_system_setting`（记 谁/哪项/旧→新值） |
| ⑤ 校验 | 键、类型、默认、范围只登记在 `SystemSettingKey` 枚举(ADR-110)，服务端按登记校验，列表接口随项下发 `minValue/maxValue` 给前端提示；表里只存值 |
...
- [ ] 配置：先在 `SystemSettingKey` 登记(类型/默认/范围/说明/是否公开) + 迁移补数据行 + 按枚举读；写入走唯一批量路径(再认证 + 审计)；只放阈值（密钥/部署类走 env）
...
| ③ **再认证** | `enter` 标 `@RequiresStepUp`(统一再认证，§3.7；输错与改密/再认证共用计数)，签发绑定当前会话的限时 `modeToken`(`impersonation_window_minutes`，默认 15 分)；窗口内 `start` 反复切换免再输 |
