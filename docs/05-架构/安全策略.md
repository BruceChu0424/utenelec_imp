...
- **数据变化层**：V169 为公开业务表补齐 `AFTER` 触发器，提供脱敏 before/after；凭证、令牌、短信码、
  Flyway、序号与迁移运行元数据表不复制行内容。`request_id` 将一次请求与其多条数据变化串联。
...
并持续复核“端点→显式事件→表触发器”覆盖矩阵。目标字段如下：
...
- **权限契约复核**：当前 146 个 Flutter `Perm` 常量均进入超管 fallback；后端 Java 活动引用的
...
- 🟡 **已有安全基线**：Argon2id、JWT/refresh 轮换、会话空闲超时、pgcrypto、V169 业务表审计触发器、
  进入应用的 GET/POST/PUT/PATCH/DELETE/HEAD `/api/**` 请求元数据审计（含匿名、401、CORS 403、
  404/405 与前置异常）、部门权限 + 个人覆盖 + 按权限点脱敏。当前不得宣称“全量端到端安全验证”：
