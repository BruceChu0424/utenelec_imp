...
2026-09-07 起，`applyAutofillHint` 统一负责黄色描边、浅黄底和框内警告图标，直接使用它的采购/委外供应商、币种、汇率、税率、结算方式、销售收货地址/电话、车间和负责人不再遗漏图标。调用方已有具体预填说明时保留原说明，其余采用中英韩通用核对文案。汇率和税率表格列为图标与值共同留足空间，错误仍优先使用红框。
