...
开发原库 `uten_imp` 2026-08-10 已确认到 V244/225；当前源码到 V252/233（V251 货品导入，V252 审计覆盖），一次性生产物料克隆仍是 V250/231 历史证据；公司目标库仍以 V238 既有只读证据为准。真实岗位/实物 UAT、完整 HTTP IQC 和发布签字仍未完成，生产 **NO-GO**。
...
flowchart LR
    ...
    Procure --> IQC["收货 + IQC 合格入库"]
    Outsource --> IQC
    Make --> Finished["子件计划/报工/合格入库"]
    IQC --> Refresh["同事务刷新命中分析"]
    Finished --> Refresh
