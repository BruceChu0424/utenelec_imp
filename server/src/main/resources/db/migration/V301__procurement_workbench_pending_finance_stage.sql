-- V301: 采购/委外任务中心补「等待财务审核」生命周期档。
--
-- V207 的投影只把待财务订单的数量从申请行 open_qty 里扣除（purchase_pending /
-- subcontract_pending CTE），订单本体在任务台不可见——采购员提交财务后就找不到
-- 这张单据，「待完成」（open_qty > 0）也漏掉这批仍在途的任务。
-- 本迁移 CREATE OR REPLACE 同名视图，新增 purchase_order_pending_rows /
-- subcontract_order_pending_rows 两个 CTE：status=0 且当前有 PENDING 审批案件的
-- 订货单明细行，task_status = ORDER_PENDING_APPROVAL，open_qty = 行全量
-- （与申请行的扣减互补，待完成 = 所有未完成任务的总口径）。
-- 列名/类型/顺序与 V207 各行完全一致（mapRow 按固定列位读 31 列）。
...
COMMENT ON VIEW v_procurement_decomposition_tasks IS
    '采购/委外任务中心权威投影（设计对齐）：采购与委外均含 申请待分解(WAITING_ORDER,申请行剩余量,黄) / 等待财务审核(ORDER_PENDING_APPROVAL,V301,蓝) / 财务已通过·待采购完成(FINANCE_APPROVED,已生效订单未收完,青) / 财务驳回(FINANCE_REJECTED,审核未过未重提,红) / 已完成(COMPLETED,近30天收完,绿) 五档；申请行仍扣除已生效和待财务订单占用，待完成(open_qty>0)=所有未完成任务';
