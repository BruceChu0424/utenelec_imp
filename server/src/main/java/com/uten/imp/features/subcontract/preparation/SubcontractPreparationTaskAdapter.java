...
public class SubcontractPreparationTaskAdapter
    ...
    public Page tasks(TaskQuery query, boolean canStart, boolean canOpen) {
        int page = Math.max(1, query.page());
        int size = Math.max(1, Math.min(100, query.size()));
        String status = normalizeStatus(query.status());
        String like = query.keyword() == null || query.keyword().isBlank()
                ? null : "%" + query.keyword().strip() + "%";
        String statusClause = status == null ? "" : " AND effective.status = ? ";
        String keywordClause = like == null ? "" : """
                AND (plan.order_bill_no ILIKE ?
                     OR goods.code ILIKE ? OR goods.name ILIKE ?)
                """;
        String planItemClause = query.planItemId() == null
                ? "" : " AND item.id = ? ";
        String sourceNodeClause = query.sourceAnalysisId() == null
                || query.sourceMaterialLineId() == null ? "" : """
                AND EXISTS (
                    SELECT 1
                    FROM preplan_supply_action_allocations source_allocation
        ...
        String base = """
                ...
                JOIN subcontract_material_plans plan
                  ON plan.id = item.plan_id AND plan.is_deleted = FALSE
                JOIN subcontract_orders order_header
                  ON order_header.id = plan.order_id AND order_header.is_deleted = FALSE
                JOIN subcontract_order_items order_item
                  ON order_item.id = item.order_item_id AND order_item.is_deleted = FALSE
                ...
                LEFT JOIN warehouses warehouse ON warehouse.id = item.preparation_warehouse_id
                LEFT JOIN LATERAL (
                    SELECT COUNT(*) AS link_count,
                           (array_agg(source_action.id ORDER BY source_action.id))[1]
                               AS source_action_id,
                           (array_agg(source_allocation.id ORDER BY source_action.id,
                                source_allocation.id))[1] AS source_allocation_id,
                           (array_agg(source_allocation.analysis_id ORDER BY source_action.id,
                                source_allocation.id))[1] AS source_analysis_id,
                           (array_agg(source_material.id ORDER BY source_action.id,
                                source_allocation.id))[1] AS source_material_id
                    FROM preplan_supply_action_allocations source_allocation
                    JOIN preplan_supply_actions source_action
                      ON source_action.id = source_allocation.action_id
                     AND source_action.analysis_id = source_allocation.analysis_id
                    ...
                    JOIN subcontract_application_items source_application_item
                      ON source_application_item.id = source_allocation.external_item_id
                ...
                LEFT JOIN LATERAL (
                    SELECT handoff.id AS handoff_id,
                           COALESCE((
                               SELECT SUM(CASE requirement_event.event_type
                                   WHEN 'TAKEOVER' THEN requirement_event.qty
                                   ELSE -requirement_event.qty END)
                               FROM preplan_subcontract_requirement_handoff_events
                                    requirement_event
                               WHERE requirement_event.handoff_id = handoff.id
                           ), 0) AS takeover_qty,
                           COALESCE((
                           ...
                           ), 0) AS entitlement_qty
                    FROM preplan_subcontract_requirement_handoffs handoff
                    WHERE handoff.plan_item_id = item.id
                ) handoff_state ON TRUE
                CROSS JOIN LATERAL (
                    SELECT CASE
                        WHEN item.preparation_status <> 'IN_PREPARATION'
                            THEN item.preparation_status
                        WHEN EXISTS (
                            SELECT 1
                            ...
                            WHERE production_plan.material_analysis_id =
                                      item.preparation_analysis_id
                              AND production_plan.material_analysis_item_id =
                                      item.preparation_analysis_item_id
                ...
                """ + statusClause + keywordClause + planItemClause + sourceNodeClause;
        Object[] filters = filters(status, like, query.planItemId(),
                query.sourceAnalysisId(), query.sourceMaterialLineId());
        Long count = jdbc.queryForObject("SELECT COUNT(*) " + base, Long.class, filters);
        long total = count == null ? 0 : count;
        List<Task> content = jdbc.query("""
                SELECT item.id, plan.order_id, item.order_item_id,
                       plan.order_bill_no,
                       item.goods_id, goods.code, goods.name,
                       item.color_id, color.name, item.unit_id, unit.name,
                       item.planned_qty, item.prepared_qty, item.issued_qty,
                       order_header.deliver_date, effective.status,
                       item.preparation_warehouse_id, warehouse.name,
                       item.preparation_analysis_id, item.preparation_analysis_item_id,
                       item.preparation_version, item.updated_at,
                       source_link.link_count,
                       source_link.source_analysis_id,
                       source_link.source_material_id,
                       handoff_state.handoff_id,
                       handoff_state.takeover_qty,
                       handoff_state.entitlement_qty
                """ + base + """
                ORDER BY order_header.deliver_date ASC NULLS LAST,
                         item.updated_at ASC, item.id
                LIMIT ? OFFSET ?
                """, (rs, rowNum) -> {
                    String taskStatus = rs.getString(16);
                    UUID warehouseId = rs.getObject(17, UUID.class);
                    UUID analysisId = rs.getObject(19, UUID.class);
                    long sourceLinkCount = rs.getLong(23);
                    UUID sourceAnalysisId = sourceLinkCount == 1
                            ? rs.getObject(24, UUID.class) : null;
                    UUID sourceMaterialId = sourceLinkCount == 1
                            ? rs.getObject(25, UUID.class) : null;
                    UUID handoffId = rs.getObject(26, UUID.class);
                    BigDecimal takeoverQty = decimal(rs.getObject(27));
                    BigDecimal entitlementQty = decimal(rs.getObject(28));
                    String handoffStatus = handoffStatus(
                            sourceLinkCount, analysisId, handoffId, takeoverQty);
                    String handoffBlocker = handoffBlocker(
                            sourceLinkCount, analysisId, handoffId);
                    return new Task(
                            ...
                            rs.getObject(8, UUID.class), rs.getString(9),
                            rs.getObject(10, UUID.class), rs.getString(11),
                            rs.getBigDecimal(12), rs.getBigDecimal(13), rs.getBigDecimal(14),
                            rs.getObject(15, LocalDate.class), taskStatus,
                             blocker(taskStatus, warehouseId), warehouseId, rs.getString(18),
                             warehouseId == null,
                             sourceAnalysisId, sourceMaterialId,
                             handoffStatus, takeoverQty, entitlementQty, handoffBlocker,
                             analysisId,
                             rs.getObject(20, UUID.class),
                             allowedActions(taskStatus, analysisId, canStart, canOpen,
                                     sourceLinkCount <= 1),
                             rs.getLong(21), rs.getObject(22, OffsetDateTime.class));
                }, append(filters, size, (long) (page - 1) * size));
