...
V188/V189 的销售仓库事件与退货质量事件另有业务级 append-only 守卫；V190 随后重新遍历公开业务表，
只为缺失者挂批准的脱敏审计函数，并严格要求每表恰有一个启用的 AFTER ROW I/U/D 审计触发器。
这些机制补的是以后发生的状态/原因时间线与 before/after，不替代彼此，也不反向补造迁移前历史。
...
采购批量转单必须同时具备来源采购申请 `view` 与目标采购订单 `edit`；只有其中一个权限不得出现批量
动作。仓库/委外行没有真实可执行入口时显示只读状态或锁定说明，不渲染“点击后无动作”的伪按钮。
...
- [x] 审计：业务 API 全方法请求层 + 显式安全/业务事件 + V169/V183 业务表覆盖、V184 触发器强校验、V185 软删除/脱敏、V188/V189 追加式业务事件及 V190 新表后完整 sweep 已落源码并完成隔离候选测试
...
| 审计中心与应用操作留痕 | 🟡 主体已实施 | V169/V171/V172/V173 覆盖请求、显式事件、只读核查、设备证据、加密导出及两阶段留存；V183–V185 补新表覆盖、触发器强校验、软删除和脱敏加固；目标库迁移、生产容量、边缘联查与恢复演练待验收 |
...
**最后更新**：2026-08-01（补聚合工作台精确授权/真实点击、V187–V189 销售商业/预留/仓库事件/退货冻结边界及 V190 审计完整 sweep；保留目标迁移、全模块权限负向 E2E 和岗位 UAT 为生产门禁）
