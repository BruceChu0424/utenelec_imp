...
- 合格审核人 = 财务部门(DEPT_FIN 及子树)内在职、账号启用且持有 `finance_order_approval:review` 的员工，或在权限管理被个人加授(`user_permission_overrides` effect='grant')该权限的人(不限部门)。提交时不再冻结 `assignee_user_id`；超级管理员不在财务部且未点名不能对象绕过。
- 审批权由 `finance_order_approval:review` + 财务部门资格决定，在权限管理授权；不再单点配置负责人（V229 删除 `workflow_assignment:manage` 与 `workflow_responsibility_assignments`，详见 ADR-027）。
...
V196–V202 已包含在开发原库 `uten_imp` V244 和公司目标库 V238；当前源码到 V252/233（V251 货品导入，V252 审计覆盖），一次性生产物料克隆仍是 V250/231 历史证据；但迁移存在不等于业务验收。
负责人离职/停用处置、真实多账号/并发、仓库与供应商退回实物 UAT、完整 IQC 和发布签字仍未完成，
...
> 本轮审计与修复全貌见 [2026-08-07 权限安全与并发加固审计报告](../99-项目治理/2026-08-07-权限安全与并发加固审计报告.md)。
...
- `production_material_analysis:*` 细分 view/manage/route/notify/generate/reallocate/bom_override。路由 any-of 只允许进入工作台；每个按钮和 API 仍按动作权限、对象范围、analysis version/指纹及服务端动作条件独立拒绝。
...
- V247 的阶段/包装边、V248 的 EXACT_SNAPSHOT 和 V249 的 ZERO_MATERIAL 证据必须由服务端/数据库计算并冻结；ZERO_MATERIAL 仅允许 `DIRECT_MAKE`、`PLAN_BOM_OVERRIDE`、`NO_PRODUCTION_HARD_GATE`，前两类冻结分析/授权事实，第三类校验并冻结“BOM 存在且无生产硬门槛”的形状。客户端提交的 ready 数量、平均单耗或“无需求行”不能成为事实。
...
- 开发原库仍为 V244，公司目标库仍为 V238 既有证据，V247–V250 只有一次性隔离克隆证据；目标部署、历史来源对账、完整收货/IQC/反向和岗位 UAT 未完成，生产 **NO-GO**。
...
**最后更新**：2026-08-10（V247–V250 生产分析/执行快照/来源谱系、订货财务 PENDING 临时详情范围及
